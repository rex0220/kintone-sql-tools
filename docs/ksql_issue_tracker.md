...
| B20 | 正規表現関数（`REGEXP_LIKE`/`REGEXP_REPLACE`/`REGEXP_SUBSTR`） | 改善 | 📝 仕様案 R2（codex レビュー前）。**本体は ReDoS 対策**＝利用者パターンは 1 行 14 秒の指数時間を書けてしまい、kSQL のタイムアウトは同期 `RegExp` を止められず**ブラウザが固まる**（MySQL の `regexp_time_limit` 相当が JS では不可能）→**安全部分集合を構文的に強制**（実需 22 パターンで検証済み）。**重い未決＝方言はホストエンジン依存**（プラグイン=ブラウザ / CLI=Node）で **v2.0.0 の「同じ SQL は同じ結果」原則に構造的に反する最初の機能**。フラグは意味に関わる `i`/`m`/`s` のみ受理し `u` 強制（`g`/`y`=駆動方法・`d`=結果の形・`v`=エンジン版依存）。**B21 の後**（B21＝正しさ・B20＝機能） | 機能 | 中 | [spec](internal/ksql_regexp_function_spec.md) |
