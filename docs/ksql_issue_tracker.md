...
| B3 | バッチ変数：配列展開 `IN (@list)`（1 変数＝複数値） | 改善 | 📝 **【B: 棚上げ】**提案（**仕様なし**＝1a 仕様 §2.2/§6 で対象外・配列型の導入が要る。R4 の `IN (@a, @b)` スカラー並べは **v2.1.0 で出荷済み**）。**推奨構文案あり**（§6・2026-07-19）＝定義は既存配列リテラル再利用 `SET @l = ['1','2']`・参照はカッコ無し `IN @l`（曖昧さ回避）・リテラル文法は新規不要 | 機能 | 低 | [1a spec §6](internal/ksql_batch_variables_phase1a_spec.md) |
