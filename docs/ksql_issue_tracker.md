...
| B36 | `REGEXP_REPLACE` に `occurrence` 引数（先頭／N 番目だけ置換） | 改善 | 📝 **提案（v3.3.0 の B20 実機検証中に派生・2026-07-18）**。現状 `REGEXP_REPLACE` は `global` 固定＝**全置換**（`evalFunc.ts:220` が `compileRegexp(..., true)`）で、先頭 1 件や N 番目だけを置換する手段がない。MySQL/Oracle は `REGEXP_REPLACE(..., pos, occurrence, ...)`（`occurrence`=0 で全部・N で N 番目）を持つ。**回避策あり**＝`^(.*?)PATTERN` アンカー＋`$1` で先頭 1 件のみ（実機確認: `REGEXP_REPLACE('aXbXcX','^(.*?)X','$1_')`→`a_bXcX`）→優先低。案＝第 5 引数に `occurrence`（省略時 0＝全置換で後方互換）を追加。`g` はエンジン制御のままユーザーには露出しない | 機能 | 低 | [B20 spec](internal/ksql_regexp_function_spec.md) |
