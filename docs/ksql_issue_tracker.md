...
|---|---|---|---|---|---|---|
| B19 | スカラー関数の拡充バンドル（`TRUNCATE`/`LEFT`/`RIGHT`/`INSTR`/`GREATEST`/`LEAST`/`LPAD`/`RPAD`/`LAST_DAY`）＋**既存関数の文書誤り修正** | 改善＋**バグ** | 📝 仕様案 R1（codex レビュー前）。第2部＝**`DATE_ADD` の構文が文書と実装で食い違い**（文書どおり書くと必ず ParseError）・**不正な単位が黙って DAY 扱い**・`SUBSTRING` の負数開始が MySQL と異なる | 機能・**正しさ** | 中 | [spec](internal/ksql_scalar_function_bundle_spec.md) |
