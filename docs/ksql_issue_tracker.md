...
|---|---|---|---|---|---|---|
| B14 | 一時テーブル/CTE 列の型メタ伝播（B13 フェーズ2・temp のテキスト `MIN`/`MAX`・**B16 の前提**） | 改善 | 📝 仕様案 R1（codex レビュー前）。`MaterializedTable` に列型メタを付与し B13 リゾルバへ配線 | 正しさ | 中 | [spec](internal/ksql_temp_column_type_meta_spec.md) |
