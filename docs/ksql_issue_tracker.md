...
- 次回リリース計画: **v3.2.0 = B34＋B22＋B23＋B24＋B28＋B35**（B23/B24 は純加法的な minor 機能。B28/B35 は同居）。以降の目安: **v3.3.0 = B9＋B29**（numberPrecision 領域ペア・仕様作成から）
...
|---|---|---|---|---|---|---|
| B21 | **`UPDATE SET` が文字列関数を直接受け付けない** | **バグ**（一貫性） | 🚧 **実装・実機完了（2026-07-18・[実機記録](internal/evidence/b21_update_set_string_func_smoke.md)）・v3.2.0 リリース待ち**。AssignmentValue 限定で STRING_FUNC を受理し既存の行評価経路（算術 SET）へ統合。参照フィールドは GET 収集・VALIDATE ONLY/ON ERROR SKIP が評価値を検証（実機で ERR_LENGTH_MAX 捕捉）・UPDATE…FROM/サブテーブルは明示 ParseError・算術内 STRING_FUNC は従来どおり拒否・エラー文言修正。実機=UPPER 書込・**LPAD('7',5,'0')→00007 の先頭ゼロ保持** | **正しさ** | 中 | [issue](internal/ksql_update_set_string_func_issue.md) |
...
| B28 | **DML の単項符号（負数・正数リテラル）の受理範囲が経路ごとに違う** | **バグ**（一貫性） | 🚧 **実装・実機完了（2026-07-18・[実機記録](internal/evidence/b28_unary_sign_smoke.md)）・v3.2.0 リリース待ち**。VALUES（親/サブ/UPSERT）で符号付き数値リテラル受理・UPDATE SET は単項 + を数値直前のみ追加・符号ネストは明示 ParseError（-- は行コメント）。実機=−5/+0.5 書込→読返し・+7 UPDATE・ネスト拒否 | **正しさ** | 低 | [issue](internal/ksql_dml_unary_sign_issue.md) |
...
| B35 | **プラグインが message なしのネットワークエラーを「⚠」のみで表示する** | **バグ**（表示・診断性） | 🚧 **実装済み・実機確認待ち（v3.2.0）**。message なし・空文字列・undefined の reject を cause 付き Error とネットワーク fallback 文言へ正規化し、表示層にも汎用文言の最終防衛を追加。kintone 正規エラーと Cursor 系 Error の表示は維持し、`[object Object]` を露出しない | **正しさ**（診断性） | 低 | [issue](internal/ksql_plugin_messageless_error_display_issue.md) |
