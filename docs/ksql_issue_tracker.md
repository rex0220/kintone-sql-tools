...
- 最終更新: 2026-08-05
- 現在の最新リリース: **v3.45.0**（2026-08-05・**リポジトリ内で release 化済み・push / PR / タグ / GitHub Release / npm publish は未実施**）。→ [リリース履歴](ksql_release_history.md)
- 次回リリース計画: **未定**。要対応は **B124**（集計算術式の非集計オペランド・**実需は `ksql-analytics` で実証済み**・優先 中・`HAVING` の扱いを仕様で先に決める）。他は優先 低 か実需待ち（§1）。クローズ済みは §3。
