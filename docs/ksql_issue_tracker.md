...
| B26 | **文字列の並び順を kintone のコードポイント順へ統一する**（旧 B25 を統合） | **バグ** | 📋 **方針決定済み**（横断仕様 §4.4・2026-07-17）。**kintone REST API は Unicode コードポイント順**（実機実測: 空文字, 亜, ｱ, 😀, 𠮟）。現状 kSQL は **3 系統すべてが kintone と食い違う**＝①`ORDER BY` の JS 経路は **ICU `localeCompare('ja')`**（`process.ts:597`/`execute.ts:4117`・**BMP でも不一致**）②`MIN`/`MAX`（`process.ts:344`）・`GREATEST`/`LEAST`・`WHERE` の `<`/`>` は **UTF-16 コードユニット順**（**サロゲートで不一致**＝実機 `MAX`=`ｱ` だが kintone の最後は `𠮟`）③**`executeSimpleSelect`（`execute.ts:1288`/`:1328`）が `LIMIT`≤500 なら kintone の並びをそのまま採用し、それ以外は JS で並べ直す** → **`LIMIT 500` と `LIMIT 501` で並びが変わる**（v1.14.0/v2.0.0 が major を切って潰した「LIKE のモード不一致」と同型）。**統一すると モード不一致・ホスト依存・サロゲート差の 3 つが同時に消える**。比較器は全順序を検証済み（2,197 組の推移律）。SemVer=**major 相当**（日本語の並びが辞書順でなくなる） | **正しさ** | **高** | [semantics §4](internal/ksql_string_semantics.md) |
