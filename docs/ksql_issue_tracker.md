...
| B20 | 正規表現関数（`REGEXP_LIKE`/`REGEXP_REPLACE`/`REGEXP_SUBSTR`） | 改善 | ⛔ **R3 — 設計が破綻・着手不可**（codex レビュー #6）。**安全部分集合 R-1/R-2 では ReDoS を塞げない**＝量化グループも後方参照も無い `^a?×n a×n b$`（入力 `a×2n`）が指数時間（実測 n=26 で 470ms・既知の `(a+)+$` と同等）。**任意の曖昧 NFA を構文規則で検出することはできない**（却下した NFA 解析ライブラリこそが実際に解いている手段だった）。**「安全部分集合」というアプローチ自体を codex が設計し直す**（棚上げ・CLI/MCP 限定も可）。別問題として **方言のホスト依存**（`new RegExp` の解釈がホスト任せ・後読みは Safari 16.4 未満で SyntaxError）が残る | 機能 | 低 | [spec](internal/ksql_regexp_function_spec.md) |
