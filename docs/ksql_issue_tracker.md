...
| B3 | バッチ変数：配列展開 `IN (@list)`（1 変数＝複数値） | 改善 | 📝 **【B: 棚上げ→設計具体化】v1 仕様 R1・codex レビュー済・要 R2**（2026-07-19）。構文＝定義 `SET @l = ['A','B']`（既存 `ArrayLiteral` 再利用・STRING 限定）／参照 カッコ無し `IN @l`（`IN (@a,@b)` と無曖昧）。R4 の `IN (@a,@b)` スカラー並べは v2.1.0 出荷済み。**R2 必須6点**＝空配列の親aware boolean簡約・親DML `NOT IN []` 安全契約・EXPLAIN 配列SET評価・validate-all-first 静的型検査・WHERE以外の条件位置範囲・IN上限は「分割なし/API依存」明記。工数 3.5〜6.5人日（B41 の0.7〜0.8倍） | 機能 | 低 | [spec](internal/ksql_array_variable_in_expansion_spec.md) |
