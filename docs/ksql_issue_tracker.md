...
- 最終更新: 2026-07-20
- 現在の最新リリース: **v3.6.1**（B39 IMPORT の面 UX 改善・修正＋ドキュメント整備・tag/GitHub Release 公開済み・**npm publish 済み（latest 3.6.1・2026-07-19）**。v3.6.0 は publish スキップ）。前版 v3.5.0（B41/B3/B10-B）。
- 次回リリース計画: 未定。**後続候補**＝B42 VALIDATE のサブテーブル子フィールド監査（B41 v2・要仕様）・B40 プロパティグラフ（大規模・要 R2）。**棚上げ**＝B4/B5/B6/B7。
...
進捗が動くのはここ。優先度は「正しさ/安全性 > 機能 > 性能改善の上積み」で暫定。**残るは A: 実装候補（B42=B41 の v2 スコープ・B40=設計探索済みの大型候補・要 R2）と、B: 棚上げ・低優先の提案（B4/B5/B6/B7）**。直近の実装候補だった B39（IMPORT）は v3.6.0/v3.6.1、B41/B3/B10 は v3.5.0 でリリース済み（§2）。次の着手候補は B42（実需あり・監査の抜け）か B40（大規模・探索的需要）。

| # | 課題 / 改善案 | 種別 | 状態 | 効果 | 優先 | 文書 |
|---|---|---|---|---|---|---|
| B42 | `VALIDATE` がテーブル（サブテーブル）内フィールドの制約をチェックしない | 改善 | 📝 **【A: 実装候補】起票（2026-07-20・実需あり＝実利用で指摘）**。`VALIDATE APP…` はテーブル内フィールドを監査対象にしない＝**B41 v1 の明示的対象外**（spec §1/§8「サブテーブルセル監査は強い v2 候補」・言語 §17.4 も「トップレベル」と記載済み＝文書と実装は一致・silent bug ではない）。実装裏取り＝既定対象から `inSubtable` を除外（[execute.ts:745](../src/execute.ts#L745) `resolveExistingValidationTargets`）・`(fields)` での明示指定も ArgumentError で拒否（[execute.ts:756](../src/execute.ts#L756)）→**テーブル内の必須/数値上下限/文字数/選択肢/B29 桁の違反は `#err` に一切出ない＝監査の抜け**。検証ロジック本体（`validateAndNormalizeDmlValue`）は子フィールドの `KintoneFieldInfo` にそのまま流用可の見込み。仕様論点＝①エラー行にサブテーブル行位置をどう載せるか（固定5列に行 ID/序数を追加 or `$err_field`=`テーブル.子` 表記・B41 spec §8 は「行番号付き」を想定）②取得フィールドへテーブル本体コードを追加（子コード指定では取れない）③`(fields)` でのテーブル/子フィールド指定の可否④`CHECK` のサブテーブル参照は対象外のまま（B37 も非対応）⑤NUMBER 子フィールドの B29 数値精度適用。**仕様 R1 起草済（codex・2026-07-20）→ Claude レビュー済（spec §11・P1×3=行ロケータへ `$err_subrow_id`(=`_rid`) 追加で8列一括確定・`VALIDATE APP100$明細` 代替構文の採否理由明記・**エラー行爆発（レコード×行×子の積が tempTableMaxRows=10,000 で完走不能）対策に生成時集約 `SUMMARY` モード**（ユーザー発案・承認済み）／P2×2=SemVer は前例(B8/B9/B13=minor)に合わせ minor へ・0行非発火は B12-A 実測の再確認扱い／P3×1）→ **R2 反映済み・Claude 再レビュー済（指摘なし・詳細8列＋SUMMARY 5列・soft keyword・minor v3.7.0 相当。ユーザー承認で実装着手可）**。R1 が実装確認で発見した副次ギャップ=WHERE の子参照に静的ガードなし（infoByCode が子を含み存在チェック素通り）＝B42 実装時に ArgumentError 固定 | 安全性・機能 | [spec](internal/ksql_validate_subtable_audit_spec.md) / [B41 spec §1/§8](internal/ksql_existing_record_validation_spec.md) |
