...
- 次回リリース計画: **未着手**。**B91** は Pro の M6 待ち。残るバックログは B53（18〜29 人日・実需待ち）・B54・B61・B63・B73（保留）。**JOIN の打ち切り**（LEFT JOIN で右が打ち切られると `NULL` が入る）は [B97 §5](internal/ksql_b97_incomplete_aggregate_failclosed_spec.md) で対象外とした残件で、**起票する価値がある**。- **Pro（ksql-dashboard-pro）**: v3.31.1 の検証完了（依頼①②とも動作・§4§5 該当なし・CI 199 件通過）。回帰報告は取り下げ → **B93** で伝え方を改善。**新たに依頼④（取得上限の構造化フラグ）を受領 → B95 として起票**。**急ぎではない**（Pro 側は `onLimitReached: "error"` で解決済み）。**v3.32.0 を取り込み済み**（CI 26 ファイル / 230 テスト通過・破壊的変更なし）。**§1 §2 とも実機で再現**〔**APP912 実件数 10,228 → 10228・GET 1 回**（従来は 10000 の誤り）／**APP730 実件数 618,525 → 1 リクエスト・676ms**（従来は `FetchAllLimitError`）／JOIN の `fetchedRows 233 > maxRecords 230` でも `limitReached=false`〕。**「大規模アプリでは総件数を出せない」という記述をレシピ集から削除**したとのこと。**新たに B96（Pro 側で発見・共有）と B97（案 B の要望）を受領**。**M6 は A 群 9 項目のうち 7 項目完了**、残り実機確認 2 項目。依頼③（B91）は再評価待ち。**v3.33.0 の連絡文を用意済み**（[ksql_pro_report_v3330](internal/ksql_pro_report_v3330.md)・送付待ち）＝**K-37 の前提が解けたので `"truncate"` へ戻せる**ことが主題。**Pro はモード追加を前提に依頼していたが、調べたら不要だった**ので §2 で理由を説明している。
