...
- 次回リリース計画: **B93 実装済み（未リリース・patch 相当）**。着手候補は **B94**（`SELECT COUNT(*)` の 1 リクエスト化・1.25〜2.0 人日・効果大）。**B91**（依頼③の再評価）は Pro が「急ぎではない」としており M6 待ち。
