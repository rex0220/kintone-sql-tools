...
- 最終更新: 2026-07-19
- 現在の最新リリース: **v3.5.0**（既存レコード制約チェック B41＋バッチ変数の参照拡張 B3/B10-B・tag/GitHub Release 公開済み・**npm publish 済み（latest 3.5.0・2026-07-19）**）。前版 v3.4.0（B37/B38）。
...
進捗が動くのはここ。優先度は「正しさ/安全性 > 機能 > 性能改善の上積み」で暫定。**残るは A: 設計探索済みの大型実装候補（B40・要 R2）と、B: 棚上げ・低優先の提案（B4/B5/B6/B7）**。直近の実装候補だった B39（IMPORT）は v3.6.0、B41/B3/B10 は v3.5.0 でリリース済み（§2）。次の着手候補は B40（大規模・探索的需要）か、B 群の実需確認。
