...
- 最終更新: 2026-08-05
- 現在の最新リリース: **v3.50.0**（2026-08-06・npm publish 待ち）。→ [リリース履歴](ksql_release_history.md)
- 次回リリース計画: **未定**。要対応は **B138**（文書の構造検査を `npm test` へ。リンク切れ 184 件の仕分けが前提）。**B128 は `LAG`/`LEAD` 先行へ組み替え**、**B134** は B53 との関係整理が先。他は優先 低 か実需待ち（§1）。クローズ済みは §3。
...
|---|---|---|---|---|---|---|
| B128 | 集計ウィンドウ Phase 2a（**`LAG` / `LEAD`**・移動フレームは棚上げ） | 改善 | 🚧 **実装完了・実機検算済み（リリース待ち）**（2026-08-06）→ [仕様 R2](internal/ksql_b128_lag_lead_spec.md) / [レビュー2](internal/ksql_b128_codex_review_2.md)（高 1・中 7・低 1 を全件反映）。**旧レビューの「高」4 件は方向として解消**との評価。**R2 で直した点**＝①offset 超過の規範が §2.1 と §6-3 で矛盾していた→空文字に確定②完全入力の理由名が誤り＝WINDOW_ORDER であって AGGREGATE_WINDOW ではない③**B129 の nested 検出は LAG/LEAD を自動では拾わない**＝scanner への追加が要る④代表の形は 2 段ではなく**3 段**⑤**B127 の判定器は独立 helper ではなく RANGE 専用 warning 内の inline 判定**なので抽出が要る⑥受入が内部実装を要求する文だった→出力を観測する形へ。**1 段目と 3 段目の形は現行で動くことを実測済み**（月次集約＋ウィンドウで 13 行）。**同じ実測で依頼元の前提も裏取り**＝履歴 13 か月すべてに行があり歯抜けが無い＝月次なら B134 の 0 埋め問題を踏まない。**旧レビュー 16 件のうち 8 件が本スコープに残る**（移動フレーム固有の 8 件は棚上げ側へ）。**先に潰す「高」4 件**＝①**現行 `isRankingWindow` は「AGGREGATE でなければ RANKING」の二者択一**なので `VALUE` を足すと順位系と誤判定される→positive discriminator にして `applyWindow` を 3 分岐＋未知 kind は fail-closed②**引数フィールドが required-field 収集から漏れる**（現行 walker は集計系だけ `arg` を走査）→`LAG(出庫)` の `出庫` が取得されず**B125 の集計引数漏れと同じ静かな誤り**になる③**型メタが一律 number になる**（「AGGREGATE でなければ number」の二者択一が複数ある）→同期すべき 6 箇所を名指しした。**とくに source metadata のロードゲートは集計窓の MIN/MAX だけが対象**で、直さないと direct APP の `LAG(選択肢列)` で resolver が元メタを持たない④**第 3 引数 `default` は取らない**＝`LAG(数値, 1, 'N/A')` は通常行が数値・先頭行が文字列で**列全体の型が一貫しない**（下流の比較が number comparator で文字列を扱う）。既定値が要るなら次の段で `CASE` を書く。**代表の形は 2 段構成**（月次集約 → `LAG` → 次段で前月比）＝**ウィンドウ結果を式に含められない**（B129 の契約）ため。（スコープ組み替えの経緯は以下）📝 **スコープ組み替え（2026-08-05・依頼元の回答で確定）**＝**`LAG`/`LEAD` を先行し、移動フレームと値指定 `RANGE` は棚上げ**。**依頼元が実測で「日次集約＋`ROWS` では 7 日移動平均にならない」ことを示した**〔取引のあった日数が稼働期間の半分以下で、**7 行が相当する暦日が製品ごとに 15〜29 日と 2 倍の開き**。同じ `PARTITION BY` の 1 本の中で窓幅が製品ごとに変わるため、**製品を横に並べて比較する表**に使えない。さらに直近 90 日で出庫ゼロの 2 製品は**行ごと消える**（0 埋めできない → [B134](internal/ksql_b134_series_generation_issue.md)）〕。**こちらで全数値を再測して一致を確認**（食パン 220 件/168 日・野菜ジュース 100 件/89 日ほか）。**依頼元の言葉＝「依頼③で移動平均を先に挙げたのは当方の見立ての粗さで、実際に効くのは前期比の方でした」**。**`LAG` 先行の根拠**＝月次集約なら歯抜けがほぼ起きない（履歴 13 か月すべてに行がある）／短期の窓は `SUM(CASE WHEN 日付 >= @d90 ...)` の固定窓で足りており平滑化を必要としていない。**値指定 `RANGE` は実需として弱く当面見送り**。（旧・保留時の分析は以下） → [仕様 R1](internal/ksql_b128_window_phase2_spec.md) / [レビュー](internal/ksql_b128_codex_review_1.md)（**高 9・中 7**）。**保留の理由はスコープ**＝**`ROWS` の行数指定だけでは実需「7 日移動平均」を満たせない**（同日複数行・取引の無い日があると**直近 7 行 ≠ 直近 7 日**。現行実行はソート後の index を走査し日付差を見ない）。満たすには日次集約 CTE との組合せか**値指定 `RANGE`**（`INTERVAL n DAY PRECEDING`）が要る。**再開はスコープ決定から**〔案 A＝`ROWS` のみ＋レシピで日次集約を示す（実需は条件付き充足）／案 B＝値指定 `RANGE` まで（大幅に重い）／案 C＝`LAG`/`LEAD` を先行（前月比は行数指定で素直に書ける）〕。**R1 の誤り 2 件はレビューで判明**〔①代表 SQL `ROUND(AVG(...) OVER (...), 1)` が**現行パーサで拒否される**（B125 の「ウィンドウ結果を式に含めない」契約に自分で違反）②「7 日移動平均」と「直近 7 行」の混同〕。**資産は流用可**（R1・レビュー 16 件は再開時の出発点）。（起票時の分析は以下）[B125 仕様 R2](internal/ksql_b125_aggregate_window_phase1_spec.md) §10 で Phase 2 と明記済みのもの。**実需の裏づけ＝B125 で累積が SQL に戻った結果、明細を CSV へ落とす理由が 2 つだけになった**（移動平均＝7 日平均の在庫・出庫／前期比＝前月比・前年同月比）。**フレームが固定境界のみのため CTE を重ねても書けず、自己結合も `JOIN ... ON` が等値 1 本なので「前月」を作れない**。優先は移動フレーム（在庫・需要の平滑化は定番）→ `LAG`。（→ [評価](internal/ksql_analytics_request_20260805_evaluation.md)） | 機能 | 中 | [評価](internal/ksql_analytics_request_20260805_evaluation.md) |
