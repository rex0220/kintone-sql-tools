...
|---|---|---|---|---|---|---|
| B114 | `EXPLAIN` が取得範囲を名乗らない — 出力設計の見直し | 改善 | 📝 **仕様確定（段階分け・オーナー Go 2026-08-04）**。出典は Pro の依頼 K-110 だが、**依頼の形をなぞらず EXPLAIN の設計として検討した**（オーナー指示）。**問題**＝押し下げが効いていても `mode: FULL_SCAN` と出る形が**正常系として頻出**し（ペインは大半が GROUP BY / 集計を含む）、読み手（Pro 経由では非エンジニアの kintone アプリ管理者）が恒常的に誤読する。**原因はこちらの命名**＝`FULL_SCAN` は内部の評価戦略名なのに、読み手にはコストの言明に見える。**構造的な欠陥 4 点**〔①最も目立つ `mode:` が読み手の問い（何件取りに行くか）に答えていない②**テキストが正で構造が無い**（99 ラベルがフラット・下流は正規表現で読むしかなく Pro は断念・B112 の面比較が壊れたのも同じ根）③**確定と見込みが混ざる**（`pushdown candidate` は実行時に落ちうる）④ソース単位と全体の事実が同じ平面（**実測: 1 クエリの UNION で `[union:1]` は取得 0 件・`[union:2]` は全件**＝スカラー 1 値に畳むと必ず嘘になる）〕。**方針＝段階分け**〔**第 1 段**=ソースごとに `fetch:` 行（`NONE` / `EXACT` / `PREFILTERED` / `ALL`・接尾辞 `(limit N)` と `(未確定)`）＋文ごとに最悪値の要約行。**判定材料は出力位置に既に揃っており新しい解析を足さない**。型変更なし・3 面に効く／**第 2 段**=`ExplainResult` へ構造化プランを純加法（optional・常に埋める）、`text` はそこから描画。詳細は第 1 段完了後に確定〕。**やらないこと**＝`mode` の改名（互換性・Pro も希望せず）／**件数の予測**（取得前に行数は分からない＝言えないことは言わない）／利用者向けと開発者向けで出力を 2 本に分ける（必ず片方が腐る）／99 ラベルの命名統一（第 3 段・互換リスク高）。**優先 中**（Pro は急ぎでないと明記）。 | 改善 | 中 | [B114](internal/ksql_b114_explain_fetch_scope_issue.md) |
