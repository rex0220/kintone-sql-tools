...
- 最終更新: 2026-08-05
- 現在の最新リリース: **v3.47.0**（2026-08-05・**リポジトリ内で release 化済み・push / PR / タグ / GitHub Release / npm publish は未実施**）。→ [リリース履歴](ksql_release_history.md)
- 次回リリース計画: **未定**。要対応は **B129・B130**（`ksql-analytics` の依頼由来・優先 中）。**B128 は保留**（スコープ判断が先）。他は優先 低 か実需待ち（§1）。クローズ済みは §3。
