...
3. **B12-B `ON ERROR SKIP INTO #err`** — **仕様 R7・codex レビュー反映（§7.4 実装契約）・実装着手可**。次＝codex 実装
...
| B12 | 行単位エラー隔離 `ON ERROR SKIP INTO #err` ＋ `VALIDATE ONLY`（Tier 0 事前検証） | 改善 | 🚧 **v2.13.0 バンドル**（①B12-A 済／②B11.1 済／③B12-B=仕様 R7・実装着手可） | 機能 | 中 | [spec](internal/ksql_on_error_skip_isolation_spec.md) / [plan](internal/ksql_validate_only_implementation_plan.md) / [roadmap](internal/ksql_batch_processing_roadmap.md) |
