...
- 次回リリース計画: **集計まわりの「静かに間違う」4 件（B119・B120・B121・B122）が実装済み・リリース待ち**（2026-08-05・全件レビュー合格・`npm test` 5316 通過）。**いずれもエラーを出さずに誤った値／行集合を返していた既存欠陥**で、B117・B118 から続く系列。`CaseResult` の union 拡張を含むため**移行案内が要る版**。他は優先 低 か実需待ち（§1）。クローズ済みは §3。
