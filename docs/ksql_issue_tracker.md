...
- 次回リリース計画: **集計まわりの「静かに間違う」3 件**。**B119・B120 は実装済み・リリース待ち**（2026-08-05・レビュー合格）。**B121（`HAVING` の数値比較）は未着手だが、B120 修正後に不整合が表面化するため同じ版に入れる**。3 件とも `CaseResult` の union 拡張を含むため**移行案内が要る版**。他は優先 低 か実需待ち（§1）。クローズ済みは §3。
