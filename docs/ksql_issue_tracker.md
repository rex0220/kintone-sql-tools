...
| B33 | **`KORDER BY` 大規模窓のCursor API対応** | 改善 | 📋 **R3 設計確定＋実装計画 R1（2026-07-18・codex 作成・Claude レビュー承認・[impl plan](internal/ksql_korder_cursor_implementation_plan.md)）・v3.1.0 の主役候補**。Phase 0〜4 を 21 ステップへ展開・受入条件 18 項目の対応表・`openCursor` 必須化の全実装列挙済み。単発Records APIで完結する窓は`KORDER_NATIVE`、それ以外は`OFFSET + LIMIT <= maxRecords`を満たす場合だけ`KORDER_CURSOR`。Cursor APIを使うのはこの1経路だけとし、通常ORDER/FULL_SCAN/JOIN/DMLは現行方式を維持する。**実装前blocker解消（2026-07-18 実測・R4）＝Delete済み・自動削除済みとも `404`+`GAIA_CN01` で同一・このペアのみ既解放扱い**。容量上限も実測確定（11個目=`429`+`GAIA_TM12`）・5分timeout残存は再現不能で未検証のままquarantine維持（618K件+複合4キーでも Create 9秒）。release blocker＝500件ページ境界をまたぐkintone順・同値安定性の実機確認（fixture候補=APP730） | **機能・性能** | 中 | [spec](internal/ksql_cursor_api_fetch_spec.md) |
