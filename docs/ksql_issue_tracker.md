...
- 最終更新: 2026-08-01
- 現在の最新リリース: **v3.42.0**（2026-08-04・npm publish 済み）。→ [リリース履歴](ksql_release_history.md)
...
|---|---|---|---|---|---|---|
| B118 | 関数呼び出しの診断が弱い（引数の数を黙認する／未知の関数名で無関係なエラー） | 課題 | 🔧 **対応中（①②同梱・オーナー決定 2026-08-04）**。**Pro の反省（「構文エラーを機能の非対応と読み替えていた」）を読み、原因を実測して発見**。**②引数の数を黙認する**（`DATE_ADD(受注予定日, 1)` → `2026-08-13` で単位なしが DAY 扱い／`ROUND(売上, 1, 2)` → 余分な引数を黙って無視。`ksql_validate` も通る）＝**B117 と同じ「静かに間違う」形**で、B117 を「その形は残さない」理由で直した直後に同種を残すのは一貫しない。**①未知の関数名・誤った引数構文で無関係なメッセージ**（`DATE_SUB(...)` → 「文の区切りには ; が必要です」／`DATE_ADD(d, INTERVAL 1 MONTH)` → 「「)」が必要です」）＝**Pro の誤読の直接原因**で、実際に欠陥（B117）の発見を 1 往復ぶん遅らせた。**原因は設計の欠落ではなく適用漏れ**＝`assertArity`（evalFunc.ts:489）は既にあり **16 箇所で使用／関数の case 分岐は 65**、`?? "DAY"` のような既定値が書き間違いを正当化していた。**方針**＝②は全分岐で引数の数を検証（**正本は言語リファレンスの構文列 85 行**・既存 `assertArity` を使い新しい仕組みを作らない・省略可能な引数は不変・**正しい呼び出しは完全に不変**）、①は識別子＋`(` の形で失敗したときに**関数名または期待する書式を名指し**（関数名一覧は既存定義を単一ソースに・自然な縫い目が無ければ止めて報告）。**破壊的変更**（引数を書き忘れた SQL がエラーになる）だが、それらは現状も意図しない値を返している。**優先 中〜高**・**minor＋移行案内**。 | 正しさ | 中 | [B118](internal/ksql_b118_function_call_diagnostics_issue.md) |
