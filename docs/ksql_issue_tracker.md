...
| B41 | 既存レコードの制約チェック（組み込み制約＋カスタムチェックの読み取り監査） | 改善 | 📝 **【A: 実装候補・本命】v1 仕様 R1・codex レビュー済・要 R2（2026-07-19・[参考記事](https://qiita.com/rex0220/items/de02e64dc34f3362d1f8)）**。kintone のフォーム制約は過去データに効かない→既存レコードを**組み込み制約（必須/数値上下限/文字数/選択肢）＋カスタムチェック（B37 構文）で監査**し違反報告（`$id`/フィールド/コード/メッセージ/現在値・CLI は CSV）。**B37 の読み取り版**（書込み時 B37 ⇄ 既存監査 B41 で対）。**実現性高**＝`validateAndNormalizeDmlValue`（必須/min-max/length/choice を既に実装）・型解決・`CHECK`・fetch を既存流用、新規は「既存レコードへ検証適用＋出力」経路のみ＝**B37 より軽い見込み**・engine 側全面。設計案＝`VALIDATE <app> [(fields)] [WHERE] [CHECK …] [INTO #err]`（read-only）。**v1 仕様 R2・codex レビュー待ち**（[spec §10](internal/ksql_existing_record_validation_spec.md)）＝再利用成立・B37 より軽い（工数 4.5〜8.5 人日 ≈ B37 の 0.7〜0.9 倍）。R2 で 3 重大を確定（Claude コード裏取り）＝①値二系統（組み込み=生値 `record[code].value`／WHERE・CHECK=flat `ProcessRow`・`isEmpty` が flatten `"[]"` を非空扱い→USER/ORG/GROUP/複数選択の必須取り逃がしを回避）②内部行契約 `{id, record, flat}` を fetch 直後保持（取得=$id∪制約∪WHERE∪CHECK）③read-only 配管（`VALIDATE` を `isReadOnlyType` へ・`isDmlType` に入れず `writesKintone=false`・`requiresCompleteInput=true`・書込み API 0）。#err 実体化・Cursor $id ページング限定・EXPLAIN も確定。**実需明確**（ユーザー自作 Chrome 拡張あり）・**直近提案で最も実装向き** | 安全性・機能 | 中 | [eval](internal/ksql_existing_record_validation_evaluation.md) |
| B3 | バッチ変数：配列変数と `IN @list`（B10-B と統合） | 改善 | 📝 **【A: 実装候補】統合仕様 R2・Claude レビュー承認・実装着手可**（2026-07-19）。B10-B（SELECT 列 `@var`）とバンドル＝「バッチ変数の参照拡張」。codex 作成 R2 が B3 R1 の必須6点を解決（空配列→BOOLEAN＋親aware固定点簡約・親DML root TRUE 拒否・EXPLAIN 配列SET評価・静的array/scalar検査・全WhereExpr位置・IN上限なし）。Claude が全主張コード裏取り・要修正1点（`@x||field` 退行防止の分岐順）反映済み。構文＝`SET @l=['A','B']`／`IN @l`。SemVer minor・工数 5.2〜8.2人日（統合） | 機能 | 低 | [統合仕様](internal/ksql_batch_variable_reference_extension_spec.md) |
...
| B10 | バッチ変数 後続：`NULL` 代入 / SELECT 列 `@var`（Part B は B3 と統合） | 改善 | 📝 **【A: 実装候補（Part B）／クローズ（Part A）】**（2026-07-19）。**Part A（NULL 変数代入）はクローズ**＝kSQL は値 NULL を持たない設計（空文字）、`SET @e=''`＋フィールド `IS NULL` で実需充足（実機: 両者とも空7件）。**Part B（SELECT 列 `@var`）は B3 とバンドルし統合仕様 R2・Claude レビュー承認**（`VARIABLE_COL`→既存 `LITERAL_COL`/`ARITH_COL` 変換で型メタ保持・AS 必須・`@x||field` 退行防止）。評価初稿の誤り（式内 @var は既に動く／親列ノード非変換／規模小〜中）は codex レビューで訂正済み | 機能 | 低 | [統合仕様](internal/ksql_batch_variable_reference_extension_spec.md) |
