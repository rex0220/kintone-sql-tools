...
- **Pro（ksql-dashboard-pro）**: v3.28.0 までの完了報告 → **返信受領（2026-07-27）** → **v3.29.0 の連絡文を用意（送付可・2026-07-28）**＝[v3.29.0 連絡](internal/ksql_pro_report_v3290.md)。**Pro が返信 §7 で報告した「engine ライブラリでは一時テーブルが使えない」制約を v3.29.0 で解消**＝要望がそのまま実装された形。確認依頼2点は回答済み〔JOIN の数値は**演算子の違い**（非厳密は IEEE-754 境界のため不可・**「v3.25 以前と入れ替わった」は誤解**で旧実装から厳密のみ）／選択系 `=` は **kintone のクエリ文法が `in` のみ**〕で **B84 として起票**。**B73 は Pro 都合でクローズ可**（**エラー `code` の値と意味は変えない**ことが契約要求）。**v3.29.0 の実機確認は Pro 側（engine library 利用）が実質的な受入**。
