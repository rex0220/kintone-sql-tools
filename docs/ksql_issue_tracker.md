...
|---|---|---|---|---|---|---|
| B29 | **kintoneの数値精度・丸め設定とDML/Tier-0を整合させる** | **バグ / 言語意味論** | 🚧 **仕様 R1 に基づく Codex 実装・mock受入テスト完了（2026-07-18）・Claude 実機確認待ち（[spec](internal/ksql_number_precision_semantics_spec.md)）**。numberPrecision(digits30/小数10/HALF_EVEN・UP・DOWN)を Tier-0 へ取り込み VALIDATE ONLY 偽合格を解消。設定取得は getFieldsCached 同経路・NUMBER 書込対象のみ・取得失敗 fail-closed。全 DML×VALIDATE ONLY/ON ERROR SKIP へ横断（B34 の形）。B9 の厳密10進 primitive(parseExactDecimal/ExactDecimal/compareExactDecimal)を再利用・二重実装なし。v1 は検証のみ(超過エラー・自動丸めなし)・算術任意精度化は v2 分離。HALF_EVEN は文字列/桁ベース(Math.round 不可)。SemVer=minor(従来 valid 行が invalid へ)。残作業は §11 の実機一致・surface確認・cleanup証跡 | **正しさ** | 中 | [spec](internal/ksql_number_precision_semantics_spec.md) / [issue](internal/ksql_number_precision_semantics_issue.md) |
