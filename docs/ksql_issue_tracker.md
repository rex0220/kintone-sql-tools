...
- 最終更新: 2026-07-16
- 現在の最新リリース: **v2.14.1**（B15 `IN` 負数リテラル・npm publish 済み・latest 2.14.1）
...
|---|---|---|---|---|---|---|
| B14 | 一時テーブル/CTE 列の型メタ伝播（B13 フェーズ2・temp のテキスト `MIN`/`MAX`・**B16 の前提**） | 改善 | ✅ R2 実装済み（codex・コードレビュー待ち、1,294 テスト green）。`MaterializedTable` に列型メタを付与し B13 リゾルバへ配線 | 正しさ | 中 | [spec](internal/ksql_temp_column_type_meta_spec.md) |
