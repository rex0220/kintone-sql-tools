...
|---|---|---|---|---|---|---|
| B21 | **`UPDATE SET` が文字列関数を直接受け付けない** | **バグ**（一貫性） | 📝 課題 R2（**評価経路の設計は codex へ委譲**）。**同じ式が `CASE WHEN` の中では書けるのに直接書くと `ParseError`**（`SET x=UPPER(x)` は不可・`SET x=CASE WHEN … THEN CONCAT(x,'★') ELSE x END` は実書き込み成功）。**R1 の「parser の accept list に足すだけ」は誤り**（codex レビュー #7）＝単純 UPDATE は `buildUpdateRecord`（`dmlToKintone.ts:172`）が**行を持たずに** `toKintoneValue` へ渡す。CASE 版が動くのは `:539` が取得済み `row` を渡す**別経路**。**新機能を待たず既存の全文字列関数に効く**（B19 の `LPAD`/`LEFT`/`RIGHT` 等。`SET code=LPAD(code,5,'0')` が書けない）。親 DML の `WHERE` は文字列関数を一律拒否するため**1 文での正規化にはならない** | **正しさ** | 中 | [issue](internal/ksql_update_set_string_func_issue.md) |
...
| B26 | **型付き比較・文字列順・型メタ・ソート比較器を4面で統一する**（旧 B25 を統合） | **バグ / 言語意味論** | 📝 **R4 設計決定**（横断仕様 §4.5）。既定文字列順はコードポイント、通常テキストは typed string、型不明は文字列、値ベースのペア判定を廃止。`GREATEST` は集合モードを維持してコードポイント tie-break。ORDER BY は型固定タプルの strict weak order とする。**MULTI_SELECT/CHECK_BOX の同 rank は実機確認が release gate**。typed number の REST 完全一致には B9 を依存課題として統合。SemVer=**major** | **正しさ** | **高** | [semantics §4](internal/ksql_string_semantics.md) |
| B27 | **ソート不能な型の `ORDER BY` を kintone へ押し下げて必ず失敗する** | **バグ** | 📝 課題 R1（横断仕様 §4.6.3・実測）。**kintone は MULTI_SELECT / CHECK_BOX のソートを拒否する**（`GAIA_IS02`）が、`resolveSelectMode`（`selectToKintone.ts:79`）は `orderBy` のキーが `FIELD_NAME` かしか見ないため **SIMPLE のまま押し下げる**。**EXPLAIN は `ok` を返して `order by チェックボックス asc` を表示するが、実行は必ず kintone API 400**（B20 §11 論点 4 と同型＝EXPLAIN が実行不能な計画を表示）。正しくは**型メタで押し下げ可否を判定し FULL_SCAN へ落とす**。**kintone がソートを拒否する型の一覧は未実測**（USER_SELECT/ORGANIZATION_SELECT/GROUP_SELECT/STATUS_ASSIGNEE/CATEGORY 等）。**B26 と同時に直す**（型メタに基づく判定という同じ仕組み） | **正しさ** | 中 | [semantics §4.6.3](internal/ksql_string_semantics.md) |
...
| B9 | 厳密 10 進比較（案B・`<=`/`>=` 押し下げ） | 改善 | 🔗 **B26 の依存課題へ格上げ**。同時実装しない場合は typed number の SIMPLE/FULL_SCAN 差を制限事項として残す | 正しさ | 高 | [issue](internal/ksql_exact_decimal_compare_issue.md) |
