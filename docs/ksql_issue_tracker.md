...
- 次回リリース計画: **未定**。要対応は **B129**（方向確定・診断＋レシピ）と **B130**、**B135 の再発防止**（期待値の二重管理／`mcp:smoke` を `npm test` か `prepack` へ）。**B128 は `LAG`/`LEAD` 先行へ組み替え**、**B134** は B53 との関係整理が先。他は優先 低 か実需待ち（§1）。クローズ済みは §3。
