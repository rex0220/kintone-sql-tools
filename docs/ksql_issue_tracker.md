...
| B26 | **`ORDER BY` が `LIMIT` の値でソート主体を変える** | **バグ** | 📝 課題 R1（codex レビュー前）。`executeSimpleSelect`(`execute.ts:1288`/`:1328`) が **`LIMIT` ≤ 500 なら単発 GET で kintone の並びをそのまま採用し、それ以外は JS で並べ直す**。**kintone は バイナリ順・JS は ICU `localeCompare('ja')` で別物**（実機: 同データで kintone=`B,a,あ,ア,亜,ｱ` / JS=`a,B,ｱ,ア,あ,亜`）→ **`LIMIT 500` と `LIMIT 501` で並びが変わり、`LIMIT` を外しても変わる**。`WHERE` に `LIKE` を 1 つ足すだけでも変わる（FULL_SCAN は常に JS）。**v1.14.0/v2.0.0 が major を切って潰した「LIKE のモード不一致」と同じ構造** | **正しさ** | **高** | [semantics §4.3](internal/ksql_string_semantics.md) |
| B25 | **`MIN`/`MAX` と `ORDER BY` の文字列比較が食い違う** | **バグ**（一貫性） | 📝 課題 R1（codex レビュー前）。**`MIN`/`MAX`・`GREATEST`/`LEAST`・`WHERE` の `<`/`>` はコードユニット順で kintone のバイナリ順と整合**しているが、**`ORDER BY` の JS 経路だけ ICU `localeCompare('ja')`**（`process.ts:597`/`execute.ts:4117`）。実機: 同データで `MAX`=`ｱ`（kintone の最後と一致）だが JS 経路の `ORDER BY` の最後は `亜`。ICU は `'あ'`/`'ア'`/`'ｱ'` を**等しい**と判定し順序が比較器で決まらない。**B26 と同根**（JS 経路の ICU が浮いている）で、揃えるなら一緒に判断する | **正しさ** | 中 | [semantics §4.4](internal/ksql_string_semantics.md) |
