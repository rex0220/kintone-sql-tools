...
|---|---|---|---|---|---|---|
| B99 | MCP サーバーの新規格（プロトコル改訂 2026-07-28）対応 | 改善 | 📝 **評価（優先 低／いま着手できない）**（2026-07-29・オーナー指示）。**現状**＝SDK `@modelcontextprotocol/sdk` **1.29.0**（npm latest は **1.30.0**）・対応プロトコルは **`2025-11-25` まで**・**stdio のみ**・tools 13／resources 4・ログは stderr。**【重要】廃止・削除予定の機能を 1 つも使っていない**（実測）〔`ping` / `logging/setLevel` / `resources/subscribe` / Roots / Sampling / Logging / Elicitation / セッション / HTTP+SSE **すべて未使用**〕＝**新仕様が消すものを何も使っていないので、待っても負債が増えない**。**【いま着手できない】プロトコル改訂 `2026-07-28` は公開されているが、実装した SDK が存在しない**〔`npm view dist-tags` → `{"latest":"1.30.0"}`／79 版すべて 1.x／1.30.0（2026-07-27 公開）の `LATEST_PROTOCOL_VERSION` は `"2025-11-25"`〕。**【調査上の注意】WebFetch の要約が「2026-07-28 対応の SDK v2.0.0 が同日公開」と答えたが誤りで、npm の実データで否定できた**＝**再評価は要約ではなく `npm view` の実データで行う**。**仕様サイトの「現行版」表記も揺れている**（versioning ページは「現行 `2025-11-25`」と書きつつ本文リンクは全部 `2026-07-28`）。**2026-07-28 の中身は基盤の作り直しに近い**〔①`initialize` ハンドシェイクを削除＝**ステートレス化**（各リクエストの `_meta` に版と能力）②セッションと `Mcp-Session-Id` を削除 ③**`server/discover` をサーバーが MUST 実装** ④`resources/subscribe` を `subscriptions/listen` へ ⑤`ping` / `logging/setLevel` 削除 ⑥**MRTR** がサーバー起点リクエストを置換 ⑦全 result に `resultType` 必須 ⑧SSE の再開・再送を削除〕。**こちらが書く必要があるのは 1 つだけ**＝**`tools/list` / `resources/list` / `resources/read` の result に `ttlMs` と `cacheScope` が必須**（`CacheableResult`）。**それ以外は SDK が吸収するか、stdio なので無関係**。**`tools/list` の決定的順序（SHOULD）は既に満たしている**（登録順）。**案 A（推奨）＝1.30.0 だけ上げて 2026-07-28 は待つ**〔1.30.0 は保守リリースでプロトコル版は据え置き。**`@hono/node-server` の脆弱性対応（GHSA-frvp-7c67-39w9）を含み、依存木に実在する**（stdio なので実行経路には入らないはずだが bundle 混入は要確認）〕。**案 B（新仕様を自前実装）は不採用**＝`McpServer` を迂回することになり、SDK 対応時に全部捨てる。**副産物の可能性**＝**stdio しか使わないなら `@hono/node-server` を bundle から落とせるかもしれない**（別の改善）。案 A は 0.25 人日。2026-07-28 対応は SDK の形が出てから見積もる。 | 改善 | 低 | [B99 issue](internal/ksql_b99_mcp_spec_2026_07_28_issue.md) |
