...
|---|---|---|---|---|---|---|
| B29 | **kintoneの数値精度・丸め設定とDML/Tier-0を整合させる** | **バグ / 言語意味論** | 📋 **仕様 R1 作成・Claude レビュー承認（2026-07-18・[spec](internal/ksql_number_precision_semantics_spec.md)）・実装待ち（B9 先行）**。numberPrecision(digits30/小数10/HALF_EVEN・UP・DOWN)を Tier-0 へ取り込み VALIDATE ONLY 偽合格を解消。設定取得は getFieldsCached 同経路・NUMBER 書込対象のみ・取得失敗 fail-closed。全 DML×VALIDATE ONLY/ON ERROR SKIP へ横断（B34 の形）。B9 の厳密10進 primitive(compareDecimal 拡張)を再利用・二重実装禁止・**着手順 B9→B29**。v1 は検証のみ(超過エラー・自動丸めなし)・算術任意精度化は v2 分離。HALF_EVEN は文字列/桁ベース(Math.round 不可・検算9/9一致)。SemVer=minor(従来 valid 行が invalid へ) | **正しさ** | 中 | [spec](internal/ksql_number_precision_semantics_spec.md) / [issue](internal/ksql_number_precision_semantics_issue.md) |
| B20 | 正規表現関数（`REGEXP_LIKE`/`REGEXP_REPLACE`/`REGEXP_SUBSTR`） | 改善 | 📝 **R5: ⛔→実装着手可（ユーザー責任・案 A）**。判断基準を「kSQL が ReDoS を防げないなら出荷しない」から「暴走・ホスト依存はパターンを書いた利用者の責任」へ差し替え（[再評価 R4](internal/ksql_regexp_user_risk_evaluation.md)・2026-07-18 正式採用）。**opt-in ゲートは付けず通常の関数として 4 面提供**（kSQL のゲートはデータ破壊専用・ReDoS は未制約 JOIN と同じ可用性リスクのカテゴリ＝正規表現だけをゲートするのは一貫性を欠く）。要件は「防ぐ」→「文書明示」: 暴走の中断不能・ホスト依存（`\p{}`/`u+i` case folding の Unicode 版差・Node18 の新構文差・**DML 書き戻しで保存データに波及**）・MCP/プラグインの波及・警告 lint（任意）。実測=31 字入力 7 秒の指数カーブ・`%a%a%…%b` で既存 LIKE も同カテゴリ。SemVer=minor | 機能 | 低 | [spec §12](internal/ksql_regexp_function_spec.md) / [eval](internal/ksql_regexp_user_risk_evaluation.md) |
...
| B9 | **最大30桁の厳密10進比較** | **バグ / 言語意味論** | 🔗 **実装計画 R1・Claude レビュー承認（2026-07-18・[plan](internal/ksql_exact_decimal_compare_implementation_plan.md)）・実装着手可**。既存 dmlValidation.ts:compareDecimal を exactDecimal.ts へ分離・拡張（指数表記・signed scale・再利用可能な正規形）＝新規実装禁止。NumberLiteral に raw lexeme を optional 追加（既存 value:number 互換）。band2/GREATEST・LEAST/全消費経路を exact primitive へ配線・固定バンド不変。**空白は末尾バンドへ移す・指数字句 SQL 受理は純加法**。**SemVer は §8 実機（16桁超アプリで v3.2.0 と WHERE=/MIN-MAX/ORDER BY を比較）で major/minor 確定→major なら版番号再計画**。B29 はこの primitive を再利用 | 正しさ | 中 | [issue](internal/ksql_exact_decimal_compare_issue.md) |
