...
| B11 | `UPDATE … FROM`（SET 値に他テーブルのフィールド参照＝アプリ間転記） | 改善 | 📋 仕様 R4・codex 承認・実装着手可（Phase 2・B12 の依存・**#temp＋APP** ソース・`cteName` で collectRefs 自動配線・`$id` 単一等値・50件チャンク・MCP maxRecords・複数マッチ実行前エラー・親のみ） | 機能 | 中 | [spec](internal/ksql_update_from_spec.md) / [roadmap](internal/ksql_batch_processing_roadmap.md) |
