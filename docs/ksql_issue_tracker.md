...
- 最終更新: 2026-08-05
- 現在の最新リリース: **v3.46.0**（2026-08-05・**リポジトリ内で release 化済み・push / PR / タグ / GitHub Release / npm publish は未実施**）。→ [リリース履歴](ksql_release_history.md)
- 次回リリース計画: **未定（未リリースの変更なし）**。**要対応なし**。残る課題はいずれも優先 低 か実需待ち（§1）。クローズ済みは §3。
