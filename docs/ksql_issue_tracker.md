...
| B86 | 実体化ソースで存在しない列参照が検証されず LIKE で全件が返る | 正しさ | 📝 **評価・起票（優先 高／silent wrong result）**（2026-07-28・オーナー検証）。`WITH A AS (SHOW APPS) SELECT * FROM A WHERE アプリ名 LIKE 顧客`（引用符なし）が**977 件＝全件**を返す。**エラーも警告も無し**。引用符を付ければ正しく **44 件**。**原因＝実体化ソースで列の実在が検証されない**〔裸の識別子は列参照として解釈される仕様（v1.14.0 の右辺フィールド比較）で、実在列なら正常に動く（`LIKE 説明` で 690 件）。**存在しない列が黙って空文字へ解決される**〕。**`LIKE` が最悪の方向**＝空パターンは任意の文字列に含まれるため**全件一致**（`=` は 0 件で取りこぼす側）。**スコープ＝物理アプリは守られているが実体化ソースは素通し**〔物理アプリは `ArgumentError: unknown field code(s)` で拒否／CTE（SHOW APPS 由来・物理アプリ由来とも）・一時テーブルは素通し（実測で全件が返る）〕。**実体化された結果は列名の集合が確定している**ので**検証できないのではなく検証していない**。**面ごとの不整合**でもある＝同じ SQL がソースの種類でエラーになったりならなかったりする。**対応案 A（推奨）＝実体化ソースでも列の実在を検証し取得前に拒否**（物理アプリと同じ挙動へ揃える・fail-closed と一致）。**破壊的変更**だが、**そのクエリは誤った結果を返していた**ので B78・B79 と同じ判断基準で正当化できる。**要確認＝サブクエリ・UNION 枝・列別名の解決順序**。**引用符の付け忘れという最頻のミスが黙って全件を返す**形で顕在化する。1.0〜1.75 人日・SemVer=minor。 | 正しさ | 高 | [B86 issue](internal/ksql_b86_materialized_unknown_column_issue.md) |
