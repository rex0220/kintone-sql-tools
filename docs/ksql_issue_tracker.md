...
|---|---|---|---|---|---|---|
| B97 | 打ち切られた入力の集計を fail-closed にする（B95 案 B） | 正しさ | 📋 **未着手**（2026-07-29・Pro が要望）。**[B95](internal/ksql_b95_truncation_visibility_issue.md) で「要望次第」としていた案 B に要望が出た**〔v3.32.0 の連絡 §3 で「要望をいただければ実装します」と伝え、Pro が受けた〕。**`limitReached` だけでは分けられない**＝案 A で**打ち切られたかは判別できる**ようになったが、**「その結果が集計かどうか」は判別できない**。**Pro の反例が正しい**＝`rowCount < fetchedRows` で推測しようとすると、**`SELECT 顧客No, COUNT(*) … GROUP BY 顧客No` はキーが全件一意なら `rowCount == fetchedRows` になり素通りする**。穴を塞ぐには SQL 解析が必要で、**エンジンの判定を再実装することになる**（**依頼③ と同じ理由**＝数日で版が進むので再実装は確実にずれる）。**求められている線引き**＝〔集計 / `GROUP BY` / `DISTINCT` / window / ローカル絞り込み → **エラー**（値が変わる）／行をそのまま返すだけの明細 → **打ち切り + 警告**（行そのものは本物）〕。**「打ち切り + 警告」を残したいのが要点**＝明細ペインだけは行を出したい。**Pro の実測でも 1 件出ている**〔`SELECT ステータス, COUNT(*) … GROUP BY` maxRecords=500 → **499（誤り）**・`limitReached=true`〕。**前例＝[B72 §7.2](internal/ksql_b72_relative_date_fullscan_exact_spec.md)** が「部分集計を成功結果として返し得る」として fail-closed を選び「緩和しない」と明記している。**違いは「誰が選んだか」**＝`truncate` は利用者が明示的に選んだ設定。**だから既存 `"truncate"` の意味は変えず、新しいモードにする**（B95 §4.2 の懸念を回避。Pro もモード追加を前提に書いている）。**最大の注意点＝判定を新規に書かない**。B94 で `PredicateCapability` を再利用した前例がある。新規に書くと B78 / B79 / B86 と同じ silent wrong result になる。**急がない**（Pro は `"error"` で運用中・**B94 でエラーになる場面自体が減っている**）。1.0〜1.5 人日・SemVer=**minor**（新モードなら純加法）。 | 正しさ | 中 | [B97 issue](internal/ksql_b97_incomplete_aggregate_failclosed_issue.md) |
| B96 | `getRecords()` の応答契約が文書に無く、ラッパーが `searchAborted` を落とすと fail-closed が無効になる | 改善 | 📋 **未着手**（2026-07-29・Pro の報告 §3）。**Pro が自分で見つけて共有してくれた**＝9 ペインが同じアプリを参照するため `ReadonlyKintoneClient` を**自前のキャッシュで包んで**渡しており、そのラッパーが `return { records: [...records] };` と書かれていた〔**リクエスト側の `params.totalCount` は素通ししていたのに、応答を組み立て直すときに落としていた**〕。**Pro 側は修正済み**（対応モックと非対応モックの両方で回帰テストを固定）。**エンジンの不具合ではない**。**【核心】非対称がある**＝**`totalCount` の欠落はエンジンが自分で塞げる**（`isValidTotalCount` で検査し全件取得へフォールバック＝B94 §1.3）が、**`searchAborted` の欠落は塞げない**。検出経路が **client の応答だけ**（[execute.ts:935](../src/execute.ts#L935) / [fetchAll.ts:215](../src/api/fetchAll.ts#L215)）で、**`false` と欠落を区別する手段が無い**。→ **ラッパーが落とした時点で `SEARCH_ABORTED` の fail-closed が静かに無効になる**＝**打ち切られた 10 万件を「全件」として集計する**。**B78 / B79 / B86 / B90 と同じ silent wrong result で、しかもエンジン側の防御が全部素通りする**。**文書には挙動しか書いていない**〔「client が `searchAborted: true` を返した場合…hard error」〕**が、「落としてはいけない」という義務が書かれていない**。**型でも区別できない**＝両方 `?` であり、**打ち切りが無ければ `searchAborted` を省略するのが正常**。**BYO で躓いたのは 3 回目だが種類が違う**〔B85＝制約を落とす／B93＝`$id` を足す＝どちらも**自作クライアントの実装漏れ**。本件は**公式 factory をそのまま使っているのに間に挟んだラッパーが応答を痩せさせた**〕。**ラッパーを挟む実装は珍しくない**（キャッシュ・計測・リトライ）ため、**自作クライアントを書く人だけの問題ではない**。**案 A（推奨）＝B93 と同じ形で `getRecords()` の応答契約を「渡すもの」「落としてはいけないもの」の対で文書化**し、**危険度の差**（`totalCount` は性能だけ・`searchAborted` は正しさ）も書く。`ReadonlyCursorPage` など他の応答型も確認する。**案 B（型で必須化）は不採用寄り**＝破壊的変更であり、**B95 でまさに同じ理由で任意プロパティへ倒したばかり**。**案 C（実行時検出）は不可**＝`false` と欠落を区別できない。0.25 人日・SemVer=**patch**（文書のみ）。 | 改善 | 中 | [B96 issue](internal/ksql_b96_getrecords_response_contract_issue.md) |
