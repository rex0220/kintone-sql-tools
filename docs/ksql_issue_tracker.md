...
- **Pro（ksql-dashboard-pro）**: v3.29.0 連絡 → **返信受領（2026-07-28）** → **v3.30.0 の連絡文を用意（送付可）**＝[v3.30.0 連絡](internal/ksql_pro_report_v3300.md)。**返信の確認依頼③が B85 として実装済み**（v3.30.0）＝**Pro の推測（契約上の限界）は外れで、自前クライアントに制約を渡せば今日から動く**のが最も価値ある情報。**確認依頼①②は Pro 側が回答を確認し、①は「入れ替わったという見立ては誤りだった」と認めた**（回避策も実機で有効・取得半減）。**Pro が `VALIDATE` を「データ品質チェック」ペインとして採用検討中**（K-33）で、**`constraintMetadata` が採否の判断材料**になる。**v3.30.0 は B86 の破壊的変更を含む**ため、**Pro 側でレシピ・テンプレート・AI 生成 SQL に引用符の無い文字列リテラルが無いかの確認が必要**。`runBatch` は Pro のペインが `runQuery` 1文実行のため当面不使用（Phase 2）。**B73 は `code` 維持を了承済み**。
