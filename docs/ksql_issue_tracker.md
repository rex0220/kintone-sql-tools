...
| B27 | **ORDER BY の押し下げ同値性を型メタで判定する** | **バグ** | 📝 設計 R2（横断仕様 §4.6.3・実測）。意味型、ローカル順の有無、サーバ能力 `equivalent / unsupported / unknown` を分離し、**ローカル契約あり + R4 comparator と同値な型だけ allowlist で押し下げる**。ローカル契約ありの unsupported/unknown は FULL_SCAN、複合型で契約未定義なら `ArgumentError`。schema-aware planner を EXPLAIN/実行で共有する。B26 と基盤・major リリースを共有するが別課題 | **正しさ** | **高** | [semantics §4.6.3](internal/ksql_string_semantics.md) |
