...
| B27 | **ORDER BY の押し下げ同値性を型メタで判定する** | **バグ** | 📝 **R8.1同期**（横断仕様 §4.6.3 / §9.2.1）。サーバ受理と押し下げ同値性を分離し、`supported + equivalent` だけallowlistで押し下げる。トップレベルのSIMPLEは利用者キー末尾へ **`$id asc`** を補い、FULL_SCANの安定入力順と一致させる。値比較器へ`$id`を混ぜずRANK/DENSE_RANKのpeer判定を維持。非同値群はASC/DESCで反転、同値群内のcanonical tie順は不変。CREATOR/MODIFIERはsupported + non_equivalent。**R8.1 Blocking＝`STATUS` の `states.*.index` を全クライアント面で保持し、`STATUS` の ORDER BY が実在する場合だけ取得してrank mapへ統合する**（`nodeKintoneClient.ts:259` が index を捨てるため FULL_SCAN がプロセス定義順を再現できず、実装しない限り `STATUS` は non_equivalent。取得の条件付けは v2.7.0 の `status.json` が前例）。**R8.2 実測（実装確定）＝`index` は文字列**（公式明記・実測 `"0"`〜`"3"`）→`Number()` で整数化（辞書順のままだと2桁で `"10"<"2"`）／**`enable:false` でも `states` は非 null**（APP4148 実測）→ `states` の非 null を有効判定に使わず `enable` を見る。未設定アプリは `states:null` で別状態／必要権限はレコード閲覧or追加のみ＝新たな権限エラー経路なし。**`$id asc` 契約の一般性は測定済6型どまり**＝RADIO逆順定義・アプリコード付きRECORD_NUMBER・LINK(TEL)・CREATED_TIME/UPDATED_TIME が未確定 | **正しさ** | **高** | [semantics §4.6.3](internal/ksql_string_semantics.md) |
