...
|---|---|---|---|---|---|---|
| B29 | **kintoneの数値精度設定とDML/Tier-0を整合させる（整数桁超過の事前検出）** | **バグ / 言語意味論** | 🚧 **R2（案B・小数素通し）へ改訂中（2026-07-18）**。R1（整数＋小数の両拒否）は main マージ済み（PR #106）だが、**小数拒否を撤回**＝kintone が REST/CSV/編集画面/計算フィールドの全面で小数を自動丸めするため kSQL だけ拒否は不整合。**整数部超過(`ERR_NUMBER_INTEGER_DIGITS`)の事前検出のみ維持**（kintone も整数超過は全面ハード拒否＝一貫・`ON ERROR SKIP` で行隔離の実利）。`ERR_NUMBER_DECIMAL_PLACES` と `quantizeDecimal` を削除。設定取得は getFieldsCached 同経路・NUMBER 書込対象のみ・fail-closed。**実機確定=整数部予算 I=digits−decimalPlaces=12 が kintone ハード境界(12桁受理・13桁 CB_VA01)。小数は kintone が暗黙丸め(12.34567→12.3457)＝素通しの根拠**（[evidence](internal/evidence/b29_number_precision_smoke.md)）。SemVer=minor。v3.3.0 予定 | **正しさ** | 中 | [spec](internal/ksql_number_precision_semantics_spec.md) / [issue](internal/ksql_number_precision_semantics_issue.md) |
| B20 | 正規表現関数（`REGEXP_LIKE`/`REGEXP_REPLACE`/`REGEXP_SUBSTR`） | 改善 | 🚧 **R5: 実装済み・実機確認待ち（ユーザー責任・案 A）**。`i`/`m`/`s` のみ受理・`u` 強制・3語を予約語化し、式/フィールドを実行時評価。**opt-in ゲートなし**で SELECT/WHERE/HAVING/ORDER BY/temp/CTE/通常 UPDATE SET に提供し、B26 型メタでは3関数とも string を維持する。ReDoS の中断不能・ホスト依存（`\p{}`/`u+i` case folding・新構文）・DML 書き戻しの保存差・MCP/プラグインの波及を言語リファレンスへ明記。自動テスト実装済み、CLI/MCP/Chromium/Firefox/Safari の実機確認待ち。SemVer=minor・v3.3.0 | 機能 | 低 | [spec §12](internal/ksql_regexp_function_spec.md) / [eval](internal/ksql_regexp_user_risk_evaluation.md) |
| B36 | `REGEXP_REPLACE` に `occurrence` 引数（先頭／N 番目だけ置換） | 改善 | ✅ **実装・Claude レビュー・実機検証完了・main マージ（2026-07-18・PR #111）**。`REGEXP_REPLACE(x, pattern, replacement [, flags [, occurrence]])`（arity 3→5）。省略/0=全置換（後方互換）・1=先頭・N=N番目・一致数超=無変化。非負整数以外は ArgumentError（実行時評価）。N番目でも後方参照 `$1`/`$$`/`$&`/`$<name>` を手書き展開（再マッチ方式のアンカー差を回避）。ゼロ幅一致は String.replace の global に委譲。第4引数 flags のため MySQL/Oracle と引数位置が異なる。実機: 全'x-x-x'/occ1'x-a-a'/occ2+後方参照'12-34'→'12-[34]'/occ5無変化/-1→ArgumentError。SemVer=minor・v3.3.0 同梱 | 機能 | 低 | [spec](internal/ksql_regexp_replace_occurrence_spec.md) |
...
| B4 | 保存クエリのパラメータ化 `:name` | 改善 | 📝 **価値評価の再検討が必要（棚上げ寄り・2026-07-18 棚卸し）**。仕様（詳細 R2）は流用可能水準だが、**評価が `@var`（DECLARE 外部注入）実装前に書かれ重複を未検討**。B4 の中核価値「外部から動的値を安全注入」は `@var` が既に提供済み。残る固有価値は「カタログ永続化＋保存クエリでの利用」だけで、それも「保存クエリの単文制約を緩めて DECLARE+SELECT バッチ＋既存 @var」の軽量路線の方が安い。着手前に @var との差分で再評価（eval に追記済み） | 機能 | 中 | [eval](internal/ksql_saved_query_params_evaluation.md) / [draft](internal/ksql_saved_query_params_spec.md) |
| B5 | KLIKE 親レコード DML 解禁 | 改善 | 📝 **改善案・専用仕様が要る（2026-07-18 棚卸し）**。前提の検索打ち切り検出は **Node/CLI/MCP のみ整備済**（プラグインは `kintone.api()` がヘッダー非露出＝B7 未解決で安全前提が崩れる）。実装には①DML result 型への `warnings` 追加②プラグイン面を対象外にするか（Node 系限定解禁）③サブテーブル DML 恒久非対応、を確定する専用仕様が必要（v1 spec §3.5 に面依存性を追記済み） | 機能 | 中 | [v1 spec](internal/ksql_klike_native_search_spec.md) |
| B6 | KLIKE 外部結合 非 nullable 側の押し下げ解禁 | 改善 | 📝 **改善案・棚上げ妥当（2026-07-18 棚卸し）**。INNER 限定は v3.2.0 でも有効（`klikePushdownPlan.ts:31`）。非 nullable 側判定は結合順・来歴付与を要する非自明な正しさ問題で、現状は将来メモ1行のみ。実需確認まで棚上げ・着手なら専用仕様から | 性能 | 低 | [v2 spec](internal/ksql_klike_pushdown_v2_spec.md) |
| B7 | プラグインでの検索打ち切り検出（raw fetch 経路） | 改善 | 📝 **改善案・B5 従属（2026-07-18 棚卸し）**。Node 系は実装済・プラグインのみ未検出（`ui/kintoneClient.ts` が `searchAborted` 未設定）。案a=`kintone.api.url()` で raw fetch 化しヘッダー取得だが実機確認未着手。B5 をプラグインまで解禁する段で必須化する従属課題。当面は案b（プラグイン非検出を明記）で許容・棚上げ妥当 | 安全性 | 低 | [issue](internal/ksql_search_abort_warning_issue.md) |
| B9 | **最大30桁の厳密10進比較** | **バグ / 言語意味論** | 🚧 **実装完了・Claude レビュー承認・実機 SemVer=major 確定（2026-07-18・[evidence](internal/evidence/b9_exact_decimal_semver_probe.md)）・branch feat/b9-exact-decimal**。exactDecimal.ts へ単一 primitive（指数対応・signed scale・検算15/15）・NumberLiteral raw lexeme・全経路配線・空白は末尾バンド・GREATEST/LEAST 集合モード保存。全1,835テスト green。**実機で公開 v3.2.0 と MIN/MAX・ORDER BY が変わる（binary64 衝突ペア .9991/.9992・従来は誤り）＝互換性注意**。**ユーザー判断で v3.3.0 に含める・移行ガイドに明記**。B29 はこの primitive を再利用 | 正しさ | 中 | [issue](internal/ksql_exact_decimal_compare_issue.md) |
| B10 | バッチ変数 後続：`NULL` 代入 / SELECT 列での `@var` 参照 | 改善 | 📝 **提案・棚上げ妥当だが NULL 前提が陳腐化（2026-07-18 棚卸し）**。1b/1c は実装済・実需薄い（`SET @empty=''`・`IN (SELECT)` で代替可）。**NULL 側の 1a 根拠「`IS NULL` 構文なし」は誤り**＝v3.2.0 で `NULL` トークン・`IS NULL` 述語がフィールドには実装済（1a §6 に訂正注記）。再評価は「変数への NULL 代入＋変数 null の意味論」に論点を絞る。SELECT 列 `@var` は前提不変の純後続 | 機能 | 低 | [1a spec](internal/ksql_batch_variables_phase1a_spec.md) |
