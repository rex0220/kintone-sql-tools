...
- 最終更新: 2026-08-05
- 現在の最新リリース: **v3.51.0**（2026-08-06・npm publish 待ち）。→ [リリース履歴](ksql_release_history.md)
- 次回リリース計画: **未定**。要対応は **B140**（CTE の `GROUP BY` キーを全順序として証明する。B128 の主用途で警告が偽陽性になる）。**B134** は B53 との関係整理が先。他は優先 低 か実需待ち（§1）。クローズ済みは §3。
