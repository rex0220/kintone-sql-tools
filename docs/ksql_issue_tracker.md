...
- 最終更新: 2026-07-28
- 現在の最新リリース: **v3.29.0（2026-07-28）＝B68 engine ライブラリの read-only 拡張／B81／B82**。**純加法**（既存 API の型も挙動も不変）。**npm publish はオーナー実施待ち**。版ごとの内容と決定理由は [リリース履歴](ksql_release_history.md)。
...
- **Pro（ksql-dashboard-pro）**: 完了報告を送付済み → **返信受領（2026-07-27）**。確認依頼2点は回答済み〔JOIN の数値は**演算子の違いによる仕様**・選択系 `=` は **kintone のクエリ文法が `in` のみ**〕で、**B84 として起票**。**B73 は Pro 都合ではクローズ可**（ただし**エラー `code` の値と意味は変えない**ことが契約要求）。**Pro は engine library 利用**のため、**v3.29.0 の実機確認は Pro 側が本来の受入**になる。
