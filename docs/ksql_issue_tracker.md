...
| B85 | ライブラリの VALIDATE が制約を検証できず黙って 0 件を返す | 正しさ | ✅ **案 A・B・C すべて実装済み（未リリース）**（2026-07-28・Pro 返信の確認依頼③）。**同じ `VALIDATE` 文が MCP は 10 行/errorCount 12、ライブラリは 0 行/0**（選択肢違反だけ一致）。**エラーも警告も無く「エラーなし」と読める**＝fail-closed の思想と一貫しない（Pro 指摘）。**原因は Pro の推測（契約上の限界）とは違った**＝**実測でパイプラインは制約を素通ししている**〔`getFields` が `minLength/required` を返せば 2 件検出、返さなければ 0 件〕。**真因＝型が宣言していないこと**。**TypeScript は余剰プロパティを拒否する**ので自前クライアントの利用者は**渡そうとしても止められる**。**`createReadonlyKintoneClient` は実行時には制約を返している**（型だけ狭められている）＝**同関数の利用者は既に完全な検証ができる**。**メタデータ取得量も増えない**。**案 A（実装済み）＝`ReadonlyFieldInfo` へ `required/minValue/maxValue/minLength/maxLength` を宣言**（`hasAuditableConstraint` が見る集合と完全一致・型は `string`）。公開 snapshot が全 12 プロパティを照合するので**型が痩せたら落ちる**。**案 C（実装済み）＝内訳は `COUNT(*)` ではなく `SUM($err_count)`**（サブテーブル違反が1行に複数エラーとしてまとまるため 10 行 vs errorCount 12）。**【重要・案 B の設計を変更】当初の「制約情報が皆無なら警告」は実測で破綻**〔**APP4147 は 21 項目すべて制約なし**＝制約が1つも無いアプリは実在し、警告すると誤警告になる。**無視される警告は無いのと同じ**。さらに**ライブラリは「アプリに制約が無い」と「クライアントが落としている」を原理的に区別できない**〕。→ **推測をやめ「検証したもの」を開示する**設計へ変更（§8）＝**渡されたメタデータに含まれていた制約種別を列挙**する。観測可能な事実のみで推測を含まない。置き場所は `validateStats`（`warnings` より KPI 提示時に必ず目に入る）。**「0 件でした」より「選択肢だけ検証して 0 件でした」のほうが誤った安心を与えない**。**Pro の質問への回答＝parity テストは受理可否の担保であって結果の一致は対象外**（本課題はその隙間で、parity の定義を見直す契機）。**B78（黙って 0 件）・B79（黙って誤った値）と同系列**。案 B 残 0.5〜1.0 人日・SemVer=minor。**【案 B 実装】`validateStats.constraintMetadata { present, absent }`**（4種＝必須/文字数/上下限/選択肢）。**実際の VALIDATE 対象フィールドだけから算出**し、**`hasAuditableConstraint` と判定ロジックを共有**（二重管理を作らない）。**独立検証で4形すべて期待どおり・警告 0**〔全種別/選択肢だけ/文字数だけ/**制約なし**〕＝**開示は入力の写しで推測が入っていない**ことを実測。 | 正しさ | 高 | [B85 issue](internal/ksql_b85_library_validate_constraints_issue.md) |
| B86 | 実体化ソースで存在しない列参照が検証されず LIKE で全件が返る | 正しさ | 📋 **仕様確定・実装待ち（優先 高／silent wrong result ＋ silent wrong write）**（2026-07-28・オーナー検証）。`WITH A AS (SHOW APPS) SELECT * FROM A WHERE アプリ名 LIKE 顧客`（引用符なし）が**977 件＝全件**を返す。**エラーも警告も無し**。引用符を付ければ正しく **44 件**。**原因＝実体化ソースで列の実在が検証されない**〔裸の識別子は列参照として解釈される仕様（v1.14.0 の右辺フィールド比較）で、実在列なら正常に動く（`LIKE 説明` で 690 件）。**存在しない列が黙って空文字へ解決される**〕。**`LIKE` が最悪の方向**＝空パターンは任意の文字列に含まれるため**全件一致**（`=` は 0 件で取りこぼす側）。**スコープ＝物理アプリは守られているが実体化ソースは素通し**〔物理アプリは `ArgumentError: unknown field code(s)` で拒否／CTE（SHOW APPS 由来・物理アプリ由来とも）・一時テーブルは素通し（実測で全件が返る）〕。**実体化された結果は列名の集合が確定している**ので**検証できないのではなく検証していない**。**面ごとの不整合**でもある＝同じ SQL がソースの種類でエラーになったりならなかったりする。**対応案 A（推奨）＝実体化ソースでも列の実在を検証し取得前に拒否**（物理アプリと同じ挙動へ揃える・fail-closed と一致）。**破壊的変更**だが、**そのクエリは誤った結果を返していた**ので B78・B79 と同じ判断基準で正当化できる。**要確認＝サブクエリ・UNION 枝・列別名の解決順序**。**引用符の付け忘れという最頻のミスが黙って全件を返す**形で顕在化する。1.0〜1.75 人日・SemVer=minor。**【2026-07-28 調査で想定より深刻と判明】①書き込みまで届く**〔`INSERT INTO APP100 (件名) SELECT 存在しない列 FROM #t` が **ok=true で空文字レコードを実際に POST**。モックで POST 呼び出しを観測して確認（実書き込みはしていない）。**B79（誤った値を読む）より深刻**＝読みは再実行できるが**書き込みは戻せない**〕。**②混在 JOIN では物理アプリ側の検証も迂回する**＝穴は起票時の把握より広い。**検証位置は `executeQueryWithCte()` の実行ディスパッチ前**（実体化済み columns が使え、サブクエリ・UNION 枝・物理 GET・DML mutation より前に拒否できる）。**スコープ実測**＝CTE（SHOW APPS/DESCRIBE/物理由来）・一時テーブル・各種サブクエリ・UNION 両枝・列別名・混在 JOIN で素通し（派生表は構文未対応）。**`=` も対象**＝0 件は正しい空集合ではなく不存在列を空文字評価した誤結果で、B78/B79 の「エラー化しても正しい結果を失わない」基準が使える。**既存テスト影響 0 件**（静的監査）。**`defs.length===0` の抜け道は持ち込まない**（materialized columns が非空なら mock metadata に関係なく検証）。**残る限界＝`rows=[] && columns=[]`** は x と missing を区別できず、R1 は JOIN なし 0 行読み出しに限定して既存挙動を維持し JOIN 利用は取得前 error。**見積 1.0〜1.75 → 2.25〜3.25 人日**（columns 完全復元まで含めるなら さらに 0.75〜1.5）。 | 正しさ | 高 | [B86 計画](internal/ksql_b86_materialized_unknown_column_spec.md) / [B86 issue](internal/ksql_b86_materialized_unknown_column_issue.md) |
