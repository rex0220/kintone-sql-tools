...
| B68 | kSQL read-only ライブラリの read-only 機能拡張（VALIDATE・一時テーブルバッチ） | 改善 | 📋 **仕様確定・実装待ち**（2026-07-27）。**オーナー方針＝Phase A と B を 1 リリースにまとめる**。実装計画を作成済み。**確定した設計**＝①**判定を `isReadOnlyStatement()` へ一本化**（ライブラリ独自の文型列挙を残さない＝B76 の「能力表が2つ」の再生を防ぐ）②**`runQuery` は純加法で拡張**〔既存レコード `VALIDATE` は `SELECT` と**同じ `SelectResult`** ＋任意 `validateStats` なので、**新関数も判別共用体も不要**。`QueryResult` に `validateStats?` を足すだけで**既存利用者に完全非破壊**。当初 `explainQuery` の前例から `runValidate()` を推したが、**戻り値の型を確認する前の判断**だったため却下〕③**`runBatch()` を新設**〔`BatchEnvelope`（CLI/MCP 共通）に倣い **`results[]` は `QueryResult` を再利用**〕④**追加ゲート**〔`APPLY` は `VALIDATE ONLY` 経由だと `writesKintone=false` で**分類器を通ってしまう**ため別途拒否／`IMPORT` 既定 off。**`statementHasApplyBlocks` は `src/mcp/tools.ts` にあるので `src/core/` へ移設**＝ライブラリから mcp を import すると**bundle 汚染＋三重管理**になる〕⑤**`searchAborted` の hard error は維持**（構文の許可範囲とは別の軸・B79）。**DML `VALIDATE ONLY` は非スコープ**（別型 `DmlValidationResult` で追加フィールド 10 以上・用途からも外れる。将来は `results[]` の判別共用体で）。**Step 1 判定統一＋単文 VALIDATE（1.0〜1.5）／2 `runBatch` API（1.5〜2.5）／3 temp lifecycle・上限契約（1.5〜2.5）／4 limits・4面 parity（0.5〜1.0）／5 docs・smoke（0.5〜1.0）＝合計 5.0〜8.5 人日**。SemVer=**minor（純加法）**。**docs 必須項目**＝一時テーブルは**作る時点で絞ってから JOIN**（JOIN 入力が一時テーブルだと server-only 関数は使えない・実測済み）。 | 機能 | 中 | [B68 計画](internal/ksql_b68_engine_library_readonly_impl_plan.md) / [B68 eval](internal/ksql_b68_engine_library_readonly_extensions_evaluation.md) |
