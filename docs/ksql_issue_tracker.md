...
- 次回リリース計画: **未定**。**Pro への連絡が送付待ち**。次の着手候補は **B91**（依頼③の再評価）。
- **Pro（ksql-dashboard-pro）**: 依頼①②を v3.31.0 で実装 → **回帰（B92）を一度公開し v3.31.1 で修正**。**v3.31.1 の連絡文を用意（送付可）**＝[v3.31.1 連絡](internal/ksql_pro_report_v3311.md)。依頼③（B91）は再評価待ち。
