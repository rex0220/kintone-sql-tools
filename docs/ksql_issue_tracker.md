...
|---|---|---|---|---|---|---|
| B112 | `EXPLAIN` の `app:` / `JOIN:` 行で profile が二重になり、別名形では内部仮想 ID が残る | 課題 | 📝 **評価（実測済み・3 面で再現）**（2026-08-02・B111 の実測中に発見）。**症状 2 つ**〔①別名なし＝物理アプリで `app: APP4149@dev@dev` と profile 二重／②別名・JOIN 形＝`app: LAPP_案件管理@dev AS a (900000000)` と**括弧内の内部仮想 ID が残る**（**engine ライブラリ面でも露出**・実測）〕。**原因＝`restoreSqlDiagnosticValue` の 2 段置換**〔物理は display(`APP4149@dev`) が internal(`APP4149`) を含むため②が①の出力を再処理／別名つき行は `APP<n> AS <別名> (<n>)` で①の隣接パターンに一致せず括弧が残る〕。**B109 / B110 の回帰ではない**（v3.37.0 のコードでも同形・v1.13.x 以来）。**B108 / B109 と同じ契約違反の残り**＝仕様で「経路」は列挙したが**出力の形**を列挙していなかった（範囲の取り落としが 2 度目）。**方針＝2 段 split をやめ 1 パスの関数形置換**（別名と括弧つき内部 ID までを 1 パターンで畳み、自分の出力を再処理しない。括弧内 ID は落とす）。`target:` 専用規則・ライブラリの physical 表示・B108 §6.2 は不変。**優先 中**＝**Pro の実行計画ボタンが JOIN 形で直接踏む**（K-87 の依頼例そのものが JOIN）。修正は小。 | 正しさ（表示） | 中 | [B112](internal/ksql_b112_explain_app_line_restore_issue.md) |
| B111 | 変数に相対日付関数トークンを束縛する（`DECLARE` 側で宣言する形） | 改善 | 📝 **評価（方向のみ回答済み・実装は実需待ち）**（2026-08-02・Pro 相談 K-6 変数コントロールバー）。**依頼＝変数の値が相対日付関数トークンなら関数として押し下げてほしい**（単一条件化・日時境界・サーバー TZ・ローリング窓）。**測って前提が 1 つ覆った**＝`DECLARE @p = THIS_MONTH()` は現行 ParseError で、通る `TODAY()` は**位置で意味が変わる別物**（DECLARE 右辺＝クライアント評価のリテラル `"2026-08-02"`／WHERE 右辺＝サーバー押し下げ）＝**拡張の出発点ではなく既存の罠**。**実害（日時フィールドの終端取りこぼし）は engine 変更なしで解決**＝実測 `作成日時 <= '2026-07-14'` は 18/20 件、`<= '2026-07-14T23:59:59Z'` は 20 件で、RFC3339 は変数経由でも EXACT 押し下げ（**Pro は制限なしでリリースできる**）。**方向＝採用可。ただし「値が関数になる」形は採らない**〔値束縛の保証＝値は決して code にならない／**DML で fail-open**（`DELETE ... WHERE 日付 = @period` が単日→年間に変わりうる・B102 と同じ形）／グローバルフラグは B107 で却下した「設定で意味が変わる」形〕→ **SQL 側で宣言**（`DECLARE @period RELATIVE_DATE = THIS_MONTH()` の形・純加法・非ホワイトリストは API 前に fail-closed・DML は拒否）。**実装は安い見込み**＝変数解決は planning より前なので**関数の AST ノードへ差し替えれば既存 B67/B72/B75 の判定と fail-closed がそのまま効く**。トークン解釈は**既存パーサを再利用**（引数つき関数があるため完全一致では足りない・文法の二重管理を避ける）。**優先 低〜中・実需待ち**＝Pro に代替手段があり 2 変数形は今後も動く。副産物として `TODAY()` の位置差と RFC3339 の書き方は文書化価値あり。 | 改善 | 低 | [B111](internal/ksql_b111_relative_date_variable_issue.md) |
