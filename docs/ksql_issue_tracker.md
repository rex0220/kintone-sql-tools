...
| B26 | **文字列の比較・並び順を kintone のコードポイント順へ統一する**（旧 B25 を統合） | **バグ** | 📝 **方針のみ決定・規則は codex が設計中**（横断仕様 §4.5）。**kintone は Unicode コードポイント順**（実機実測）。現状 **3 系統すべてが食い違う**＝①`ORDER BY` の JS 経路は **ICU `localeCompare('ja')`**（BMP でも不一致）②`MIN`/`MAX`/`GREATEST`/`WHERE` は **UTF-16 コードユニット順**（サロゲートで不一致）③**`LIMIT`≤500 の単発 GET だけ kintone の順**（`execute.ts:1288`/`:1328`）→ **`LIMIT 500` と `LIMIT 501` で並びが変わる**。さらに **④`ORDER BY` の複合規則が全順序でない**（`2<10<1a<2` の循環・実機で入力順により結果が変わる）**⑤通常テキストに `sortKind` が付かない**（`formFieldInfo.ts:92`）→ `'9','10'` が SIMPLE=文字列順 / FULL_SCAN=数値順。**比較器の差し替えだけでは直らない**（型の表と複合規則全体の全順序検証が要る）。SemVer=**major 相当** | **正しさ** | **高** | [semantics §4](internal/ksql_string_semantics.md) |
