...
|---|---|---|---|---|---|---|
| B98 | JOIN の右側が打ち切られると一致するはずの行に `NULL` が入る | 正しさ | 📋 **未着手**（2026-07-29・[B97 §5](internal/ksql_b97_incomplete_aggregate_failclosed_spec.md) で意識的に対象外とした残件）。**B97 の原理（行を畳む処理）では JOIN を説明できない**ので外したが、**別の問題が残る**＝**右側が取得上限で打ち切られると、一致するはずの行が `NULL` になる**。**行そのものは本物だが、`NULL` は「相手がいない」という誤った主張**であり、**「件数が足りないだけ」では済まない**。**仕組みはコードで確認済み**〔①[`selectCompleteInputReasons()`](../src/core/dmlGuard.ts#L172) は `stmt.joins` を見ない＝**JOIN 由来の理由が無い**②main と JOIN 各テーブルは**それぞれ同じ `maxRecords` / `onLimit`** で取得され（[execute.ts:4393](../src/execute.ts#L4393)）、**打ち切り後の配列がそのまま `applyJoin()` へ渡る**（[process.ts:1632](../src/engine/process.ts#L1632)）③`applyJoin()` は**渡された行だけ**で判定する（[process.ts:123](../src/engine/process.ts#L123)）＝**打ち切られたのか元から無いのかを区別しない**〕。**エンジンは「右側が打ち切られた」ことを知っている**＝[`tryFetchJoinRecordsBySourceKeys()`](../src/execute.ts#L5200) の `onTruncate` が警告を出し B95 の `limitReachedApps` に記録する。**知っているのに、その入力で JOIN を続けている**。**検索打ち切り（10 万件）では既に fail-closed**＝外部結合を含む文は `searchAborted` で停止する（[outerJoinSearchAbortGuard](../src/core/outerJoinSearchAbortGuard.ts#L12)）。**同じ危険を kintone 側の都合では止めているのに、利用者が選んだ `truncate` では止めていない**＝**B97 と同じ構図**。**【実機での再現は未達成・理由あり】**手元の 2 アプリは `$id` でも `顧客No` でも**一致がほぼ 1:1** のため、**右の一致集合が常に左と同じ窓に収まる**。`maxRecords=18` で右側の打ち切り警告は出たが**結果は正しいまま**だった（先頭 18 件に一致がすべて含まれていた）。**再現には「左が上限に収まり、右の一致集合が上限を超える」形が要る**〔1:N の JOIN、または右の一致が後方にある〕。**実装時に検証用データを作ること。「作れなかった」を「起きない」と読まないこと**。**案 A＝JOIN を完全入力の対象にする**（実装は浅いが**影響が大きい**＝JOIN を含むクエリが `truncate` で一切使えなくなり、**B97 で守った「明細は行を出せる」を JOIN について取り消す**）。**案 B＝右側が打ち切られたときだけ止める**（正確だが**判定が実行時**になり、AST から静的に決まる既存の配管に乗らない＝**新しい仕組みが要る**。B97 の教訓＝新しい判定は silent wrong result の温床）。**案 C（既存の扱いと最も整合）＝INNER と OUTER で分ける**〔**`NULL` が入るのは OUTER だけ**。INNER は行が減るだけで返る行は本物。**`searchAborted` の既存の扱いと揃う**〕。**優先度は低い**＝**Pro は現時点で影響なし**（JOIN を使うペインはいずれも小さいアプリ同士の結合）・**検索打ち切りでは既に fail-closed**・**集計を伴う形は B97 で既に止まる**。**ただし silent wrong result であり B78 / B79 / B86 / B90 / B97 と同じ系列**。要望が出たら優先度を上げる。 | 正しさ | 低 | [B98 issue](internal/ksql_b98_join_truncated_right_null_issue.md) |
...
| B91 | MCP にプラグイン互換モードを追加する（環境変数） | 改善 | 📝 **再評価済み（2026-07-29・v3.33.0）＝案 D を推奨・オーナー判断待ち**。**保留条件だった B89 完了を受けて測り直した**。**【実測】受理集合の差は 5 形だけ**〔両方のガードへ同じ SQL を通した。手で数えていない〕＝**`EXPLAIN <DML>` ／ `UPDATE`・`INSERT`・`UPSERT ... VALIDATE ONLY` ／ `IMPORT`**。**すべて「DML 型だが書き込まない検証系」で、書き込む DML は両方が拒否する**ので危険な差ではない。**この差は意図的**＝engine ライブラリは**read-only が契約**で**文の型**で切っており（`isDmlType`。B89 が `EXPLAIN <DML>` を塞いだのはこの線を引き直したから）、MCP は**対話的な道具**で「DML を実行前に検証する」ことが中核の使い方（`ksql_validate` → `VALIDATE ONLY` → `ksql_mutate` を server instructions が案内）。→ **「MCP を engine ライブラリに合わせる」は MCP の意図的な機能を取り上げること**であり、**互換モードが純加法ではない**。**B89 は本件の価値を上げていない**（起票時の見込みと逆）＝**B89 は `runBatch` を厳しくした**ので**差は縮まらず 1 形増えた**。**Pro の用途で踏みにくい**〔`IMPORT` は inline sources が要るので書かない／`EXPLAIN <DML>` も書かない／データ品質ペインは**先頭 `VALIDATE APPn` で書け、それは両方が受理する**〕。**【案 D＝推奨】モードを作らず `ksql_validate` の応答に可搬性フラグを足す**＝**Pro が env var を望んだ理由は「引数だと AI が渡し忘れる」**だったが、**応答フィールドなら渡し忘れが原理的に起きない**。`ksql_validate` は**既に `canRunWithQueryTool` / `requiresMutationTool` を返して**おり、**同じ形で 1 つ足すだけ**。**案 A より優れる**〔MCP の機能を取り上げない／許可集合が 1 通りのまま／**B68 の parity テストが 2 倍にならない**／`.mcp.json` の 2 エントリ登録が不要／**`ksql_validate` の時点で気づける**＝実行時より早い〕。**注意点**＝判定を新規に書かず `assertRunBatchStatement` をそのまま呼ぶ（B97 の教訓）・**engine ライブラリの内部関数を MCP から参照する形になるので bundle-guard と依存の向きを確認**。0.5 人日。**クローズも選択肢**（Pro が困る形は実質残っていない。B4 の前例）。**【オーナー決定 2026-07-29】Pro に案 D で足りるかを確認してから決める**（[確認文](internal/ksql_pro_inquiry_b91_20260729.md)・送付待ち）＝**「Pro が困る形は実質残っていない」はこちらの推測**であり、**実際の使い方は Pro しか知らない**。**推測を確認せずにクローズしない**。確認する 3 点＝①**実際に踏んだ形があるか**（5 形のうち、AI が書いた SQL をペインへ持っていって動かなかった実例）②**`ksql_validate` のフラグで足りるか**③**そもそも今も必要か**（設定画面が `explainQuery` を通すので、埋まるのは時間差だけ）。 | 改善 | 低 | [B91 issue](internal/ksql_b91_mcp_plugin_compat_mode_issue.md) / [確認文](internal/ksql_pro_inquiry_b91_20260729.md) |
