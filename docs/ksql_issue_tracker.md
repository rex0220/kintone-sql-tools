...
|---|---|---|---|---|---|---|
| B29 | **kintoneの数値精度・丸め設定とDML/Tier-0を整合させる** | **バグ / 言語意味論** | ✅ **実装・Claude レビュー・実機検証完了・main マージ済み（2026-07-18・PR #106・[evidence](internal/evidence/b29_number_precision_smoke.md)）**。numberPrecision(digits30/小数10/HALF_EVEN・UP・DOWN)を Tier-0 へ取り込み VALIDATE ONLY 偽合格を解消。設定取得は getFieldsCached 同経路・NUMBER 書込対象のみ・取得失敗 fail-closed。全 DML×VALIDATE ONLY/ON ERROR SKIP へ横断（B34 の形）。B9 の厳密10進 primitive(parseExactDecimal/ExactDecimal/compareExactDecimal)を再利用・二重実装なし。v1 は検証のみ(超過エラー・自動丸めなし)・算術任意精度化は v2 分離。HALF_EVEN は文字列/桁ベース(Math.round 不可)。SemVer=minor。**実機確定=整数部予算 I=digits−decimalPlaces=12 が kintone ハード境界(12桁受理・13桁 CB_VA01 拒否)。小数超過は kintone が暗黙丸め(12.34567→12.3457)するため ERR_NUMBER_DECIMAL_PLACES は偽拒否でなくデータ改変防止の保護**。v3.3.0 予定・残=リリース工程 | **正しさ** | 中 | [spec](internal/ksql_number_precision_semantics_spec.md) / [issue](internal/ksql_number_precision_semantics_issue.md) |
