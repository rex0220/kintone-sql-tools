...
| B91 | MCP にプラグイン互換モードを追加する（環境変数） | 改善 | 📝 **再評価済み（2026-07-29・v3.33.0）＝案 D を推奨・オーナー判断待ち**。**保留条件だった B89 完了を受けて測り直した**。**【実測】受理集合の差は 5 形だけ**〔両方のガードへ同じ SQL を通した。手で数えていない〕＝** / ・・ / **。**すべて「DML 型だが書き込まない検証系」で、書き込む DML は両方が拒否する**ので危険な差ではない。**この差は意図的**＝engine ライブラリは**read-only が契約**で**文の型**で切っており（B89 が  を塞いだのはこの線を引き直したから）、MCP は**対話的な道具**で「DML を実行前に検証する」ことが中核の使い方（server instructions が案内）。→ **「MCP を engine ライブラリに合わせる」は MCP の意図的な機能を取り上げること**であり、**互換モードが純加法ではない**。**B89 は本件の価値を上げていない**（起票時の見込みと逆）＝**B89 は  を厳しくした**ので**差は縮まらず 1 形増えた**。**Pro の用途で踏みにくい**〔 は inline sources が要るので書かない／ も書かない／データ品質ペインは**先頭  で書け、それは両方が受理する**〕。**【案 D＝推奨】モードを作らず  の応答に可搬性フラグを足す**＝**Pro が env var を望んだ理由は「引数だと AI が渡し忘れる」**だったが、**応答フィールドなら渡し忘れが原理的に起きない**。 は**既に  /  を返して**おり、**同じ形で 1 つ足すだけ**。**案 A より優れる**〔MCP の機能を取り上げない／許可集合が 1 通りのまま／**B68 の parity テストが 2 倍にならない**／ の 2 エントリ登録が不要／** の時点で気づける**＝実行時より早い〕。**注意点**＝判定を新規に書かず  をそのまま呼ぶ（B97 の教訓）・**engine ライブラリの内部関数を MCP から参照する形になるので bundle-guard と依存の向きを確認**。0.5 人日。**クローズも選択肢**（§8.4 のとおり Pro が困る形は実質残っていない。B4 の前例）（2026-07-29・Pro 依頼③＝継続）。環境変数（例 `KSQL_PLUGIN_MODE=1`）で ksql_validate / ksql_explain / ksql_query に runQuery と同じ許可リストを適用してほしい。**ツール引数ではなく環境変数を希望する理由＝引数だと AI が渡し忘れる**（`.mcp.json` に探索用と互換モード用の2エントリを登録して併存できる）。**Pro が優先度を下げた**＝**設定画面の構文チェックが `explainQuery`（プラグインの guard 経路）を通すため、CLI/MCP 専用構文はペインを開いた時点で既に弾かれる**。本件が埋めるのは**「AI が MCP で書いた SQL が設定画面を開くまで誤りと分からない」時間差だけ**で、誤った結果が出る類ではない。**B89 が実現すると本件の価値も上がる**（バッチが検証できれば互換モードでも同じ判定が返せる・Pro 談）ため**先に B89**。**検討事項3点**＝①環境変数という形（面ごとの許可集合を環境変数で切り替える前例が無い。プロファイルは既にツール引数）②**Pro が欲しいのが `runQuery` 相当か `runBatch` 相当か**（`runQuery` は行を返す文に限られる。**Pro は `runBatch` を採用済み**なので後者の可能性が高い）③MCP の許可集合が2通りになると **B68 の parity テストの対象も2通り**になる保守コスト。**未見積もり＝B89 完了後に再評価**。 | 改善 | 低 | [B91 issue](internal/ksql_b91_mcp_plugin_compat_mode_issue.md) |
