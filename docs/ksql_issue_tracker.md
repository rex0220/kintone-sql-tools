...
- 最終更新: 2026-08-01
- 現在の最新リリース: **v3.41.0**（2026-08-04・npm publish 済み）。→ [リリース履歴](ksql_release_history.md)
...
|---|---|---|---|---|---|---|
| B115 | CLI テストが開発者のシェル環境変数に依存して緑赤する | 課題 | 📝 **評価（原因特定済み・実測）**（2026-08-04・B114 のレビュー中に発見。**B114 の回帰ではない**）。`logicalExecution.test.ts` の 2 件が**リポジトリの状態と無関係に、実行するシェルの環境変数で緑にも赤にもなる**〔そのまま=2 failed／`KSQL_USERNAME`・`KSQL_PASSWORD` 等を外す=11 passed／**B114 を退避した HEAD でも同じ 2 件が落ちる**〕。v3.39.0 のゲート（2026-08-02）は全 green で、**その後に環境変数が設定されただけ**。**原因**＝対象テストは `tokenMap` だけの設定を作りアプリごとのトークン振り分けを検証するが、認証解決は **CLI 引数 → 環境変数 → 設定ファイル**の順で `auto` は「ユーザー名とパスワードが揃えば userpass」（`src/node/runtime.ts:292-297`）なので、シェルに認証系 env があると `userpass` が選ばれ `X-Cybozu-API-Token` が付かない。**優先順位そのものは仕様どおりで、問題はテストが環境を遮断していないこと**。**方針**＝認証系 env（`KSQL_AUTH` / `KSQL_USERNAME` / `KSQL_PASSWORD` と `KINTONE_*` 系・実装を確認して網羅）を退避・復元する**共通ヘルパー**を用意し、CLI を駆動する該当テスト群へ適用する（このテストだけの小細工にしない・env を書き換えたまま戻さない形にしない）。**優先 中**＝**開発者の環境によっては赤が常態化し「またあれか」で無視されるようになる**（今回も B114 が壊したと誤認しかけた）。修正は小。**別論点（未確認）**＝設定に `tokenMap` があるとき env のパスワード認証が黙って優先されるのが利用者の期待に合うかは未検討。 | 正しさ（試験） | 中 | [B115](internal/ksql_b115_env_dependent_test_issue.md) |
...
| **v3.39.0** | B111 相対日付変数 `DECLARE @x RELATIVE_DATE`（注入トークンを関数として押し下げ・DML は fail-closed）／B112 `EXPLAIN` の別名形で内部仮想 ID が残る欠陥 |
