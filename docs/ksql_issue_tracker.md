...
- 最終更新: 2026-07-18
- 現在の最新リリース: **v3.3.0**（厳密10進比較・数値精度整合・正規表現の 4 件バンドル B9/B29/B20/B36・tag/GitHub Release 公開・npm publish 済み・latest 3.3.0）
...
|---|---|---|---|---|---|---|
| B37 | DML カスタムチェック（行レベル業務ルールの `#err` 隔離） | 改善 | 📋 **仕様 R7・実装着手可（2026-07-18・codex 4 次差分レビューを反映）**。`CHECK WHEN <条件> THEN <メッセージ式>` を 1 つ以上。**`CHECK` ブロック=グループ**（ブロック内先勝ち・ブロック間独立）。**評価行契約**＝INSERT/UPSERT SELECT は元 SELECT 出力行（先頭N列=書込み/残り末尾列=CHECK専用・案b・出力名一意）／VALUES=挿入列／UPDATE=更新前ターゲット／UPDATE FROM=`APP<n>.f`=旧値・`<src_alias>.f`=新値の B37 専用修飾解決。組み込み検証範囲は CHECK 有無で不変・素 DML CHECK は全行収集の専用経路・評価器例外は決定順で文全体 fail-closed・サブテーブル+CHECK は ParseError。`#err` は `ERR_CHECK`。メッセージ式は B38 の `ScalarValueExpr` に依存＝**B38 先行**。SemVer=minor・B38 と束ねて次リリース | 機能 | 中 | [spec](internal/ksql_custom_check_spec.md) |
| B38 | スカラー値式基盤＋文字列連結 `\|\|` | 改善 | 📋 **仕様 R4・実装着手可・B37 先行（2026-07-18・案A）**。①`\|\|`（`CONCAT` 同義・NULL/空は空文字・優先順位=加減算同レベル左結合・予約語増なし）②**`ScalarValueExpr` 新 AST（`ConcatExpr` 含む）＋`parseScalarValueExpr` 公開入口**＋関数引数の `@var` 受理（現状 `CONCAT('x=',@v)` は ParseError）。§3.1.1 で既存 `StringFuncArg`(`AggOperand` 保持=`FORMAT(SUM())` 非回帰)・`FIELD` vs `FIELD_REF`・消費側 audit を**非破壊加法**で確定。`IS NULL` は述語で除外。B37 が依存＝先行実装。SemVer=minor・B37 と束ねて次リリース | 機能 | 低 | [spec](internal/ksql_concat_operator_spec.md) |
