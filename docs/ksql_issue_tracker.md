...
| B132 | `ksql_docs` のセクションキー `window-functions` だけ番号が無い | 改善 | 🐞 **残課題（未着手）**（2026-08-05 起票・依頼の補足）。**v3.45.0（B125）で入れた自分の退行**。他は `01-basic-rules`〜`26-assert` と `## N.` 見出しから番号が導かれるが、`## 10.1 ウィンドウ関数` は導出規則に乗らず番号なしで追加された。**AI が `10-1-window-functions` のような番号付きキーを推測して外した実例あり**（依頼元）。`10-1-window-functions` にすれば一貫する。索引の一貫性の問題で実害は無い。（→ [評価](internal/ksql_analytics_request_20260805_evaluation.md)） | 改善 | 低 | [評価](internal/ksql_analytics_request_20260805_evaluation.md) |
