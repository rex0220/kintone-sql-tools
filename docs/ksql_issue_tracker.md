...
|---|---|---|---|---|---|---|
| B107 | `LAPP_<NAME>` をブラウザ向け公開 API（`runQuery` / `runBatch`）でも解決する | 改善 | 📝 **評価（Pro へ質問を返して回答待ち）**（2026-08-01・Pro 依頼 K-87・**急ぎではない＝Pro はテキスト置換のフォールバック案を保有**）。依頼＝`logicalApps: Record<string, number>` の受け口。狙いは設定ファイルの環境非依存化。**【測定】解決機構は `src/node/appProfiles.ts` の前処理**＝**字句認識つきテキスト書き換え**（文字列・バッククォート・コメントを読み飛ばす）で core パーサは `LAPP_` を知らない。**Pro の懸念 3 点（文字列内誤置換・表示の食い違い・字句規則の二重管理）はエンジンの機構がすべて解決済み**＝依頼の筋は良い。診断は**併記**（`LAPP_ORDERS -> APP1234@prod`）・未定義名は**名前入り `ArgumentError`**。**【前提が覆った】論理名は ASCII 限定**（`[A-Z][A-Z0-9_]{0,63}`・大小無視）で、**依頼文の例 `LAPP_案件管理` は現行構文で PARSE_ERROR**（実測。`案` が名前開始文字でない）。**Pro のフォールバックは日本語名を扱えるため、受け口を開けるだけでは欲しいものにならない可能性**。→ **返信で「ASCII 名で足りるか」を質問**〔足りる→案 A（スキャナを `fs` 非依存の共有モジュールへ分離・`@profile` は browser で拒否・ゼロ依存維持）＝費用 小〜中／日本語必須→案 B（構文拡張・**CLI / MCP も同時に広げる＝面で同一**・文字種・NFC 正規化・識別子境界の設計）＝費用 中〜大〕。**回答を待ってから実装を判断**。 | 改善 | 中 | [B107](internal/ksql_b107_lapp_engine_library_issue.md) |
