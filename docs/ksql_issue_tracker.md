...
- 次回リリース計画: **未定**。要対応は **B130・B133**（B129 は `ksql-analytics` へ照会中）。**B128 は保留**（スコープ判断が先）。他は優先 低 か実需待ち（§1）。クローズ済みは §3。
