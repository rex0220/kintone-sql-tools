...
| B33 | **`KORDER BY` 大規模窓のCursor API対応** | 改善 | 🚧 **実装・実機完了（2026-07-18）・v3.1.0 リリース準備完了**。`KORDER_CURSOR` planner/executor、必須`openCursor`契約、Node/plugin adapter、retryなしRequestGate、host lease・Create直列化・cleanup再確認・quarantine、batch timeout cleanup、CLI/env/profile/MCP/plugin設定、EXPLAIN/metrics/文書を実装済み。レビュー指摘P2×2/P3×1に加え、実機smoke指摘のMCP `ksql_explain.maxRecords`とCLI通常EXPLAINのconcurrency表示を修正。**順序release blockerはAPP730・618,525件で10,001件/21ページ・同値群境界・ASC/DESC・LIMIT/OFFSETをraw Cursor APIと照合し7/7完全一致で解消**（[evidence](internal/evidence/b33_cli_mcp_smoke.md)）。通常ORDER/FULL_SCAN/JOIN/DMLは現行方式を維持する。5分Create timeoutは再現不能のため専用応答判定を推測せず、応答喪失はquarantineする。**plugin release blockerも解消（Chromium/Firefox・[evidence](internal/evidence/b33_plugin_smoke.md)）**＝SPA遷移は完走・F5で離脱DELETE送信・blocked GET直後のfinally DELETE=200・quarantine→CursorCapacityError実証・二重DELETE rejectは{code:GAIA_CN01}でstatusプロパティ無し（plugin専用判定の裏付け）。残=リリースPR→tag→publish | **機能・性能** | 中 | [spec](internal/ksql_cursor_api_fetch_spec.md) / [plan](internal/ksql_korder_cursor_implementation_plan.md) |
...
| B35 | **プラグインが message なしのネットワークエラーを「⚠」のみで表示する** | **バグ**（表示・診断性） | 📝 課題R1・実機確認済み（2026-07-18・B33 plugin smoke で発見・**B33 固有でなく既存挙動**＝records.json 遮断/オフラインでも同経路）。ネットワーク層の reject は message を持たず、toDetailedApiError が素通し→renderError の String() が空相当になり文言ゼロ。対策=message なし reject を fallback 文言付き Error へ正規化＋renderError 側の二重防衛。cleanup 自体は正常（blocked GET 直後の DELETE=200 実測）。SemVer=patch | **正しさ**（診断性） | 低 | [issue](internal/ksql_plugin_messageless_error_display_issue.md) |
