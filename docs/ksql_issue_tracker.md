...
- 次回リリース計画: **B93・B94・B95 実装済み（未リリース）＝minor 相当（v3.32.0）**。**B94 と B95 は Pro が報告した「10,228 件が 10000 と表示される」症状に対応する**〔B94＝`COUNT(*)` に `maxRecords` を適用しないので**その形では症状自体が消える**／B95＝打ち切りを `metrics.limitReached` で判別できるので**残る経路でも利用者が分岐できる**〕。**B91** は M6 待ち。**B95 案 B（打ち切られた入力の集計を fail-closed）は要望次第**。
