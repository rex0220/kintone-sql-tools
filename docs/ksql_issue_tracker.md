...
| B35 | **プラグインが message なしのネットワークエラーを「⚠」のみで表示する** | **バグ**（表示・診断性） | 🚧 **実装・実機完了（2026-07-18・プラグイン実機で fallback 文言表示をユーザー確認・B23 の LENGTH_CHAR もプラグイン面で確認済み）・v3.2.0 リリース待ち**。message なし reject を cause 保持の fallback Error へ正規化＋renderError の最終防衛 | **正しさ**（診断性） | 低 | [issue](internal/ksql_plugin_messageless_error_display_issue.md) |
