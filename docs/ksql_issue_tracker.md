...
|---|---|---|---|---|---|---|
| B117 | `DATE_ADD` の月・年加算が月末で翌月へ繰り上がる | 課題 | 🔧 **対応中（案 1＝月末へ丸める・オーナー決定 2026-08-04）**（Pro 報告 K-112・**こちらの実測で完全一致**）。`DATE_ADD('2026-01-31', 1, 'MONTH')` が **`2026-03-03`**（期待 `2026-02-28`）。`DATE_ADD('2026-03-31', -1, 'MONTH')` も**同じ日**になり、**往復しても戻らず順序も壊れる**。`DATE_ADD('2024-02-29', 1, 'YEAR')` → `2025-03-01`。`DAY` は影響なし。**原因＝`applyDateAdd`（evalFunc.ts:633）が JavaScript の `Date` の正規化をそのまま漏らしている**（2 月 31 日 → 3 月 3 日）＝設計された挙動ではなく副産物。**エラーにならず、もっともらしい日付が返るため気づく手段がない**。**判断＝丸める**〔①いまの出力は擁護できる意味論ではない②**主要な SQL エンジンはすべて丸める**（MySQL / PostgreSQL / Oracle）③**静かに間違う形**で締め日が月末の業務が踏む——本プロジェクトは同種を fail-closed で潰してきた（B97/B98/B102）のでここだけ残すのは一貫しない④壊れるのは「今日ちがう月の日付を返している」ケースだけ〕。**丸めの有無を切り替えるオプションは採らない**（設定で SQL の意味が変わる形・B107 と同じ判断）。**往復は丸めが起きた場合だけ戻らない**（`1/31 → 2/28 → 1/28`・MySQL も同じ）ことをテストで意図として固定する。リファレンスに月末の扱いを明記（現状は書式しか書いておらず利用者は実測して初めて気づく）。**優先 高**・修正は小・**minor＋移行案内**。 | 正しさ | 高 | [B117](internal/ksql_b117_date_add_month_end_issue.md) |
| B116 | 未リリースの文書変更を次の機能リリースに同梱する | 課題 | 📝 **待機（次の機能リリースに同梱・単独リリースはしない）**（2026-08-04 オーナー判断）。v3.41.0 の後に**文書のみの変更が 3 件**たまった〔①日時フィールドの境界を**半開区間の推奨**へ（B113 の副産物。v3.39.0 で書いた RFC3339 の案内は kintone 側の TZ 解決に依存する）②README「ローカル開発」に**テスト実行時は `KSQL_*` を外す**ことと落ちたらまず `env | grep KSQL_` を見ることを記述（B115）③engine ライブラリ文書に**未知の `fetch` 値を「軽い側」へ寄せない**旨を追記（B114・Pro の実装から。寄せると将来より重い区分が入ったとき全件取得を安全と誤って見せる）〕。**いずれも main にコミット済み**で、**残る作業は次のリリース時に CHANGELOG へ 1 節としてまとめること**だけ。**単独の文書リリースを出さない理由**＝3 件とも「読めば分かる」性質の改善で、成果物 3 点の再ビルドと検証に見合わない。**ただし忘れると価値がゼロ**（とくに①は v3.39.0 で出した案内より安全な書き方）なので起票する。**優先 低・ただし次のリリース時に必ず消費すること。** | 改善 | 低 | [B116](internal/ksql_b116_pending_doc_changes_issue.md) |
