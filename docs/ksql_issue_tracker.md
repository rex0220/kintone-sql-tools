...
- 最終更新: 2026-08-05
- 現在の最新リリース: **v3.44.0**（2026-08-05・npm publish 済み）。→ [リリース履歴](ksql_release_history.md)
- 次回リリース計画: **未定（未リリースの変更なし）**。要対応は **B123**（`EXPLAIN` / `--dry-run` が通常の `GROUP BY` で落ちる・原因特定済み・純加法 1 条件・優先 中）。**B125**（集計ウィンドウ `SUM() OVER`・実需あり・機構は既存）は方向判断待ち、**B124** は文書追記が先。他は優先 低 か実需待ち（§1）。クローズ済みは §3。
...
|---|---|---|---|---|---|---|
| B123 | 通常の `GROUP BY` だけの SELECT は `EXPLAIN` / `--dry-run` がエラーになる | 不具合 | 🚧 **実装済み・リリース待ち**（2026-08-05・**実機確認済み**）＝`selectNeedsOwnMetadata` に `statement.groupBy.length > 0` を追加（純加法 1 条件）。CLI `--dry-run` で計画が返り、`metadata API: form definition` のみでレコード API を呼ばないことを実機確認。`npm test` 5,359 件成功。（起票時の分析は以下）**原因特定済み**＝`selectNeedsOwnMetadata` がメタデータ要否の判定に**通常の `GROUP BY`（`statement.groupBy`）を見ていない**（B65 の `ROLLUP`/`GROUPING SETS` は入っている）。偽になると `noOpClient` が渡り、`GROUP BY` の計画作成が `getFields` を呼んでガードに弾かれる（`No-op client should not be called.` / CLI は `DryRunError`）。**MCP と CLI が同じ述語**（`dryRunNeedsMetadata`）のため両方で再現。**【実測 v3.44.0・6 形】分かれ目は `GROUP BY` の有無ではなく「`GROUP BY` があり、かつフィールドを参照する `WHERE` も `ORDER BY` も無い」こと**〔`GROUP BY` のみ ❌／`+ORDER BY` ✅／`+WHERE ... IN` ✅／`+HAVING` ❌／JOIN+`GROUP BY` ❌／JOIN+`GROUP BY`+`ORDER BY` ✅〕。**`HAVING` は判定に寄与しない**。**回避策＝`ORDER BY` を 1 つ足す**。**軽微な理由＝エラーで止まっており誤った計画は返していない**（「静かに間違う」系ではない）。**重い理由＝分析クエリはほぼ全部 `GROUP BY` を含むため「大量取得や JOIN は `EXPLAIN` まで通す」という運用ルールが集計に対して最初から機能しない**（出典セッションで 2 記事ぶん気づかれなかった）。**裏づけ＝計画側は `GROUP BY` をメタデータ必要と扱っている**（通った計画に `metadata API: form definition` と `group key 分類: PHYSICAL (source=0, field=分類)`・`complete input reason: GROUP_BY`）。**案 A（推奨）＝述語に `statement.groupBy.length > 0` を足す純加法 1 条件**（`explainNeedsAppMetadata` は WITH/UNION/サブクエリを再帰走査するので CTE 内の `GROUP BY` も同時に直る・レコード API を呼ばない契約は不変・CLI も同時に直る）。**要確認＝プラグイン（desktop.js は EXPLAIN エンジンをバンドル）**。（2026-08-05・`ksql-analytics` の在庫分析セッションで AI が遭遇 → 記事の裏取りで真因特定） | 正しさ | 中 | [B123](internal/ksql_b123_explain_groupby_metadata_issue.md) |
| B125 | 集計のウィンドウ関数（`SUM(x) OVER (...)`）— 累計・移動集計 | 改善 | 🚧 **実装済み・リリース待ち**（2026-08-05・**実機で外部検算済み**）→ [実装報告](internal/ksql_b123_b125_codex_impl_report.md)。`SUM`/`COUNT`/`AVG`/`MIN`/`MAX` の `OVER`・既定 `RANGE`・明示 `ROWS`/`RANGE`・`AGGREGATE_WINDOW` 完全入力・`EXPLAIN` の実効フレーム表示・`window-functions` の docs セクション独立化まで実装。`npm test` 5,359 件成功。**外部検算＝APP4228 の 8 製品の最小在庫が `ksql-analytics` の独立実装（40 行スクリプト）と完全一致**（26/34/37/74/21/29/61/42・件数合計 1,000）。**型メタ修正の実証＝牛乳が min=74/max=1004**（コードポイント順なら逆になる）。**同日 3 件で `RANGE`=444/444/444（末尾値）・`ROWS`=530/496/444**。**仕様 §3.2 の数値例は 2 回誤り**（R1=130・R2 初版=80・正しくは 110）で実装レビューが検出——アルゴリズムと数値例を別々に書くと数値例だけが腐る。（仕様の経緯は以下）→ [Phase 1 仕様 R2](internal/ksql_b125_aggregate_window_phase1_spec.md) / [codex レビュー1](internal/ksql_b125_codex_review_1.md)（高 6・中 5・低 1 を**全件反映**）。**R1 で誤っていた 6 点**〔①**`SELECT DISTINCT` 併用は v1 で実装済みなのに「不可」と書いていた**（実装すると既存機能を壊すところだった）②`RANGE` 比較表の値が**同順グループの先頭値**になっていた（正しくは末尾値＝日次残高）③集計引数の物理フィールドが `selectToKintone` で収集されず**全行 undefined でスキップ**される④`WINDOW_COL` は型メタが**一律 number** で `MIN`/`MAX` が壊れる（`execute.ts` に 4 箇所）⑤`ORDER BY` 無しの集計ウィンドウが**完全入力を要求せず** truncate で部分集計になる⑥「通常集計値と必ず一致」は**浮動小数の加算順序**と **canonical 同値の raw 表記**で成立しない〕。**R2 で追加した契約**＝完全入力 reason `AGGREGATE_WINDOW`／`MIN`/`MAX` は `inferAggregateArgMeta` を通す／一致条件は整数=完全一致・小数=相対誤差 1e-12・`MIN`/`MAX` は canonical 比較（raw 表記は契約外）／`frame.source` で既定と明示を区別／パーサ変更は `parseSelectColumn` のトップレベル分岐に限定（`parseAggregateRef` に入れると HAVING 等まで飲み込む）。**着手条件だったフレーム既定値はオーナー判断で確定＝標準どおり `RANGE`**（他 RDB と同じ値になる）。**同順の取り違えは 3 点セットで緩和**〔①`ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW` の明示を Phase 1 に入れる ②**`EXPLAIN` に実効フレームを出す**（`frame: RANGE ... (既定)`）③言語リファレンスに `RANGE`/`ROWS` 比較表を載せる〕。**Phase 1 対象＝`SUM`/`COUNT`/`AVG`/`MIN`/`MAX` の 5 つ**（`MEDIAN`/`MODE`/分散系は**増分計算できず O(N²) になる**ため除外、`GROUP_CONCAT` は出力が O(N²) バイト）。**中核設計＝`evalAggregate` の「行→値/スキップ」を切り出して共有**し、空値・比較規則を通常集計と 1 箇所に閉じる（→ 受入の「一致条件」＝**フレームがパーティション全体に一致する行ではウィンドウ値＝通常集計値**で機械的に検証できる）。**実装順は B123 → B125**（`explainMetadata` の同じ関数を触る）。**外部検算は手元にある**（`inv_v2_runstock.mjs` の 8 製品×最小/平均/最大/最終＝行ごと累積なので `ROWS`＋全順序と一致するはず）。**最大のリスク＝`evalAggregate` は 12 集計共有コードで、切り出しを誤ると全集計に波及**（回帰で B119〜B122 の受入を全再実行）。（起票の経緯は以下） **【起票時メモ】****実需あり**＝在庫の累積推移を出せず、**SQL を諦めて CSV + ローカル 40 行のスクリプト**で計算した（`ksql-analytics` v2 分析）。**[ウィンドウ関数仕様](internal/ksql_window_function_spec.md) §3 で「非対象（v2）」・§8 で後続と既に明記済み**。**【実測 v3.44.0】`SUM(仕入価格) OVER (ORDER BY 製品名)` → `ParseError: 文の区切りには ; が必要です（位置 22、トークン: 「OVER」）`＝未対応であることすら伝えていない**。**機構は既にある**＝`applyWindow`（`src/engine/process.ts`）のパーティション分割・ソート・**同順境界の検出**（`sortedResult.compare` — `RANK` のために作ったもの）が**そのまま `RANGE` フレームに要るもの**で、足すのはアキュムレータだけ。FULL_SCAN 強制・比較器共有・ソートメタのゲートは仕様 §0.3/§2.3/§5.1 で確立済み。**最大の論点＝フレーム既定値と同順の扱い**〔標準の既定は `RANGE UNBOUNDED PRECEDING AND CURRENT ROW` で**同順の行は同じ値**＝同日取引のある台帳では「その日の終わりの残高」を返す。出典が欲しかったのは**取引ごとの残高**（`ROWS`）で、レコード番号をタイブレークに足して回避していた〕。**既定だけ実装して `ROWS` を出さないと、行ごとの累計を書いたつもりで日次残高を受け取り、エラーも出ない**。**案 A（推奨）＝Phase 1 は `SUM`/`COUNT`/`AVG`/`MIN`/`MAX` ＋ 既定フレーム ＋ 明示 `ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW`**（`ROWS` は同順判定を外すだけで既定より簡単。**移動フレームだけが本当に高い**ので切る）。**案 B＝診断だけ直す**（案 A を採らなくても要る）。**着手条件＝同順の扱いを受入条件で固定してから**。**受入データは手元にある**（ローカルスクリプトの 8 製品×最小/平均/最大/最終）。（2026-08-05・`ksql-analytics` v2 分析から起票） | 機能 | 中 | [B125](internal/ksql_b125_aggregate_window_function_issue.md) |
| B124 | 集計算術式に非集計オペランド（`GROUP BY` 列・`@変数`）を書けない | 改善 | 📝 **評価（方向判断待ち）**（2026-08-05 起票）。`SUM(t.個数) * m.仕入価格`（**`m.仕入価格` は `GROUP BY` に含まれている**）が `ParseError`。原因＝`parseAggPrimary`（`src/parser/parser.ts`）が**集計関数・数値リテラル・括弧・単項マイナスのみ**を受ける意図的な fail-closed で、**バグではなく仕様**。**【実測で射程が広がった】`@変数` も拒否される**（`SUM(仕入価格) * @r` → 同じ ParseError）——`@変数`は**グループ内一定が構造から保証**されるので、グループキー判定を要しない最も安全な部分集合。**重くない理由＝エラーで止まる／回避策 `SUM(a * 単価)` がグループ内一定なら厳密に等価＝新しい表現力は増えない／B120 の位置・トークン診断が効き出典でも 1 回で直せた**。**それでも起票する理由＝標準 SQL では当然書ける形で経験者ほど先に書く／`GROUP BY` に含めているのに拒否され理由が分からない／制約が言語リファレンスに 1 行も無い**。**案 C（どの案でも必須）＝§8 に制約と回避形を 2 行追記**。**案 A1＝`@変数`を許可**（安い・単独出荷可）。**案 A2＝`GROUP BY` キーのフィールド参照を許可**（機能従属性の推論は**しない**・`GROUP BY` 無しは常に拒否）。**A2 の費用は実装より受入条件側**〔①**alias が落ちない回帰**（`parseAggregateColumn` の `AS` 非消費ヘルパーを触るため集計算術式 alias 消失バグの回帰を全再実行）②**`HAVING SUM(a) * 単価 > 100` が通るのか静的に拒否かを固定**（B122 で `HAVING` の集計式が評価可能になったため。曖昧なまま出すと「別名なら通る/直接なら 0 行」の再来）〕。**見立て＝まず案 C（文書 2 行）→ A1 は安いので単独出荷可 → A2 は実需が出てから**。（2026-08-05・`ksql-analytics` 在庫分析から起票） | 機能 | 低 | [B124](internal/ksql_b124_aggregate_arithmetic_nonaggregate_operand_issue.md) |
...
| バージョン | 内容 |
|---|---|
| **v3.44.0** | B119〜B122 集計まわりで静かに間違う 4 件を修正（**結果が変わります**・移行案内あり）。集計の引数の文字列関数／`CASE` 内の集計／`HAVING` の文字列比較・集計式 |
