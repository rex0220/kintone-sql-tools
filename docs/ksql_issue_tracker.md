...
- 次回リリース計画: **未定**。着手候補は **B91**（依頼③の再評価・許可集合は `runBatch` 相当で確定済み）。他は実需待ち（B53 / B54 / B63）・継続運用（B61）・保留（B73）。
- **Pro（ksql-dashboard-pro）**: 依頼①②を v3.31.0 で実装 → 回帰（B92）を一度公開し v3.31.1 で修正。**v3.31.1 の連絡を送付済み（2026-07-29）**＝[連絡](internal/ksql_pro_report_v3311.md)。**次は返信待ち**（v3.31.0 を飛ばす案内・`EXPLAIN <DML>` と非数値変数の該当有無・M6 の受け入れテスト）。
