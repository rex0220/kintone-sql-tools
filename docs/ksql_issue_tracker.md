...
| B9 | **最大30桁の厳密10進比較** | **バグ / 言語意味論** | 🔗 **R5 中優先＋実装計画に進める状態（2026-07-18 棚卸しで確認・陳腐化なし）**。衝突は2^53=9.0e15（16桁）超のみ・`Number()`丸めは単調で**順序逆転なし偽同値のみ**・`$id`/`RECORD_NUMBER`はv3.0.0で厳密化済み。**着手時に取り込む点=①既存 `dmlValidation.ts:compareDecimal`（厳密10進文字列比較・範囲検証で使用中）を拡張/共通化して新規実装の二重化を避ける②算術由来`"1e+21"`の科学記法正規化③`GREATEST`/`LEAST` 経路（影響範囲リスト漏れを補記済み）**。再昇格トリガー=①16桁超実データ報告②B29着手（同領域・同時が安価）。`decimalPlaces`/`roundingMode`の量子化はB29へ分離 | 正しさ | 中 | [issue](internal/ksql_exact_decimal_compare_issue.md) |
