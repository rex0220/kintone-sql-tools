...
| B16 | 文字列集約 `GROUP_CONCAT`（1対多の連結・`#err` メッセージ集約） | 改善 | 📋 仕様案 R2（codex レビュー反映・実装着手可）。**B14 とは独立**（型メタ不要）・v2.14.0 で下地あり。v2.15.0 予定 | 機能 | 中 | [spec](internal/ksql_group_concat_spec.md) |
