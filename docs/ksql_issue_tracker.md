...
- 最終更新: 2026-08-05
- 現在の最新リリース: **v3.46.0**（2026-08-05・npm publish 済み）。→ [リリース履歴](ksql_release_history.md)
- 次回リリース計画: **未定**。`ksql-analytics` の依頼から **B126〜B132** を起票（→ [評価](internal/ksql_analytics_request_20260805_evaluation.md)）。**要対応は B126・B127**（いずれも**利用者から検出できない**系列で `warnings` の純加法で塞げる）。他は §1。クローズ済みは §3。
...
|---|---|---|---|---|---|---|
| B126 | 選択系フィールドの `=` が警告なしで全件取得になる（**方針転換＝警告ではなく押し下げ正規化**） | 改善 | 🐞 **残課題（未着手）**（2026-08-05 起票・`ksql-analytics` の依頼 ①）。**【実測 v3.46.0】`WHERE 入出庫区分 = '出庫'` は `fetch summary: ALL`＋`reason: WHERE_RESIDUAL` で `warnings: []`／`IN ('出庫')` は `PREFILTERED`＋`kintone query: 入出庫区分 in ("出庫")`**。結果は同じで違うのは転送量だけ、しかも警告が無いので**踏んでも誰も気づかない**（B119〜B122 が「値が黙って誤る」なのに対し「コストが黙って跳ね上がる」）。エージェントは自然文の依頼に対して**まず `=` を書く**。**【重要】依頼の案 A（`=`→`in` 正規化）は対象 4 型のうち複数値型で意味が変わる**＝`CHECK_BOX`/`MULTI_SELECT` の `IN` は**集合 overlap（いずれかを含む）**でスカラー等値と別物（言語リファレンス §6・`ksql_language_reference.md:1080,1096`）。**正規化すると結果が変わり「静かに間違う」を新たに作る**。**【方針転換 2026-08-05・オーナー判断】「書き換えを案内できるなら押し下げも可能では？」→ そのとおりだった。案 A（正規化）を Phase 1 にする**（→ [仕様 R3](internal/ksql_b126_b127_warnings_phase1_spec.md)）。利用者は何も書き換えなくてよく、**警告そのものが不要になる**。R2 で悩んだ JOIN の文面・重複排除・運搬経路の論点もまとめて消える（**正規化は「利用者が `IN` と書いた場合と同じ AST」に落とすので下流に分岐を足さない**）。**唯一の障害は実在しない選択肢**＝**【実測】`= '存在しない値'` は 0 行で正常終了するが `IN ('存在しない値')` は `GAIA_IQ10` エラー**（COUNT 経路・通常経路の両方。`ksql_language_reference.md:447` に文書化された既存挙動）。**無条件の正規化は動いていたクエリを壊す**ため、**`optionOrder` で実在を確認できたときだけ正規化する**。**`!=` は Phase 2**（`not in` は空セル次第で部分集合になり行が落ちる）。**対象は単一値の `LOCAL_SCALAR_TYPES` のみ**（複数値は `IN` が集合 overlap で意味が変わる・`USER_SELECT` 等が該当／`CHECK_BOX`・`MULTI_SELECT` は手前で `UNSUPPORTED`）。（→ [評価](internal/ksql_analytics_request_20260805_evaluation.md)） | 正しさ | 高 | [評価](internal/ksql_analytics_request_20260805_evaluation.md) |
| B127 | ウィンドウ既定フレーム `RANGE` が警告なしで別の意味の値を返す | 改善 | 🐞 **残課題（未着手）**（2026-08-05 起票・依頼 ②）。**【実測 v3.46.0】`SUM(x) OVER (PARTITION BY p ORDER BY d)` は `frame: RANGE ... (既定)` と表示するが `warnings: []`**。標準どおりの既定だが、**同日 3 件の台帳は 3 行とも「その日を締めた残高」**になり、取引ごとの残高を期待して書くと**エラー無しで別の意味の値**が返る。在庫台帳・入出金明細では同順の行があるのが普通で、**レポートまで届く**。**標準準拠（既定 `RANGE`）は変えず、`ORDER BY` あり `ROWS` 未指定のとき `warnings` を 1 行**。`ORDER BY` が一意キー（`レコード番号`/`$id`）を含み全順序と静的判定できる場合は出さない。（→ [評価](internal/ksql_analytics_request_20260805_evaluation.md)） | 正しさ | 高 | [評価](internal/ksql_analytics_request_20260805_evaluation.md) |
| B128 | 集計ウィンドウ Phase 2（移動フレーム・`LAG`/`LEAD`） | 改善 | 📝 **評価**（2026-08-05 起票・依頼 ③）。[B125 仕様 R2](internal/ksql_b125_aggregate_window_phase1_spec.md) §10 で Phase 2 と明記済みのもの。**実需の裏づけ＝B125 で累積が SQL に戻った結果、明細を CSV へ落とす理由が 2 つだけになった**（移動平均＝7 日平均の在庫・出庫／前期比＝前月比・前年同月比）。**フレームが固定境界のみのため CTE を重ねても書けず、自己結合も `JOIN ... ON` が等値 1 本なので「前月」を作れない**。優先は移動フレーム（在庫・需要の平滑化は定番）→ `LAG`。（→ [評価](internal/ksql_analytics_request_20260805_evaluation.md)） | 機能 | 中 | [評価](internal/ksql_analytics_request_20260805_evaluation.md) |
| B129 | ウィンドウ結果を同一 SELECT の式に使えない（診断に次の一手が無い） | 改善 | 📝 **評価**（2026-08-05 起票・依頼 ④）。ABC 分析の構成比・累積構成比は「総計で割る」ため **3 段 CTE** になる（集計 → `SUM(x) OVER (...)` と `SUM(x) OVER ()` を列に出す → 次段で割る）。2 段に詰めると診断で止まる。**診断は的確だが次に何を書けばよいかは分からない**。**案 B（安価・実用上これで足りる）＝診断文に「総計は `SUM(x) OVER ()` として別列に出し、割り算は次段の CTE で」を足す**。案 A（同一 SELECT で使えるようにする）は評価順の追加が要り高い。**依頼元は 3 段構成で回避できており優先は B128 より下**。（→ [評価](internal/ksql_analytics_request_20260805_evaluation.md)） | 改善 | 中 | [評価](internal/ksql_analytics_request_20260805_evaluation.md) |
| B130 | `ksql_describe_app` に `lookup`/`unique`/`calc` のフラグ | 改善 | 📝 **評価**（2026-08-05 起票・依頼 ⑤）。`describe` はフィールドコード・ラベル・型だけを返し、ルックアップか素の文字列か・`unique` の有無・CALC の中身が分からない。**型からは推測できないが推測がしばしば当たる**ため、外れたときだけ静かに間違う。**実例＝履歴側の `製品名` を素の文字列と推測して「結合キーが無い」とレポートに書いたが、実際はルックアップでマスタ側は `unique: true`**（結論は正しく理由が誤っていた）。**依頼側の案を採る**＝値の中身は不要で**フラグ 3 つ**だけ。「型だけでは分からない何かが付いている」の 1 ビットが分かれば `ksql_app_metadata` を引く判断ができる。**Qiita 下書きで私は「説明文ではなく出力に 1 行足す」案を推したが、依頼側の案のほうが停止規則が明確（3 つで打ち止め）で行動にも直結する**ため差し替える。（→ [評価](internal/ksql_analytics_request_20260805_evaluation.md)） | 改善 | 中 | [評価](internal/ksql_analytics_request_20260805_evaluation.md) |
...
| B132 | `ksql_docs` のセクションキー `window-functions` だけ番号が無い | 改善 | 🚧 **実装済み・リリース待ち**（2026-08-05）＝キーを `10-1-window-functions` へ改め他の 26 セクション（`01-basic-rules`〜`26-assert`）と表記を揃えた。**旧キー `window-functions` は `LANGUAGE_SECTION_ALIASES` で引き続き解決**（索引には出さない）ので利用側は壊れない。（起票時の分析は以下）（2026-08-05 起票・依頼の補足）。**v3.45.0（B125）で入れた自分の退行**。他は `01-basic-rules`〜`26-assert` と `## N.` 見出しから番号が導かれるが、`## 10.1 ウィンドウ関数` は導出規則に乗らず番号なしで追加された。**AI が `10-1-window-functions` のような番号付きキーを推測して外した実例あり**（依頼元）。`10-1-window-functions` にすれば一貫する。索引の一貫性の問題で実害は無い。（→ [評価](internal/ksql_analytics_request_20260805_evaluation.md)） | 改善 | 低 | [評価](internal/ksql_analytics_request_20260805_evaluation.md) |
