...
- 次回リリース計画: **未着手**。**B91**（MCP のプラグイン互換モード）は Pro の M6 待ち。**B95 案 B**（打ち切られた入力の集計を fail-closed）は**要望次第**。残るバックログは B53（18〜29 人日・実需待ち）・B54・B61・B63・B73（保留）。
- **Pro（ksql-dashboard-pro）**: v3.31.1 の検証完了（依頼①②とも動作・§4§5 該当なし・CI 199 件通過）。回帰報告は取り下げ → **B93** で伝え方を改善。**新たに依頼④（取得上限の構造化フラグ）を受領 → B95 として起票**。**急ぎではない**（Pro 側は `onLimitReached: "error"` で解決済み）。**次は M6 の受け入れテスト結果待ち**。依頼③（B91）は再評価待ち。**v3.32.0 の連絡が必要**＝B94 が「10,228 件が 10000 と表示される」症状そのものを解消し、B95 で残る経路を判別できる。
