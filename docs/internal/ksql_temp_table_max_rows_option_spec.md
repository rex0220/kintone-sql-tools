...
- **SQL 履歴**: `SqlHistoryItem` に `tempTableMaxRows` をスナップショット保存し、履歴からの再実行時に復元。**3値セマンティクス(R6)**: number = 明示値 / **null = 空欄(既定 10,000)で実行した明示スナップショット**(JSON でキーが消えないよう null で保存)/ undefined = 旧履歴(キーなし)のみパネル現在値にフォールバック。レコード連携(`readTempTableMaxRowsField`)も同じ3値(フィールドなし = undefined / 空欄 = null / 値あり = number)
