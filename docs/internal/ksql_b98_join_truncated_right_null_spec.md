...
- ステータス: ✅ **完了（v3.34.0 でリリース）**（2026-07-29・実機確認済み）
...
## 9. 実装後の実機確認（2026-07-29・APP4225 / APP4226）

**モックではなく実 kintone で 4 形を確認した。**

| | SQL | 上限 | 結果 |
|---|---|---:|---|
| ① | `APP4226 l LEFT JOIN APP4225 r`（**右＝保持されない側**が打ち切り） | 20 | **エラー** |
| ② | `APP4226 l INNER JOIN APP4225 r`（同条件） | 20 | **行＋警告**（対象外） |
| ③ | `APP4225 l LEFT JOIN APP4226 r`（**左＝保持側だけ**が打ち切り） | 25 | **行＋警告**（従来どおり） |
| ④ | `APP4225 l RIGHT JOIN APP4226 r`（**main が保持されない側**） | 25 | **エラー** |

### 9.1 **③と④が案 C' の精度を示している**

**同じ 2 アプリ・同じ上限（25）・打ち切られるのも同じ `APP4225`。**
**違うのは `LEFT` か `RIGHT` かだけ。**

| | main（`APP4225`・30 件）の役割 | 結果 |
|---|---|---|
| ③ `LEFT JOIN` | **保持側**（偽の `NULL` は起きない） | **行が出る** |
| ④ `RIGHT JOIN` | **保持されない側**（偽の `NULL` が起きる） | **止まる** |
...
onLimit=truncateは使用できません。取得件数が上限（20 件）を超えました。
WHERE 句で絞り込むか、maxRecords を引き上げてください。
```

**どのアプリが原因かが出る**ので、`WHERE` で絞るか上限を上げるかの判断ができる。
**`complete input reason:` は 1 回だけ**（受入 7）。

### 9.3 実装が仕様より一段精密だった点

仕様 §3 の表からは導けるが**明示していなかった形**を、実装が拾っている。

```ts
// a INNER JOIN b RIGHT JOIN c  →  b は RIGHT の左累積に入るので保持されない側
if (join.type === "RIGHT"
  && statement.joins.slice(0, index).some((prev) => table === prev.table)) return true;
```

**`INNER` で結合したテーブルでも、後に `RIGHT JOIN` があれば保持されない側になる。**
