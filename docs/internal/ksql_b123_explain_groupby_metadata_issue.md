...
- 起票: 2026-08-05
- ステータス: ✅ **完了（v3.45.0 でリリース）**（2026-08-05・npm publish 済み・**実機確認済み**）。`selectNeedsOwnMetadata` に `statement.groupBy.length > 0` を追加（純加法 1 条件）。CLI `--dry-run` で `SELECT 分類, COUNT(*) FROM APP4229 GROUP BY 分類` が計画を返すことを実機で確認（`metadata API: form definition` のみ・レコード API 呼出なし）。`npm test` 5,359 件成功
