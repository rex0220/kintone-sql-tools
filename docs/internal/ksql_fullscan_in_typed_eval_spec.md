...
- 作成日: 2026-07-15
- ステータス: **実装済み・v2.5.0 リリース済（フェーズ1＝型メタ付き IN/NOT IN 評価）**
...
- **CTE・一時テーブル**は型来歴がないため `undefined`（従来比較）。`FROM #t` は TableRef で `cteName="#t", appId=0`＝CTE と同じ扱い（[parser.ts:1265](../../src/parser/parser.ts#L1265)）→ resolver は undefined。
  - **制限（明記）**: 一時テーブル/CTE の列は**文字列スナップショット**。**スカラー由来列（DROP_DOWN/RADIO/STATUS/テキスト/数値）は文字列比較で正しく動く**が、**複数値/オブジェクト由来列（CHECK_BOX/MULTI_SELECT/USER 等・JSON 文字列で実体化）は要素 `IN` が効かない**（文字列比較）。元アプリで絞ってから実体化するか、元アプリで評価する。これは新規不具合ではなく従来からの制限（フェーズ1 は元アプリのみ修正）。
  - **フォローアップ（別課題）**: 一時テーブルに列型メタを併せて保持すれば temp 経由でも要素 `IN` が動く（実体化値は既に JSON 文字列のため型さえ渡せばよい）。空 `SELECT *`/一時テーブルのスキーマ保持（empty-select 二次対応）と同基盤。
