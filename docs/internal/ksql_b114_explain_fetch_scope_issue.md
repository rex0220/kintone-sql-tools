...
- **公開型は optional**（消費側が壊れない。値は常に埋める）
- **`text` はこの構造から描画**する。両者が食い違わないことをテストで固定する
- MCP への構造化payload の露出は**第 2 段では行わない**（第 1 段の `fetch:` 行で足りる）

**詳細は第 1 段の完了後に確定する。**先に構造を決め切ると、第 1 段で分かる分類の実態と
ずれるため。
