...
特に #4 が地味に効いています。`SUM(CASE WHEN 商談フェーズ='受注' THEN 売上 END)` のような書き方を試す前に `ksql_docs` で仕様を確認し、「集計内 CASE は不可」と分かったうえで **会社×フェーズ別集計（#8）から受注済み売上を取り出す** 方針に切り替えています。回答中の「CASE は集計内で使えないようなので…別々に取得します」がこの判断です。
