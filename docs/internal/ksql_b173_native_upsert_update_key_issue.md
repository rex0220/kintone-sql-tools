...
- 状態: 📋 **仕様 R5 確定・実装着手可**（2026-08-25）＝実装未着手。**R1→R5 の 5 版・Claude レビュー 4 回**（Major 4 件＝`options.confirm` の欠落／非 opt-in 面で常に `INELIGIBLE`／単文 EXPLAIN の経路統合がスコープ過大／R2 の列挙型 AC と変更ファイル一覧の欠落〔R3 の出力欠落に起因・こちらの見落とし〕）。**R5 は復元のみで指摘なし。****§7 にオーナー指摘 5 件を反映**（オーサリング／検証面との食い違い＝MCP・プラグイン・**CLI**／権限の担保は検証面の仕事ではない／opt-in の寿命／公開面の影響を受ける利用者）。**R3 で入ったもの**＝CLI opt-in（実行ごとの明示フラグ）・`nativeUpsertCalls`・EXPLAIN の 3 状態・判定の 3 者共有・`confirm` の実動化。**R4 で直す 4 件は仕様 §16.3**（うち Major 2＝非 opt-in 面で常に `INELIGIBLE` になり可視化が無意味／単文 EXPLAIN の経路統合がスコープ過大）。**【決定 §7.10】`/flow` の既定は ON（opt-out）・CLI は明示フラグのまま既定 OFF**（オーナー判断。`/flow` は公開 4 日・利用者 1 者・その 1 者が変更を待っているため契約上の制約が無い。CLI は公開 4 か月半で利用者を数えられない）＝**R4 で AC-1 の書き換え・§12 への移行注記・CHANGELOG の明示・依頼元への通知が要る**。（**codex レビュー R1 反映／確認 6 点の回答受領〔§2.5〕／実機測定 5 項目完了〔§3.2〕**）＝実装未着手。**実測で設計が 2 点変わった**＝①`record` に updateKey フィールドを含めると **kintone が拒否する**ので組み立て後に除去が要る（挿入時のキー値は `updateKey` から入るので結果は等価）②**ソース内キー重複は kintone がリクエストごと拒否する**ので適用条件⑥は外せず、**全ソースを通した重複検出**（チャンク単位では不足）が新たに要る。**次＝仕様（codex）**。依頼書の事実主張 5 件を実装・kintone 公式文書と突き合わせて検算済み（§1）。**うち 1 件は依頼書より良い方へ外れた**（内訳判別は `revision` からの推測ではなく、レスポンスの `records[].operation` で厳密に取れる）。**3 件は依頼書が見ていない穴**（①適用可能なのは素の UPSERT だけ＝CHECK / APPLY / VALIDATE ONLY / IMPORT の事前 GET は分岐以外の用途があり外せない ②UPSERT モードは **アプリのレコード追加権限**を追加で要求するため、既定 ON にすると「編集権限だけで動いていた更新専用 UPSERT」が壊れる ③**公開クライアント契約とラッパー 5 層の変更**になる＝`putRecords` は戻り値を捨てており、能力を足しても途中のラッパーが落とす・`onChunkWritten` は混在チャンクを表現できない）。**見立て＝案 C（能力検出付き opt-in・素の UPSERT 限定）**
- 仕様: [B173 native UPSERT 仕様 **R3**](ksql_b173_native_upsert_spec.md)（2026-08-25・codex 起草／**Claude レビュー済み＝§16**）＝**R4 で 4 件直す**。R3 で入ったもの＝CLI opt-in（`--native-upsert`・実行ごと）／`ExecutionMetrics.nativeUpsertCalls`（`putCalls` の内数）／EXPLAIN の 3 状態（`ELIGIBLE`/`INELIGIBLE`/`UNKNOWN`）／判定を本実行・preview・EXPLAIN の 3 者で共有／`options.confirm` 契約が CLI で実動化。**R4 の指摘**＝①**[Major] 非 opt-in 面（MCP・プラグイン）で常に `INELIGIBLE` になり可視化の目的を達しない**〔面依存条件 1・2 と文/データ依存条件 3〜6 を分けて報告する〕②**[Major] MCP 単文 EXPLAIN の経路統合はスコープ過大**〔共有すべきは判定関数。`executeExplain` から呼ぶ〕③§12 に既定反転の想定を追加し**既定 OFF の理由を「公開面の非破壊性」へ書き換え**（反転はメジャー版）④プラグイン対象外の理由を差し替え。**（R2 までの記録）** R1 の Major 1 件（`options.confirm` の欠落）と軽微 2 件を R2 で反映し、R1 §15 の未確認 6 件のうち 3 件を実機で解消して本文へ確定事実として取り込んだ。残る未確認 3 件（実クライアント経由のエラー message／追加権限なしトークン／guest space）は仕様の採否条件ではなく、実装レビューまたはリリース前に確認する
- 種別: 改善（性能）
...
### 7.9 【再訂正】`/flow` は公開 4 日 — 契約論は `/flow` には効かない。**しかし CLI には効く**

【オーナー指摘 2026-08-25】「文書化された公開 API を利用する他の利用者はいないと考えられる。公開してまだ時間がたっていない」

**公開時期を確かめた:**

| 面 | 公開 | 経過 |
|---|---|---|
| **`/flow`**（`exports` の `./flow`・commit `ff08776`・v3.69.0） | **2026-08-21** | **4 日** |
| CLI（`src/cli/index.ts`・公開リポジトリ化 `8fe766d`） | **2026-04-07** | **約 4 か月半** |

#### 7.9.1 `/flow` については §7.8.3 の「契約の遵守」も弱い

§7.8.3 は「既知の利用者は壊れないが、**未知の npm 利用者がいるかもしれない**ので契約として union を広げられない」とした。**公開 4 日では、その未知の利用者は実質いない。** → **`/flow` に関しては、契約論を根拠に既定 OFF を主張するのは弱い。**

#### 7.9.2 **CLI は別**。ここは契約論がそのまま効く

**R3 で CLI にも opt-in を足した**（§7.5）。CLI は **4 か月半公開されており、利用者を数えられない**。dialect 0 の UPSERT を任意のトークン権限で回している利用者がいてもおかしくなく、**追加権限を持たないトークンで更新専用 UPSERT を回している運用が実在し得る**。

→ **CLI 側の既定 ON は採れない。** R3 は CLI を**実行ごとの明示フラグ**としているので、この点は既に正しい。

**面によって「既定を変えられるか」が違う**という整理になる:

| 面 | 公開の長さ | 既定 ON にできるか |
|---|---|---|
| `/flow` | 4 日・既知の利用者 1 者（ksql-flow・追加権限あり・変更を待っている） | **技術的には可能** |
| CLI | 4 か月半・利用者を数えられない | **不可**（明示フラグのまま） |

#### 7.9.3 契約論が消えた後、`/flow` の既定 OFF に残る理由は 2 つ

1. **採用時期を利用者が選べる**。ksql-flow は**受入基準 1（XL 相当）の実測**を予定している。既定 ON だと**最初の本番実行がそのまま実測になる**（比較対象を先に取れない）
2. **退避弁**。本番で問題が出たときに `false` に倒せる。既定 ON でも opt-out を残せば同じだが、**フラグ自体は結局要る**

→ **推奨は「`/flow` も既定 OFF を維持」。** ただし**根拠は契約ではなく「採用の制御」**に変わる。**判断としては既定 ON も十分あり得る**（利用者 1 者が変更を待っている状況なので）。**オーナーの判断で反転してよい性質の決定。**

#### 7.9.4 反転の条件も面ごとに分かれる

§7.7.4 で「反転はメジャー版に紐づける」としたが、**これは CLI にだけ当てはまる**。

- **`/flow` の既定反転はメジャー版に縛られない**（外部利用者がいないため、契約上の制約が無い）
- **CLI の既定反転は、やるならメジャー版**（利用者を数えられないため）

**そもそも CLI の既定を反転させる必要があるかも疑わしい**＝CLI の opt-in の目的は「本番と同じ経路でリハーサルすること」（§7.5.2）であって、CLI 自体を速くすることではない。**リハーサル用途なら明示フラグのままで良い。**

### 7.10 【決定】`/flow` の既定は **ON**（オーナー判断 2026-08-25）

**`/flow` は `enableNativeUpsert` の既定を `true` とする（opt-out）。CLI は実行ごとの明示フラグのまま既定 `false`（変更なし）。**

根拠は §7.9＝`/flow` は公開 4 日で外部利用者が実質いないため契約上の制約が無く、既知の唯一の利用者（ksql-flow）は**追加権限を持ち、この変更を待っている**。

**面によって既定が違う**が、理由は明確で文書化できる:

| 面 | 既定 | 理由 |
|---|---|---|
| `/flow` | **ON**（`enableNativeUpsert: false` で opt-out） | 公開 4 日・利用者 1 者・その 1 者が変更を待っている |
| CLI | **OFF**（`--native-upsert` で明示 opt-in） | 公開 4 か月半・利用者を数えられない。目的はリハーサルであって CLI を速くすることではない |

#### 7.10.1 これで変わること（R4 の §12 に列挙させる）

**ksql-flow はアップグレードしただけで native になる。** 結果（成功時のレコード内容）は同じだが、次が変わる:

- **書込順**（新規全部 → 更新全部 ではなく、ソース順の混在チャンク）
- **部分失敗時に確定している範囲**（ソース順の prefix）
- **`onChunkWritten.operation` に `"UPSERT"` が出る**（内訳プロパティも付く）
- **`ExecutionMetrics.postCalls` が 0 になる**（書込は `putCalls` に集約・`nativeUpsertCalls` はその内数）
- **`estimatedWrites` の算出式**（`ceil(insert/100)+ceil(update/100)` → `ceil(総数/100)`）

**§7.8.2 の実測では ksql-flow はいずれでも壊れない**が、**追随作業（推定式 7.2 / 10.2・公開仕様 §3.4・`records[].operation` への切替）の期限が「有効化したとき」から「アップグレードしたとき」に前倒しになる。** → **依頼元へ通知が要る**（§7.10.3）。

#### 7.10.2 採用制御の懸念は解消できる

§7.9.3 で「既定 ON だと最初の本番実行がそのまま実測になる」と書いたが、**opt-out があるので逆に A/B が取りやすい**＝`enableNativeUpsert: false` で現行経路の基準値を取り、外して native を測ればよい。**懸念は解消。**

#### 7.10.3 R4 と運用に追加する要求

1. **`/flow` の既定を `true`**、CLI は `false` のまま。**面ごとに既定が違う理由を仕様に明記**する
2. **AC-1（既定 OFF）を書き換える**＝`/flow` は「省略時に native になる」ことを固定し、`enableNativeUpsert: false` で現行経路に戻ることを別 AC で固定。**CLI 側の既定 OFF を固定する AC も要る**
3. **§12（移行と互換性）に §7.10.1 の 5 点を列挙**し、opt-out の書き方を示す
4. **`CHANGELOG.md` に「※ `/flow` の UPSERT は挙動が変わります」を明示**（[B171](../ksql_release_history.md) の「※結果が変わります」と同じ形。**結果は同じだが観測値と失敗時の状態が変わる**ので、その旨を正確に書く）
5. **依頼元への通知**＝リリース版数・変わる 5 点・追随作業が前倒しになること・opt-out の方法・**先に CLI の `--native-upsert` でリハーサルできること**
