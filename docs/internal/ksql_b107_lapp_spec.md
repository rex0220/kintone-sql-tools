...
- ステータス: ✅ **完了（v3.37.0 でリリース）**（2026-08-01）＝**受入 1〜15 すべて満たし全ゲート green**（204 suites / 5,100 tests・snapshot 22 不変）。**codex の停止 1 回**＝旧仕様（ASCII 限定）を固定する `config.test.ts` の `注文` で、**決定の変更として許可**（ルールが設計どおり働いた）
