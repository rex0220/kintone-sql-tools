...
- 作成日: 2026-07-11
- 更新履歴:
  - 2026-07-11 R5(実装後 codex レビュー反映・1件): S2 の INSERT_SELECT テストが 1 行書き込みのみ検証で、仕様 §3.3 / CHANGELOG が約束する「confirm / `dmlMaxRows` の件数判定に 1 行として乗る」が未検証だった → 既存テストに `confirm` を渡し `count === 1`・`operation === "INSERT"` を固定(dmlMaxRows は上位層が confirm を構成する仕組みのため件数伝播の検証で足りる — codex 指摘どおり)。あわせて集計算術式 alias 消失(仕様 R3-②)を `ksql_agg_arith_alias_dropped_issue.md` として起票(codex 推奨・本 PR のブロッカーではない)
  - 2026-07-11 R4(実装完了): S0〜S5 実施済み(ブランチ `fix/ungrouped-aggregate-empty-result`、コミット 029f151 → c04913f → 5c4537f → 0c7c72b → 3a1dd80)。検証: jest 751 件(dml_guard e2e 1件は既知フレーク・単独 green)/ tsc 10 件基準 / build 全成果物 / mcp:verify・mcpb:verify / dist-cli 直実行の実機確認(APP88: 0 件集計 → 0 の 1 行、ASSERT `= 0` 成立 exit 0・不成立 `actual: 10` exit 1)。S1 で仕様 R3 の 2 件(HAVING 文法制約・alias 消失バグ)が発覚、S2 で既存 UPDATE SET 0 行テストを非集計に書き換え(§3 リスク表の想定ケース)
...
- ステータス: **実装済み・v1.12.0 リリース済（GROUP BY なし集計は 0 件でも 1 行返却・SQL 標準準拠。言語リファレンス §8）**
