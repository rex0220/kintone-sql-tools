...
- **A-8（LIMIT>500 打ち切り）— v2.11.0 実装済み**: `ORDER BY` 空・`LIMIT` 明示・`offset + limit <= maxRecords`・KLIKE なしの SIMPLE SELECT に限定し、`fetchAll.stopAfter` で必要件数に達した時点で正常終了する。`ORDER BY` 付きの押し下げやフィールド定義判定は不要な安全サブセットへ絞った。詳細は [B8 仕様](ksql_limit_over_500_fetch_truncation_spec.md)
