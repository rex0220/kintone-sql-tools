...
- 起票: 2026-07-26
- ステータス: ✅ **完了（Phase A v3.26.0・Phase B v3.28.0・2026-07-27）**。Phase B で JOIN の server-only 15関数を解禁（第5-W / 第5-L）。実機 PASS＝ブラウザ プラグイン＋ローカル CLI 実データ。
...
**JOIN で押し下がらないのは DATE と TEXT である。**

> ### 【訂正 2026-07-27】選択系についての初出の記述は誤り
>
> 本節の初版は「選択系も全件取得」と書いていたが、**これは Claude のモック不備による誤測**だった。
> 選択系の押し下げは**選択肢の実在検証**を要求するため、`getFields` の応答に `optionOrder` が
> 無いと押し下げられない。初回の probe はこれを欠いていた。
> `optionOrder` を与えて再測したところ、**DROP_DOWN / CHECK_BOX の `in` は JOIN でも押し下がる**。
> B76 Step 0 の codex レビューで指摘され、再測で確認した。
>
> **B71 の教訓（モックは実際の応答形状を再現すること）を、起草者自身が踏んでいた事例である。**
>
> なお非実在値（`区分 in ('Z')`）は単一表では押し下がるが JOIN では拒否される。
> JOIN 側だけが実在検証を課しているためで、**安全側の差**であり問題ではない。
