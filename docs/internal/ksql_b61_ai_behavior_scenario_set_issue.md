...
- 起票日: 2026-07-22
- ステータス: 📝 **起票＋シナリオ台帳の5 ラウンド実施済み（2026-07-22・Claude Code 面 計 30 シナリオ・29 PASS＋**1 意味論 FAIL（B62 で解消）**＝[証跡](evidence/b61_scenario_smoke_claude_code.md)。構文発明ゼロ（カタログは機能）。**R3-3 で初の実質失敗を検出＝UPDATE の CHECK を post-image と誤解**（正=§17.3 更新前値・§16 組み込み検証の記述を誤適用・ksql_validate ok のため構文 guard では不可視）→「AI が意味論まで正しく使える」の三層目は行動検証でしか担保できないことを実証。改善候補=§16/§17.3 相互参照 or description へ pre-update 注記。**Q6 で改善候補 2 件を観測＝@変数の使用可能位置の可視性**（算術オペランド不可・SET 右辺の変数参照不可が読み取れず各 1 回自己修正）。残=変数制約の注記強化検討・スクリプト半自動化・Desktop 面・失敗観測ループの運用化）**
