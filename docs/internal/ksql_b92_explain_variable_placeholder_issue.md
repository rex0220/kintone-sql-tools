...
- 起票: 2026-07-29
- ステータス: 📋 **R2・実装待ち（優先 高／リリース済みの回帰）**（R1 の受入 2 が実態と合わず codex が実装前に指摘）
...
4. **算術位置では `{ type: "NUMBER", value: 0, raw: "@name" }` として置換する**
   （`raw` は既に表示に使われている。ただし**この計画形では算術式自体を表示しない**ため、
   受入 2 は**内部契約**として固定する）
...
2. **数値化した痕跡（`0`）を漏らさない**（R2 で修正）

   **R1 は「計画の表示が `@name` のまま」と書いたが、実態と合っていなかった。**
   Pro の実例（一時テーブル参照の `SELECT`）の計画は次の 3 行だけで、
   **算術式そのものを表示しない。**
   ...
   → **内部契約として固定する**＝算術位置で置換した数値ノードが
   **`raw` に `@name` を保持し、`0` を漏らさない**こと。

   **加えて、算術式を表示する計画箇所が他にあれば、そこでも `@name` が出ることを固定する。**
   無ければ内部契約だけでよい（**表示契約の新設は行わない**＝patch の範囲を超える）。
