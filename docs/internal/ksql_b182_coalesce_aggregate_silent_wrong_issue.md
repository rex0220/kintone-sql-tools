...
- 状態: 🚧 **codex が実装・Claude レビュー済み（2026-09-16・`b182/dev`・コミット待ち・[報告](ksql_b182_codex_impl_report.md)）**。レビューで回帰 1 件（GROUP BY のみの文の EXPLAIN reason に「集計関数あり」が付く）を修正しテストを追加。実機（SFA）で並び順・RANK・累計・算術値の修正を確認。版数据え置き・リリースは B181・B183 と合わせて判断。実測 v3.77.0（MCP・dev profile・SFA パック）。**エラーも警告も出ない**「静かに間違う」系（B119〜B122 の隣）
