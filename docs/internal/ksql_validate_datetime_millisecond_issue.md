...
- ステータス: **v2.15.0 でリリース済み**（実装・コードレビュー承認・実機 全7項目 pass。実機パリティ＝検証と実書き込みの判定一致を確認）。
...
- [x] 実機パリティ: 検証が通った値は実書き込みでも通る（§1 の実測と一致）。**2026-07-16 確認**: 同じ INSERT で検証 pass → 実書き込みも成功し `2026-07-16T11:21:00Z` へ丸め。
