...
- ステータス: **課題（原因特定済み・実機で確定・修正方針あり）。codex レビュー前・未実装。**
...
- [ ] `2026-07-16T11:21:25.174Z`（ミリ秒付き・`NOW()` の実形式）が DATETIME で**検証を通る**。
- [ ] **`SET @now = NOW(); UPDATE … SET 日時フィールド = @now … VALIDATE ONLY`** がエラーゼロ（R1 の定番パターン）。
- [ ] **`ON ERROR SKIP` で当該行が `#err` へ隔離されない**（＝実書き込みと同じく合格扱い）。
- [ ] 小数部の桁数違い（`.1` / `.174` / `.123456`）をすべて受理。
- [ ] オフセット形式（`2026-07-16T11:21:25.174+09:00`）も受理。
- [ ] **既存の有効形式に回帰なし**（`…T11:21Z` / `…T11:21:25Z` / `2026-07-16 11:21:25` / `2026/07/16` 等）。
- [ ] **無効形式は引き続き拒否**（`2026-13-01T00:00Z`（月13）・`2026-07-16T25:00Z`（時25）・`abc`・小数部のみで時刻不正 `…T99:99:99.1Z`）。
- [ ] `DATE` / `TIME` フィールドの判定に回帰なし（TIME にミリ秒を足しても従来どおり拒否）。
