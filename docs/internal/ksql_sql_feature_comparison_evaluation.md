...
- **コスト: 小**。`evalAggregate`（process.ts:280）に集約関数を1つ足すだけ。区切り文字・`ORDER BY` 内包・`DISTINCT` は既存の集約基盤に乗る。返り値 `number|string` 化は **v2.14.0 で完了済み**＝**下地ができている**。
- **適合性: 高**。
- **推奨: 高**。B14（temp 列型メタ）と組み合わせると B12 の看板レシピが本来の形で成立する。**T1-1 より安い**ので先に入れる価値あり。
...
- **B14（temp/CTE 列の型メタ伝播）は上記 1 の前提**。`#err` は一時テーブルなので、`GROUP_CONCAT` を入れても B14 が無ければ B12 レシピは完成しない。**B14 → GROUP_CONCAT の順**が正しい。
