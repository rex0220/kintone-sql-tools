...
`"field code" / "label" / "type"` の 3 語は**残す**。**新しい 4 フラグの語も 4 か所すべてに足す。**

### 3.4 文書

- `docs/ksql_language_reference.md` の **§13（`:2099-2109`）と §14（`:2163-2175`）の両方**を直す
  （現状の `fieldCode`/`label`/`type`・`name` は**誤り**）。新 4 列も表に足す
...
9. **UNION の列数不一致で既存挙動が変わらないこと**（→ B137）。
   **R2 までここに「検出される」と書いていたのは仕様の誤り**＝現行は右辺の不足を空文字で補い
   余りを黙って捨てる。**それは直すべき欠陥だが B130 の範囲ではない**ので、
   ここでは**現状挙動を固定するだけ**にする（実装時に codex がエンジンを変更したため差し戻した）
10. **JOIN の同名列**で修飾参照 `d.ルックアップ` が正しく引ける
11. **BYO client がフラグを返さないとき、4 列とも `""` で落ちない**（H2 の fallback）

### 4.3 実機

型だけでは区別できなかった `製品名`（ルックアップ）と `仕入先`（素の文字列）が
**describe だけで見分けられる**ことを確認する。
**`仕入先` がルックアップのコピー先であることも `コピー元` で見えるようになる**（R3 で追加）。
**それでも `required` / 長さ制約などは見えない**ので、§1.2 のとおり完全な判定材料ではない。
