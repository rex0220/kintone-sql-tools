...
<!-- 状態: 公開済み（v3.77.0 で実測）。2026-09-16 に B184（v3.81.0）・B190（v3.83.0）の版注記を本文へ反映（断定文をその場で書き換え・公開時の実測は残す）。Qiita 側の反映は user。反映箇所の一覧は 公開済み版注記_v3.81.0.md -->

> **結論（3 行）**
>
> - 順位・構成比・累積構成比・ABC 区分は、kSQL でもウィンドウ関数（`RANK` / `SUM() OVER ()`）で 1 文に書けます
> - **v3.81.0 以降はウィンドウ関数を集計と同じ SELECT に書け、結果を同じ SELECT の式にも使えます**（`CASE` 条件の中は元が CTE なら v3.83.0 以降）。公開時の v3.77.0 では書けず、「集計する段 → ウィンドウを列に出す段 → その列で割る・区分する段」の 3 段の CTE にしていました。段ごとに値を確かめたいときは今もこの形です
...
<!-- 実測（dev・v3.83.0 MCP・2026-09-16）: 下の 3 段版と同じ 10 行・警告なし。集計を base に分けて次の SELECT の CASE 条件に SUM(売上合計) OVER () = 0 を書く形は v3.81.0〜v3.82.0 で unknown field code(s): __ksql_window_0 で落ち、v3.83.0 で修正（B190）。上の SQL は CASE 条件にウィンドウを置いていないので v3.81.0 から通る -->
...
<!-- 実測（dev・v3.82.0 MCP・2026-09-16）: 10 行・3 段版と完全一致・警告なし -->
...
<!-- 実測（dev・v3.83.0 MCP・2026-09-16）: 10 行・3 段版と完全一致・警告なし。v3.81.0〜v3.82.0 では CASE 条件の SUM(売上合計) OVER () が unknown field code(s): __ksql_window_0 (base) で落ちた（B190） -->

### ウィンドウ結果と式

第 2 回で見たとおり、`ROUND(SUM(x) OVER (), 1)` や `件数 - LAG(件数) OVER (...)` は v3.77.0 では ParseError でした。v3.81.0 以降は通ります（`CASE` の条件の中に直接書く形は、元が CTE のとき v3.83.0 以降。上の 2 つの SQL がその例です）。
...
`PARTITION BY` を付けずに `SUM(SUM(売上)) OVER ()` と書くと、合計行の分まで分母に入って明細の構成比が半分になります。区画を切るのはそのためです。構成比のように `PARTITION BY GROUPING(...)` を式の中に書く形は v3.84.0 から通ります（v3.81.0〜v3.83.0 では内部エラー。順位のように列として出す形は v3.81.0 から通ります）。

<!-- 実測（dev・2026-09-16）: v3.83.0 MCP では 構成比 列が「internal error: GROUPING() reference was not resolved during B65 planning.」（隠しウィンドウの PARTITION BY にある GROUPING() を B65 の計画が束縛していない＝B191）。B191 修正後の CLI では 明細 10 行（順位 1〜9・構成比 25.3 … 0）+ 合計 1 行（順位 1・構成比 100）、警告なし。
     OVER () だと合計行を含む 11 行が分母になり、サイボウズ商事の構成比は 25.3 でなく 12.7、合計行は 50 になる（仕様どおり） -->
...
上の 1 つ目・2 つ目・7 つ目は v3.81.0（B184）と v3.83.0（B190）で kSQL 側が対応したものです。残りの項目（`ROWS` の明示とタイブレーク、`OVER (...)` と `AS 別名`、完全入力、`WHERE rn = 1`、倍精度、ゼロ除算）は v3.83.0 でも同じです。
