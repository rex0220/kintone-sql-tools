...
| 5 | 安全に一括更新する | 一時テーブルで確保・`ASSERT`・`UPSERT`・`VALIDATE ONLY`・`ON ERROR SKIP`・冪等性 |
| 6 | CSV を IMPORT して検証環境を大きくする | `IMPORT CSV/JSON`（プラグインのファイル選択）・テストデータ増量 |
| 7 | CLI を導入して定期運用に載せる | CSV export（Shift_JIS）・`--format json`・`DECLARE @param`・profile と `LAPP_`・スケジューラ |
