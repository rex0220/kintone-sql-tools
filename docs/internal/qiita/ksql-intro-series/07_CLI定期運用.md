...
前回（[第 6 回: CSV を IMPORT して検証環境を大きくする](https://qiita.com/rex0220/items/301ce2e655461188ba81)）まで、実行はすべてプラグインの画面で行いました。今回は同じ SQL をコマンドラインへ移します。
...
ksql --version   # 執筆時は 3.77.0。npm の最新版が入ります
```

この記事のコマンドと出力は v3.77.0 で取りました。本記事の範囲（設定の優先順位・`--export-csv`・profile と `LAPP_`・`--dry-run` / `--allow-dml`・終了コード）は、その後の版でも変わっていません。
...
トークンはタスクの実行ユーザーの環境変数に置きます。`--var` にも `.sql` にも書きません。タスクスケジューラの「操作」は、プログラムに `cmd.exe`、引数に `/c C:\jobs\ksql\weekly_rank.bat` を指定し、上のバッチファイルを呼ぶ形にします。
