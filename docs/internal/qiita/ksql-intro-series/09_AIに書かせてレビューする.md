...
索引の先頭には `kSQL MCP server version 3.78.0 — the resident process that answered this call` と、**いま応答している常駐プロセスの版**が出ます。CLI の `--version` とは別プロセスなので、食い違うことがあります（落とし穴参照）。
...
同じ内容の要点 8 行を v3.78.0 で MCP の instructions（接続時に AI へ渡る文）に入れましたが、前置きなしで 3 回試した結果は「エンジン修正で結果は正しくなったが、SQL の形は変わらなかった」でした（番外編 4 節）。会話に見えるルールは効き、接続時に渡る文は作法までは変えない。3 回の実測なので傾向としか言えませんが、**作法は依頼文で渡す**のが確実です。以下がその全文です（v3.78.0 版。10・14・15 は番外編で見つかった 2 件の修正後の書き方に直してあります）。
