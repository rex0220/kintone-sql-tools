...
- 状態: ✅ **v3.78.0 でリリース（2026-09-16）**。codex が案 A を実装・Claude レビュー済み（コミット 0580bbc・[報告](ksql_b181_codex_impl_report.md)）。レビューで**混在 JOIN の退行と「静かに通る」経路を修正**（実行時の実体化列解決は完全一致のまま、表記ゆれは実行前の束縛だけで吸収。物理フィールドと同名なら物理が勝つ）。全体テスト 299 suites / 6,497 + 26 通過、実機で #1・#3・#4・#7 成功・#8 と物理負例は不変。起票時の実測 v3.77.0（MCP・dev profile・SFA パック）
...
- 関連: §9.14「EXPLAIN が SELECT 列の存在を検査しない」（[B185](ksql_b185_explain_select_column_existence_issue.md) として起票・2026-09-16）。本件は列が「無い」のではなく「名前が変わっている」ケースで、検出経路は同じ
