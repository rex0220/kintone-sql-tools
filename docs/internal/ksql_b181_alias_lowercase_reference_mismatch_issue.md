...
- 状態: 🚧 **codex が案 A を実装・Claude レビュー済み（2026-09-16・`b181/dev`・コミット待ち・[報告](ksql_b181_codex_impl_report.md)）**。レビューで**混在 JOIN の退行と「静かに通る」経路を修正**（実行時の実体化列解決は完全一致のまま、表記ゆれは実行前の束縛だけで吸収。物理フィールドと同名なら物理が勝つ）。全体テスト 299 suites / 6,497 + 26 通過、実機で #1・#3・#4・#7 成功・#8 と物理負例は不変。起票時の実測 v3.77.0（MCP・dev profile・SFA パック）
