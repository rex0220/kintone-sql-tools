...
| 区分 | 内容 |
|---|---|
| **B126** | 単一値の選択系 **`=` / `!=`（`<>` 含む）** を、**リテラルが実在する非空の選択肢のときだけ** `IN` / `NOT IN` へ正規化して押し下げる |
| **B127** | 集計ウィンドウの既定フレーム `RANGE` を `warnings` で知らせる |
| **非対象** | 複数値型／実在しない選択肢／空文字 |
| **非対象** | B126 の警告（正規化で問題が消えるため**出さない**） |

> **【R4 の内部矛盾を訂正 2026-08-05】** この表は R3 のまま「非対象: `!=` → `not in`」を
> 残していた。R4 冒頭の訂正・§2.1・§2.5・§4.1 はいずれも `!=` を**対象**としており、
> **この行だけが取り残されていた**。実装時に codex が矛盾を検出し、多数派（`!=` は対象）を
> 採って実装したうえで報告してきた。**自分で決めずに報告するという依頼の指示どおりの判断。**
