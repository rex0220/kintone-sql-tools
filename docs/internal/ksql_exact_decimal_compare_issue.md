...
APP4221は12桁設定なので、この境界確認には使用できない。実測未完でも、公式上限が30桁であるためB9の必要性は変わらない。

## 8. リリース判断

- B26とは分離する。B26完了時も「大精度typed numberはB9完了までRESTと完全一致しない」という制限を残す（v3.0.0 移行ガイド・横断仕様 §7 に既知制限として明記済み）
- B9は比較意味論の変更なので、単なる性能改善として扱わない
- 優先度: **中**（R5 で高から降格）

### 8.1 R5: 優先度降格の根拠（頻度）と再昇格トリガー

**降格は頻度の判断であり、R4 の事実認定（16桁級は仕様上の対象）の撤回ではない。**両立する2つの主張を区別する:

- R4 が撤回したのは「kintone は16桁級を**扱えない**ので対象外」という**能力**の主張（誤り＝digits は最大30）
- R5 の降格根拠は「16桁超を**実際に使う**アプリが少ない」という**頻度**の主張

頻度と実害の評価:

- 衝突が始まるのは 2^53 = 9,007,199,254,740,992（16桁）超。現実の金額・数量は届かない（日本の国家予算 ≈ 1.1e14 円 = 15桁）
- `Number()` の丸めは**単調**なので、**順序の逆転は原理的に起きず、実害は「偽の同値」だけ**（衝突域内の `WHERE =` 誤マッチ・境界比較・MIN/MAX の代表値）
- 16桁超が現実に入るのは金額でなく ID 的数値であり、先頭ゼロ問題からも本来テキストフィールドが適切
- 最も一般的な大きい整数 `$id` / `RECORD_NUMBER` は v3.0.0 で既に厳密比較済み（§5.2.1・binary64 不使用）

追加スコープ（R5 で判明）: 算術由来の temp 値は `String(1e21)` = `"1e+21"` の**指数表記**になり得る。現行バンドは `Number()` 経由で偶然正しく扱うが、B9 の文字列ベース厳密比較は**科学記法の正規化**を仕様に含める必要がある。

**再昇格トリガー**（いずれかで高へ戻す）:

1. 16桁超の実データでの誤動作報告（偽の同値による `WHERE =` 誤マッチ・MIN/MAX の誤代表）
2. B29（同じ `numberPrecision` 領域の検証・量子化）の実装着手 — 同領域を触るなら同時が安価
