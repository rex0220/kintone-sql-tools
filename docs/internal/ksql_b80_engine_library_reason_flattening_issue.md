...
- ステータス: ✅ **リリース済み（v3.27.0・2026-07-27）**。`statementGuard.ts` が `KlikeValidationError` を class identity で allowlist し、`code = PARSE_ERROR` と `cause` の identity を維持したまま具体的な reason を返す。B76 §17 の parity 緩和も撤回済み。**実機 PASS**＝MCP v3.27.0 で KLIKE を含む OR が具体的な reason で拒否。
