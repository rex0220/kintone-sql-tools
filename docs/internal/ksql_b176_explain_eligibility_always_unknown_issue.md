...
## 9. 【外部確認】本実行へ漏らしていないことが依頼元の実ジョブで裏付けられた（2026-08-25）

**v3.74.0 の CHANGELOG と依頼元への通知で「変わるのは `EXPLAIN` だけ。本実行と `previewStatement` は不変」と書いた。** これは**自分で書いたテストで自分の主張を確かめる**形になっており、同じ思い込みの穴を共有し得る種類の主張だった（**まさに §8.1 で「壊れた挙動がテストで固定されていた」と分かったばかり**）。

**ksql-flow が F-7 の受入実測で、独立のカウンタ（`env.http.snapshot()`）を使って実ジョブを 3 条件で測った**（[切り分け結果](../../../ksql-flow/docs/kSQLエンジンへの返信-20260825-F7-プラス1切り分け.md)）。

| 条件 | API 合計 | GET `app/form/fields.json` |
|---|---|---|
| **v3.72.0**（B173/B176 の前） | 18 | **3** |
| v3.74.0・native OFF | 18 | **3** |
| v3.74.0・native ON | 14 | **3** |

→ **本実行のメタデータ呼び出しは v3.72.0 と v3.74.0 で一致**。**B176 の +1 は EXPLAIN 経路に閉じている**ことが、**別実装のカウンタで外から確認された**。

**副産物＝native の削減量が残差ゼロで確定した。** native ON / OFF の差は**ちょうど 4 本**で、事前 GET の `IN` チャンク `ceil(200 / 50) = 4` と完全一致。**モデルに未説明の項が無い。**（依頼元の当初報告は 8/23 の基準値と比べて -3 で 1 本が未説明だったが、**基準値がランナー v0.2.0・実測が v0.4.0 で、比較の両辺でランナー版が違っていた**ことが原因と判明。**エンジンとは無関係**）

**この経緯の教訓**＝「変えた範囲はここに閉じている」という主張は、**書いた本人のテストでは強く確かめられない**。**別の実装・別の計測器・別の目的で動いている観測**が当たれば、それは拾う価値がある。今回は依頼元が**自分の +1 を追うために**測った表が、こちらの影響範囲の裏付けになった。

**なお、こちらが立てた原因仮説（`explainScript` が +1 の出どころ）は外れだった。** 規模非依存の +1 という形だけを見て、**依頼元の実行経路が EXPLAIN を通るかを確かめずに**候補を挙げた。**当たったのは仮説ではなく切り分けの設計**（エンジン版を固定して native フラグだけ動かす）で、**仮説が外れても native の無実は確定する形**になっていた。
