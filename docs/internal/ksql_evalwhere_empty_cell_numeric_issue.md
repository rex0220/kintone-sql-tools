...
- 作成日: 2026-07-15
- ステータス: **実機確認済（v2.1.2）・修正方針を −∞ 準拠へ確定（案a は棄却）。空セル意味論の修正と数値プレフィルタ全体の安全性は分離する**
- 更新履歴:
  - 2026-07-15 R1: 起票
...
> **追加確認（モデルの固定・実装前の推奨ゲート）**: 4点は「空 <= -1」でも矛盾しないため、極端な閾値で範囲を広げる: `金額 <= -1000000`（−∞ なら**含む**）／`金額 >= -1000000`（−∞ なら**除外**）。可能なら実データで扱えるさらに小さい値をもう1段。これは**数学的な −∞ を証明するものではない**ため、結論は「**kintone で確認した数値範囲内では、空値を −∞ として扱うモデルと一致する**」と表現する。`BETWEEN`（例 `金額 BETWEEN 0 AND 100` ＝ `>=0 AND <=100`）は下限 `>=` 側で空を除外する想定。
...
実機確認済。修正方針は **値ベースの −∞ 準拠**で確定（案a 棄却）。**本課題は「空セル包含性の解決（SIMPLE/FULL_SCAN 一致）」に限定**し、数値プレフィルタ全体の安全性（IEEE-754 精度・数値表現）は③で別途扱う。次は **極端閾値（`<= -1000000` / `>= -1000000`）で −∞ モデルを実装前に最終確認 → 仕様案（−∞ 実装・`evalWhere`/`compareAssertValues` 共通化・波及回帰）→ 実装（分担どおり）**。
