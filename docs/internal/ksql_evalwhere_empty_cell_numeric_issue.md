...
> 補足（別件・要確認）: FULL_SCAN の 0 件 SELECT（`金額 <= -1000000 AND …`）が `columns: []` を返した。明示列なので v2.1.1 修正後は列を返すはずで、**この MCP サーバが v2.1.1 以前のビルドの可能性**（`$id LIKE` が kintone に押し下がりエラーになった挙動も pre-v2.0.0 を示唆）。本課題とは独立だが、MCP サーバの更新確認を別途推奨。
...
2. **修正方針（案a は棄却）＝値ベースの −∞ 規則（[P1-2]・コードレベル確定）**: `evalWhere`（[:118](../../src/engine/evalWhere.ts#L118)）は**左辺のフィールド型を知らない**（`leftStr`/`rightStr` のみ）。よって「数値フィールドだけ −∞」は判定不能。**左辺型ではなく解決値で**定義する:
   ...
   > **左辺解決値が空文字（`leftStr === ""`）で、右辺解決値が空でなく数値化可能（`rightStr !== "" && !Number.isNaN(Number(rightStr))`）な範囲比較** のとき、左辺を −∞ として扱う。

   ```ts
   const emptyLeftNumericContext =
     leftStr === "" && rightStr !== "" && !Number.isNaN(Number(rightStr));
   if (emptyLeftNumericContext) {
     // ">" → false / ">=" → false / "<" → true / "<=" → true
   }
   ```
   - この規則は**空のテキストフィールドでも右辺が数値化可能なら −∞ 扱い**になる（現行の動的数値判定に沿う。「フィールド型として数値限定」ではない点を明記）。
   ...
   - **文字列比較文脈**（右辺が空 or 非数値）は**不変**。−∞ 扱いは上記の値ベース条件に限定する。
...
- **範囲比較で `leftStr==="" && rightStr!=="" && 数値化可能` のとき左辺を −∞ 扱い**（`>`/`>=`→false、`<`/`<=`→true）。文字列フォールバックはさせない。右辺空・非数値文脈は不変。
...
- **右辺空・文字列比較の現状維持（[P2-5]・共通比較器で固定）**: 左辺空だけを特別扱いする回帰防止として、
  - `"" >= "0"` → **false**、`"" <= "0"` → **true**（左辺空・数値右辺＝新 −∞ 規則）
  - `"-1" > ""` / `"-1" < ""`（左辺非空・右辺空＝現状維持。右辺を −∞ に巻き込まない）
  - `"" >= ""`（左右空＝現状維持）、`"A" > ""`（文字列比較＝現状維持）
  を固定し、右辺空や文字列比較を意図せず −∞ 規則へ巻き込まないことを担保する。
