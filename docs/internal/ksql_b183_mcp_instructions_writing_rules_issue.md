...
- 状態: 🚧 **codex が実装・Claude レビュー済み（2026-09-16・`b183/dev`・コミット待ち・[報告](ksql_b183_codex_impl_report.md)）**。Writing rules 8 行を追加（B181・B182 修正後の仕様で記述）。レビューで除数ガードを §5 の `= '' OR = 0` 形へ、JOIN の絞り込みを §7 の「INNER JOIN のみ」へ揃え、予算テストは語数 exact `{935, 347, 588}`・上限 prose ≤ 635 / total ≤ 1,015 / 段落 7。実測 initialize 応答 5,722 → 7,369 文字（+29%・固定コンテキスト全体 +4.5%）。効果（§3 の before/after 3 回）はリリース後に実測してから確定する。純加法（エンジン不変）
