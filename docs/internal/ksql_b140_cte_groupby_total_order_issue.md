# B140 CTE の `GROUP BY` キーを全順序として証明できず、主用途で毎回警告が出る

- 起票: 2026-08-06
- ステータス: ❌ **クローズ（実装しない）**（2026-08-07・オーナー判断・§9）＝**依頼元が「実装は不要。文言で解けている」と回答**。
  **仕様 R2 は完成しレビュー済み（実装着手可）だが、実需が無いので出さない**（→ [R2](ksql_b140_cte_groupby_total_order_spec_r2.md)）。
  **文言をもう 1 段足す案は [R2 §11](ksql_b140_cte_groupby_total_order_spec_r2.md) が解けたら**（§8.6）。
  **R1 は破棄**＝
  **[B148](ksql_b148_bare_column_group_by_issue.md)（v3.57.0）で中核の identity が実装された**（§7）
- 発見: [B128](ksql_b128_lag_lead_spec.md)（`LAG` / `LEAD`）の実機検算中
- 関連: [B127](../ksql_issue_tracker.md)（ウィンドウ既定フレームの警告・v3.47.0）

---

## 1. 事実

**B128 の主用途——依頼元の実需そのもの——で毎回警告が出る。**

```sql
WITH 月次 AS (
  SELECT DATE_FORMAT(日付, '%Y-%m') AS 年月, SUM(個数) AS 出庫数
  FROM APP4228 WHERE 入出庫区分 = '出庫' GROUP BY 年月
), 前月付き AS (
  SELECT 年月, 出庫数, LAG(出庫数) OVER (ORDER BY 年月) AS 前月 FROM 月次
)
SELECT ... FROM 前月付き
```

```
warnings: ["前月 の ORDER BY は全順序でないため、同順内の前後関係は未規定です。
           レコード番号等を ORDER BY に追加してください。"]
```

**しかし `年月` は CTE の `GROUP BY` キーなので、構造上 1 行 1 値で一意。**
**同順の行は存在し得ない。** つまりこの警告は**偽陽性**である。

### 判定器自体は正しく動いている

**direct APP で `レコード番号` を含む場合は抑止される**（実測・v3.51.0 相当）。

```
SELECT レコード番号, LAG(仕入価格) OVER (ORDER BY レコード番号) AS 前 FROM APP4229 ORDER BY レコード番号
→ warnings: []
```

**問題は「CTE 由来の列を証明できない」ことだけ**で、判定器の欠陥ではない。
`canProveTotalWindowOrder` は物理フィールドの `$id` / `RECORD_NUMBER` しか根拠にできない。

---

## 2. なぜ重要か

**警告疲れを起こす。**

- B127（ウィンドウ既定フレーム）は「**実害があるときだけ警告する**」設計で、
  **`ORDER BY` が全順序と静的に判定できるときは出さない**ようにしてある
- 同じ判定器を共有した結果、**B128 の主用途が毎回警告される**
- **主用途で毎回出る警告は読み飛ばされる。** そうなると **B127 の警告まで効かなくなる**

**正しさには影響しない**（値は正しい）。**警告の質の問題。**

---

## 3. 証明できるはずの根拠

**CTE の外側 `SELECT` が `GROUP BY k` を持ち、下流の `ORDER BY` がちょうど `k` なら、
その CTE の行は `k` について一意。** これは静的に判定できる。

| 形 | 一意か | 現在 |
|---|---|---|
| `WITH m AS (... GROUP BY k) SELECT ... OVER (ORDER BY k) FROM m` | **一意** | 警告が出る |
| `GROUP BY k1, k2` に対し `ORDER BY k1, k2` | **一意** | 警告が出る |
| `GROUP BY k1, k2` に対し `ORDER BY k1` だけ | 一意でない | 警告が出る（正しい） |
| `DISTINCT` を経た列 | 一意 | 警告が出る |

**materialized ソースの列メタは既に保持されている**（`execute.ts` の CTE / 一時テーブル経路）ので、
**「この列は GROUP BY キーだった」という情報を運べるかが論点。**

---

## 4. 対応案

| 案 | 内容 | 見立て |
|---|---|---|
| **A** | **CTE / 一時テーブルの `GROUP BY` キー（と `DISTINCT` 列）を全順序の根拠として証明できるようにする** | **筋が良い**。列メタに「グループキー由来」を持たせる必要がある |
| **B** | CTE 経由のときは**警告を出さない** | 偽陽性は消えるが、**本物の非全順序も見逃す**（B127 の設計思想に反する） |
| **C** | 文言を「証明できなかった」に変える（`未規定` → `判定できませんでした`） | 安いが、**主用途で毎回出ることは変わらない** |
| **D** | 現状のまま | 警告疲れが残る |

**見立て＝案 A。** ただし**列メタの拡張が要る**ので、独立した仕様と codex レビューを経ること。

**案 C は案 A までのつなぎとして併用できる**（文言だけなら安い）。

---

## 5. 未確定

1. **`GROUP BY` キー由来であることを列メタで運べるか**（既存の `columnMeta` / `fieldSemantics` に
   足すのか、別の経路か）
2. **一時テーブル経由**でも同じ証明ができるか
3. `DISTINCT` 由来の一意性まで含めるか
4. **B127 側にも同じ偽陽性があるか**（集計ウィンドウを CTE の上で使う形）。
   **あるなら本件は B127 の穴でもある**

---

## 7. **【2026-08-07】B148 の実装で中核が解けた**

**[B148](ksql_b148_bare_column_group_by_issue.md)（v3.57.0）で、B140 が必要としていた
「どの出力が `GROUP BY` キーか」の identity が実装された。**

```ts
// src/core/aggregateDependencyValidation.ts
export function buildOrdinaryDependencyPolicy(stmt, plan, schemas): {
  readonly identities: ReadonlySet<string>;   // ← GROUP BY キーの canonical identity 集合
  ...
}
```

**`plainGroupByPlan` の解決種別ごとに identity を作る。**

| plan | identity |
|---|---|
| `PHYSICAL` | `FIELD:source:<index>:<fieldCode>` |
| `ALIAS_SAFE` | **`columnIndex` の SELECT 式**を canonicalize |
| `EXPRESSION` | 対応する `GroupByKey` の式を canonicalize |

**これは [B140 の仕様 R1](ksql_b140_cte_groupby_total_order_spec.md) が
「名前一致で取る」と書いて崩れた箇所そのもの。**
**同じ罠を B148 の R1・R2 でも踏み、3 版目で解いた。**

### 7.1 残っている作業（**R1 の想定より小さい**）

1. **identity → CTE の出力列の対応**。
   **`ALIAS_SAFE` は `columnIndex` を持つので出力列が直に分かる。**
   `PHYSICAL` は fieldCode、`EXPRESSION` は出力列側の式と canonical 比較が要る
2. **実体化した relation へ候補キーを運ぶ**（[codex の対応案](ksql_b140_codex_design_1.md) 案 A）。
   **`mergeSelectWarnings` が clone 時に列メタしか引き継がない**ので、そこも要る
3. **consumer 側で、ウィンドウの `ORDER BY` が候補キーを包含するか**を見る

**B148 の canonical 一致は実測で 8 形の表記ゆれを通している**
（`1`／`1.0`・修飾／非修飾の両方向・余分な括弧・関数別名・連結・リテラル・空白）ので、
**B140 でもそのまま使える見込み。**

### 7.2 Phase 1 の線引きへの影響

**[codex の対応案](ksql_b140_codex_design_1.md) は「Phase 1-min は `ALIAS_SAFE` だけ」と提案した。**
**その提案は B148 実装の前に出たもの**である。

**いまは `PHYSICAL` / `EXPRESSION` の identity も存在する**ので、
**`ALIAS_SAFE` 限定は「identity が無いから」ではなく
「出力列への対応づけが直に取れるから」という理由に変わる。**
**Phase 1 の範囲は仕様で決め直す。**

### 7.3 依頼元への照会

**[ウィンドウ警告の扱いを聞く照会](../../../ksql-analytics/docs/internal/kSQLエンジンからの照会-20260806-B140警告の扱い.md)
を先方の文書に置いてある**（A〜E の選択肢）。**回答は打ち手の優先度を変えるが、方向は変えない**
（どの回答でも案 A が本命）。**待たずに仕様を進めてよい。**

---

## 8. **【2026-08-07】依頼元の回答で方向が変わった**

**[照会への回答](../../../ksql-analytics/docs/internal/kSQLエンジンへの返信-20260806-B140警告の扱い.md)。**

### 8.1 回答

**B（読んで、問題ないと判断して無視している）。ただし C（読んでいない）に接近。**

**正確には「一度判断して SQL のヘッダに書き、以後は読み飛ばす」**運用で、
**該当資産 2 本にその旨が明記されている。**

> **「この警告は出るものだ」と書いてしまった以上、
> 隣に別の警告が出ても同じ 1 行として流れる可能性があります。**

**警告疲れの心配は、依頼元でも現実の形になっている。**

### 8.2 **A は物理的に不可能だった**

**助言どおり `レコード番号` を足すと落ちる**（依頼元の実測）。

```
SELECT 年月, LAG(出庫数) OVER (ORDER BY 年月, レコード番号) FROM 月次
  → ArgumentError: unknown field code(s): レコード番号 (月次)
```

**CTE の上に `レコード番号` は存在しない。**
**したがって「不要な列を足させ、並びまで変えさせている」という
こちらの心配（照会 §3 の A 行）は該当しなかった。**

**v3.52.0 で文面から `レコード番号` の名指しを外したのが効いている**
（現在は「その表の中で一意になる列（元の集約のキーなど）」）。

### 8.3 **「ちゃんと直す」は不要、と依頼元は言っている**

> **v3.53.0 で入れていただいた 3 文目が、すでに問題を解いています。**

```
集約結果の列は一意でも証明できないため、すでに一意な場合もこの警告が出ます。
```

| | 依頼元の状態 |
|---|---|
| v3.52.0 まで | **消し方を探す**（見つからない）。**判断が保留のまま毎回引っかかる** |
| **v3.53.0 以降** | **「消せない警告」と分類できる。**ヘッダに 1 行書いて、以後は迷わない |

> **当方が困っていたのは「警告が出ること」ではなく「消し方が分からないこと」でした。**
> **それは文言で解けており、実装で解く必要はありません。**

**既定フレーム（`RANGE`）の警告も、依頼元では 1 度も出ていない**——
**`ROWS` を明示する規約**にしたため。**「隣の警告が効かなくなる」は別の形で回避されている。**

### 8.4 **こちらの見立ての誤り**

**照会を出すとき「回答は優先度を変えるが方向は変えない（どの回答でも案 A が本命）」と書いた。誤りだった。**

**B という回答と「文言で解けている」という説明が、案 A の前提そのものを崩した。**
**測る前に「どう転んでも実装する」と決めていた**ことになる。

### 8.5 依頼元の提案＝**文言をもう 1 段**

```
現状  集約結果の列は一意でも証明できないため、すでに一意な場合もこの警告が出ます。
案    …出ます。集約キーで並べているなら、この警告は無視して構いません。
```

> **「無視してよい条件」を書けるなら、B が安定します**（C に落ちる理由が消えます）。
> **書けないなら現状のままで十分です。**

### 8.6 **その文言が書けるかは、[R2](ksql_b140_cte_groupby_total_order_spec_r2.md) §11 に懸かる**

**条件は [R2 §4](ksql_b140_cte_groupby_total_order_spec_r2.md) が正確に定義している**＝
**`ORDER BY` が集約キーを「すべて」含むこと**（一部では足りない）。
**仕様は無駄にならず、文言の条件を与えた。**

**しかし [R2 §11](ksql_b140_cte_groupby_total_order_spec_r2.md) の穴が、文言にも効く。**

**実測（2026-08-07・v3.57.0）**

| 列の型 | comparator | grouping との一致 |
|---|---|---|
| **文字列** | **コードポイント順**（`'10'` → `'2'`／`'1'` と `'01'` は同順にならない） | **一致する** |
| **数値** | **数値比較**（`GROUP BY 個数` の CTE 列は `19` の次が `20`。データに `193` があるのに来ない） | **一致しない可能性** |

**数値列で「異なる格納値が数値的に等しい」ペアがあると、
候補キーは一意でも comparator が同順にする。**
**代表例＝空セルと `0`**（`Number("") === 0`。[別件で既知](ksql_b141_doc_sql_unverified_issue.md)）。

**未確認**＝**`NUMBER` 項目に空セルと `0` が混在するアプリが手元に無い**。
APP4228／APP4233 とも空も 0 も 0 件。**engine ライブラリ直呼びも client 形が違い、手早くは測れなかった。**
**実装時の単体テスト（モック）で確かめるのが素直。**

### 8.7 方針（**2026-08-07 時点**）

| | 判断 |
|---|---|
| **実装（案 A・候補キー）** | **保留。**依頼元が不要と明言し、**唯一の既知利用者に実需が無い** |
| **文言（依頼元の提案）** | **§11 が解けたら出す。**条件は「**集約キーをすべて含む**」 |
| **[R2](ksql_b140_cte_groupby_total_order_spec_r2.md)** | **保存する。**実装しないが、**文言の条件と `$id` との切り分けを定義している** |

**再開の合図**＝**別の利用者が同じ偽陽性で詰まったとき**、
または**「無視してよい」と書いたのに無視できない形が出たとき**。

### 8.8 返信（2026-08-07・送付済み）

→ [B140 照会への回答](../../../ksql-analytics/docs/internal/kSQLエンジンからの連絡-20260807-B140照会への回答.md)

**書いたこと**＝①実装は保留（そちらの判断に従う）②`レコード番号` が足せない件は
**v3.52.0 で名指しを外していなければ、いまも「足せない列を足せ」と言い続けていた**
③**照会の出し方が誤っていた**（答えを聞く前に「どう転んでも実装する」と決めていた）
④文言は**「集約キーをすべて含めているなら」**とするが、**数値列の空セルと `0` が未確認**
なので、**確かめられなければ出さない**。**そちらの資産に該当形が無いか任意で照会**。

**依頼元の設計がこちらより先を行っていた点も書いた**＝
こちらは「**警告の信用を保つ**」方向（だから偽陽性を消そうとした）だが、
**依頼元は「警告の信用に頼らない」**（0 行という別のトリガーで読み直す）。
**後者のほうが壊れにくい**——**警告は出す側の都合で増減するが、
「0 行」は結果の性質なのでこちらの版に依存しない。**


---

## 9. クローズ（2026-08-07・オーナー判断）

**却下。候補キーによる証明は実装しない。**

**問題は文言で解けている。**
**v3.53.0 の 3 文目**（「すでに一意な場合もこの警告が出ます」）**が「消せない警告」と分類できる状態を作り、
**v3.58.0 の追記**（「元の集約のキーをすべて `ORDER BY` に含めているなら、この警告は無視して構いません」）
**で「無視してよい条件」まで書いた。** 依頼元が求めたのはここまでである。

**[仕様 R2](ksql_b140_cte_groupby_total_order_spec_r2.md) は保存する**＝
**文言の条件（「すべて」含む）を定義しており、`$id` 関数従属との切り分けも書いてある。**
**[B146](ksql_b146_docs_version_lookup_cost_issue.md) と同じ扱い**で、**再起票の出発点にする。**

**再起票の条件は §8.7 のとおり。**
