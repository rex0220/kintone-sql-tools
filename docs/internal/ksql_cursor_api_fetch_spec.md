...
| 有効カーソル | 1ドメイン最大10個 | kSQLは既定2、設定しても最大5 |
...
### 制限5: Cursorの複数ページ順序は公式明文だけでは確定していない
...
- 回避策: 500件境界に同値群を置く実機gateを通過するまでB33を出荷しない。決定的順序が必要な利用者には `$id` の明示的な最終キーを案内する
