...
- 起票: 2026-08-02
- ステータス: ✅ **完了（v3.39.0 でリリース）**（2026-08-02・3 面で実測確認）＝仕様は §4
...
### 4.1bis 束縛が無い面との差（2026-08-02 追記・仕様の穴を実装中に発見）

**復元は「束縛がある面」だけで働くため、括弧つき内部 ID を落とすと面どうしで表示が分かれる。**

| 面 | 出力 |
|---|---|
| CLI / MCP（profile 束縛あり）・ライブラリ（`logicalApps` あり） | `app: APP76800@dev AS a`（**括弧を落とす**） |
| プラグイン（browser）・ライブラリ（`logicalApps` なし） | `app: APP76800 AS a (76800)`（復元経路を通らない） |

**この差は許容する。** 括弧内は常に同じ ID の重複で、束縛が無い面には**論理アプリが存在しない
＝内部仮想 ID の露出も起こり得ない**（本件の実害は束縛のある面だけで発生する）。
計画本文の組み立て自体を変えると、プラグインの出力と多数の生プランの期待値へ波及するため、
**リリース終盤の今回は復元側に限定する**（`restoreSqlDiagnosticValue` 本体だけ）。

- `b76JoinPushdownStep5.test.ts` は**面どうしの一致テスト**（固定期待値ではない）。
  同テストは既に `@profile` を落として面差を吸収しているため、**同じ流儀で
  「別名つき `APP<n>` の直後に続く `(<同じ n>)`」だけを正規化**する。他の PLAN_FACTS 行・
  比較対象の面・`normalizeProfile` は変えない
- **将来の選択肢**（今回はやらない）: 計画本文から冗長な `(<同じ n>)` を出さないようにすれば
  全面が揃う。B112 の再発防止としてではなく、独立した整理として検討する
