...
## 14. 方針判断（2026-08-25）— **R1 はこのまま実装候補にしない**

[codex の推奨](ksql_b175_decision_codex.md)（自分が書いた R1 を守る立場に立たないよう明示して依頼）と、Claude のレビュー（§13）が**同じ結論**に達した。

**推奨＝案 1「今は実装せず、案 A（文書）で止める」。** ただし理由の重みが 1 つ増えた。

### 14.1 決定的だったのは費用ではなく**対象範囲の誤り**

R1 は「危険なのは後続の**読取文**だけ」を前提にしている（§3.1・§3.6）。この前提は**誤り**だった。

- **`KLIKE` は v3.10.0（B5）で親レコード DML に解禁済み**＝通常（APPLY なし）の親 `UPDATE` / `DELETE` の WHERE と、APPLY 複数親 `UPDATE` の親 WHERE で使える（[klikeValidation.ts:79-103](../../src/core/klikeValidation.ts#L79)・`src/core/__tests__/klikeValidation.test.ts:64-71` が許可で固定）
- したがって危険形には **`UPDATE / DELETE ... WHERE ... KLIKE ...`** が含まれる。親 DML は **WHERE 全体を kintone クエリへ exact 変換して対象を解決する**ので、**§1 で測ったのと同じ索引経路**を通る
- 結果は読取の件数ズレではなく、**①対象を取りこぼして更新／削除しそこねる ②旧値に当たって本来対象外のレコードを更新／削除する**（旧値ヒットは 4/8）

**R1 をそのまま実装すると、助言機能の側に「最も重い形を見落とす」欠陥を固定してしまう。**

**なぜ両者とも最初に見落としたか**＝[`ksql_batch_recipes.md`](../ksql_batch_recipes.md) に「`KLIKE` は**全 DML で使用不可**」という **v3.10.0 以前のままの記述**が残っており、Claude がそれを影響評価の前提に使い、R1 の依頼文にも「決まっていること」として渡してしまった。**言語リファレンスは正しかった。** → 訂正済み。**[B141](ksql_b141_doc_sql_unverified_issue.md)（挙動を写した記述が古くなる）の実例として、判断そのものを誤らせた初のケース**なので B141 側にも記録する。

### 14.2 案 1 を採る理由（両者の一致点）

1. **露出が小さい**。推奨レシピ [R18](../ksql_batch_recipes.md) は書込が最後の文で、リポジトリ内に「書込 → `KLIKE`」の例は無い。**障害報告もゼロ**（発見は探索由来）
2. **案 A は既に具体的**＝新値の取りこぼし・旧値の偽陽性・第一回避策（書込を最後に）・`LIKE` の `maxRecords` リスクまで書いてある。**追加の EXPLAIN 注記の限界便益は小さい**
3. **EXPLAIN 注記は保護ではない**。実行を止めず、EXPLAIN を通さない利用者には届かず、MCP の単文 EXPLAIN は共通バッチ計画を通らない（`src/mcp/tools.ts:705-730`）
4. **R1 の対象範囲が未完成**（§14.1）

### 14.3 codex による Claude レビュー（§13）への指摘

| Claude の見立て | codex の評価 |
|---|---|
| 費用が 6+6 ファイルへ増えた | **概ね正しいが一部過大**。5 面は既に `buildBatchExplainPlans` を共有しているので、**物理/mapped appId だけなら中核変更は共通側に閉じられる**。テスト 6 面は技術的必須ではなく R1 が選んだ保証水準 |
| `LIKE` 助言が危険 | **中核は正しいが一部過大・かつ古い**。**「`KLIKE` 利用者は定義上大規模アプリにいる」は誤り**＝`KLIKE` は部分一致と異なる意味論を求めて小規模アプリでも使う。また**出荷済み文書は既に修正済み**なので「両方が同じ誤りを含む」は現時点では古い |
| 未文書のベンダー挙動を焼き付ける | **妥当。ただし「恒久的に嘘になる」は過大**（将来削除・改訂はできる） |
| fail-open のまま・保護が部分的 | **正しい。むしろ対象漏れを過小評価していた**（§14.1） |
| B173 同梱で受入が薄まる | **正しい** |

→ **2 点はこちらの過大表現**（「定義上大規模アプリ」「恒久的に嘘」）。**1 点は過小**（対象範囲）。記録として残す。

### 14.4 やらない場合に残る穴を、他に安く塞ぐ手

codex が 4 つ検討して**いずれも不可**と結論（要点のみ・詳細は[推奨文書](ksql_b175_decision_codex.md)）:

- **既存の静的検証に足す**＝`validateKlikeStatement` は単一 `Statement` しか見ず（`src/core/klikeValidation.ts:24-34`）、先行文との順序もフィールド交差も持たない。単純な全面拒否を足すと**実測上無害な形（別フィールド更新・DELETE）まで拒否**する
- **`VALIDATE ONLY`**＝対象取得自体が `KLIKE` 索引に依存するので、ラグによる欠落を正解集合と比較できない
- **既存 EXPLAIN 行へ無条件に追記**＝先行書込のない通常の `KLIKE` にも毎回出て警告疲れになる
- **レシピ**＝R18 は既に書込が最後なので追加変更不要

**唯一の代替**＝該当資産を持つ側（ksql-flow など）の CI で「最初の mutation 以降に `KLIKE` を置かない」保守的 lint。app binding もフォーム定義も不要。**ただし挿入点の有無は未調査**。

### 14.5 判断を覆す条件

次のいずれかで**案 3（B173 とは別リリースで実装）**へ変更する。**B173 同梱には戻さない。**

1. 実資産または障害報告で、書込後 `KLIKE` による取りこぼし・旧値偽陽性・**誤更新・誤削除**が 1 件でも確認された
2. リポジトリまたは推奨レシピに、書込後の `KLIKE` 読取／DML を必要とする実例が入った
3. [B143](ksql_b143_explain_warnings_issue.md) 案 A または `/flow` の `StatementResult.warnings` 配管に着手し、共通警告基盤へ相乗りできる状態になった
4. **先行書込後の `UPDATE / DELETE ... KLIKE / NOT KLIKE` を実機測定し、対象範囲と結果方向を確定できた**
5. 本番規模・高負荷で再現する、またはラグが 3 秒を大きく超える事例が観測された
6. kintone 公式が索引の非同期反映を契約として文書化した
7. 逆に**複数環境で即時同期化が確認された場合は、実装再開ではなく案 A の文書自体を再検証する**

### 14.6 R2 に持ち越す宿題（実装を再開する場合の前提）

- **§3.1 / §3.2 / §3.6 の対象範囲に `KLIKE` を含む親 DML を追加する**（§14.1）
- §13.1 の 3 件（書込文の範囲を実測に合わせて絞る・回避策の順序・「旧値が残る」の追記）
- **先行書込後の `KLIKE` DML と `NOT KLIKE` の実機測定**（未実施）
