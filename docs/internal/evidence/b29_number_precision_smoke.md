...
> **R2 追記（2026-07-18）**: 本証跡の実測（§3 の小数暗黙丸め）を根拠に、**小数部の桁拒否は撤回**（案B・小数素通し）。kintone が REST/CSV/編集画面/計算フィールドの全面で小数を自動丸めするため、kSQL だけが拒否・警告するのは不整合と判断。**整数部超過の事前検出のみ維持**。以下 §3/§4/結論の「`ERR_NUMBER_DECIMAL_PLACES` は保護」という当時の解釈は R2 で無効化され、同じ実測が「素通しの正当化」へ読み替わる。§2（整数部予算 I=12 の確定）は R2 でも有効。
