...
| ② **行位置 `_idx`（読み取り専用システム列・既存・0-based）** | `[_idx = 1]` / `WHERE _idx IN (0, 2)` | **不要**（画面で数えられる） | △ 実行間で行の追加/削除/REORDER があるとズレて**別の行を静かに更新**し得る → **revision ガードで◎に引き上げ可**（下記） | 人間の手書き・単発修復 |
...
**設計方針（提案)**: 行セレクタを**共通文法**（`_rid` / `_idx` / 述語 / `*`）として定義し、採用する構文案（C1 の `[…]`・X1/C3 の子 WHERE・X3 の WHERE）のどれにも同じものを載せる。

**訂正（2026-07-20・codex レビューで判明）**: `_idx` は**既に存在する**（`APPxxx$テーブル` のシステム列・**0-based**・参照専用。言語リファレンス §19・[subtableAdapter.ts:28](../../src/converter/subtableAdapter.ts#L28)）。本節の初稿は「1-based の新設擬似列・SELECT への追加を先行出荷可」と書いたが誤り。B44 で新規なのは **DML の行セレクタとして `_idx`（および述語のみ・WHERE 省略）を許可すること**＝現行サブテーブル UPDATE/DELETE は安全のため **`_rid` 条件必須**（言語リファレンス §19・[execute.ts:5729](../../src/execute.ts#L5729) `hasRidCondition`）であり、述語だけでは書けない。基数は既存契約（0-based）に従う。
