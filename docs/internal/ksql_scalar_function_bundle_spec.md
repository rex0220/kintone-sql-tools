...
**モード判定は `compareScalarValues` の n 項への自然な一般化**であり、別規則ではない。ただし**一致するのは「2 引数がどちらも非空」の場合に限る**（R4 で限定）。

| 条件 | `compareScalarValues` との関係 |
|---|---|
| **非空 2 引数** | **完全に一致**。`compareScalarValues` の `numeric` フラグ（両オペランドが数値）は **2 項における「集合が全て数値」そのもの**。→ `WHERE a > b` との整合が保たれる |
...
- [ ] **非空 2 引数では `WHERE a > b` と結論が一致**する（`compareScalarValues` と同じ規則）。**空文字を含む場合・3 引数以上の混在・数値同値**では本仕様の規則が優先する（§3.1）。
