...
- ステータス: **R8.5承認済み / v3.0.0 公開済み（2026-07-17）**（R8.1でB27のtie-breakとSTATUS訂正、R8.2で`status.json`の`index`型・無効時挙動を確定。R8.3でv3.0.0統合範囲とWHERE/GREATEST/B32を追加。R8.4のNaN sentinel案を、B14 `#err`契約との衝突によりR8.5で訂正。typed numberの正当な域外値を固定末尾バンドへ置き、ペア単位モード切替だけを禁止。**R8.6で§4.1の正規化実測を訂正** — 初出の`か゛`はU+309B＝スペーシング濁点でNFC不動点のため正規化の証拠として不成立（レビュー指摘）。U+3099結合濁点のNFD `が`で再実測し「保存・比較とも正規化しない」を確定）。
...
**対称の補強（R8.6・minLength 側）**: `minLength` = 3（「2文字より長く」）の `文字列MIN` へ素の INSERT で `😀😀`（コードポイント 2・UTF-16 4）が**受理**された。コードポイント・書記素計数なら 2 で拒否されるはずであり、min/max の両側で UTF-16 計数が確定した。あわせて **minLength は INSERT 時の未設定（空文字）フィールドにも発火する**ことを実測（値を渡さない行が `CB_VA01`「2文字より長くなければなりません」で拒否された）。
...
**追実測（R8.6・正規化の証拠を訂正）**: 上記の `か゛`（U+309B）は**スペーシング濁点で NFC 不動点**のため、正規化を行わない証拠にならなかった（`"か゛".normalize("NFC") === "か゛"`）。結合濁点 U+3099 で再実測した:

```
データ: 'が'(U+304C NFC) 'が'(U+304B U+3099 NFD) 'か゛'(U+304B U+309B)   ← APP4221 文字列MAX・単発 GET
kintone が返す順: が(U+304B U+3099), か゛(U+304B U+309B), が(U+304C)     ← コードポイント順
保存時の読み返し: 'が' は LENGTH = 2 のまま（NFC 化されるなら 1）
```

- **共通接頭辞は短い方が先**（`ab` < `abc`）
- **正規化しない（保存・比較とも）**（NFD の `が` = U+304B U+3099 が NFC の `が` = U+304C より先に並び、tie にもならない。保存も NFD のまま）
...
| 物理数値（NUMBER、数値 CALC） | フィールド定義 | B9の厳密10進順 | 最大30桁、指数表記、`-0`、末尾ゼロを単一primitiveで比較。JS算術由来値はbinary64後の文字列 |
| RECORD_NUMBER / `$id` | フィールド定義 | 数字列の厳密整数順 | B9とは別の既存ID契約。binary64不使用 |
...
| §9.1 正規化・結合・IVS（`が`/`が(U+3099)`/`か゛(U+309B)`/`葛`/`葛󠄀`） | **解消**（§4.1・**R8.6 で U+309B→U+3099 を追実測して確定**）。**kintone は保存・比較とも正規化しない** |
...
| §9.1 補助平面どうしの組（`𠮟` vs `𩸽`）・結合文字の連続 | **未**（非 Blocking）。**R8.6 追加観測**: `😀`(U+1F600) < `𠮷`(U+20BB7) の 1 組は実測でコードポイント順と一致。ただし補助平面同士は CU 順＝CP 順のため判別力は限定的＝元の未測定組の解消とはしない |
