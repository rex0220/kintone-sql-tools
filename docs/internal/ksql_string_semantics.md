...
> **B9 は縮小できないどころか、想定より広い。**  の同値性だけでなく、** と  を含む 10 進の意味論全体**が対象になる。

**未測定**:  /  の保存可否。** を 16 以上に設定したアプリ（またはフィールド）が要る**（kSQL に app settings API は無いため Claude は設定変更できない）。**検証用アプリの設定変更をお願いしたい。**
