...
1. kSQL 利用者は **CLI 中心か / プラグイン中心か**（CLI 先行 v1 はプラグイン中心ユーザーに届かない）。
2. **ローカル CSV 取込の実需**があるか（現状は kintone 純正 CSV 取込で足りているか）。
3. 取込で欲しいのは **検証/UPSERT/変換/不良行隔離**のどれか（B37/B12 との合わせ技の需要）。
4. エンコーディングは Shift_JIS 前提か。
...
**提言**：EXPORT は新規機能化しない。**IMPORT は「CLI `--bind-csv` によるソースバインド」から着手**し、MCP（inline）・プラグイン（upload）を v2 へ。着手は §7 の実需確認後。
