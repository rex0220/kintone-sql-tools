...
> **注記(v1.4.0 で更新予定)**: バッチ実行・一時テーブル対応([ksql_batch_temp_table_spec.md](ksql_batch_temp_table_spec.md) §8.2)に伴い、入力規則は5段判定(完結した単文の即実行は従来互換)へ、メタコマンドには `:run`(バッチ構築モードの実行)が追加され、メタコマンドは**バッファ非空でも**解釈されるよう変更予定。本節の記述は v1.3.x 時点のもの。
