...
> **注記**: v1.4.0(予定)でバッチ実行・一時テーブルに対応(判定の詳細は [ksql_batch_temp_table_spec.md](ksql_batch_temp_table_spec.md) §8.2)。単文入力の操作感(`;` 終端で実行)は従来と互換。DML を含むバッチは、REPL 側でバッチ全体の確認(全 DML 文の一覧)を1回行ってから実行される(`--yes` でスキップ)。
