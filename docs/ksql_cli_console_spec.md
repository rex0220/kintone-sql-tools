...
4. `--app <id>`
5. `--diag-record-id <id>`
6. `--max-records <n>`（実際に取得する行数の上限。v2.11.0 以降、`ORDER BY` なし・KLIKE なしの SIMPLE `LIMIT > 500` は `OFFSET + LIMIT <= maxRecords` なら必要件数で正常終了し、一致総数が上限を超えていても上限エラー／truncate 警告を出さない）
...
> **注記**: v1.4.0 でバッチ実行・一時テーブルに対応(判定の詳細は [ksql_batch_temp_table_spec.md](ksql_batch_temp_table_spec.md) §8.2)。単文入力の操作感(`;` 終端で実行)は従来と互換。DML を含むバッチは、REPL 側でバッチ全体の確認(全 DML 文の一覧)を1回行ってから実行される(`--yes` でスキップ)。

1. SQL は `;` で終端して実行する(1行に複文を入力した場合はバッチとして実行)
2. 複数行入力を許可する(未終端の文字列・コメント内の `;` では実行されない)
3. `CREATE TEMP TABLE` で始まる入力は**バッチ構築モード**になり、`;` では実行されず `:run` で実行する
4. メタコマンド(`:` 始まり)は入力バッファが非空でも解釈される
5. 構文エラー(typo 等)は `;` 終端時に即エラーとなり、バッファは破棄される
6. 空行のみは無視する
7. `Ctrl+C` は現在入力のキャンセル
8. `Ctrl+D` または `:exit` で終了
...
| `:run` | 入力バッファ(バッチ構築モードの複文)をバッチとして実行。エラー時(空バッファ・パース未完)はバッファを保持 |
| `:clear` | 入力バッファをクリア |
