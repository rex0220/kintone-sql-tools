...
- `ORDER BY 年月` が重複し得る粒度なら、その表の中で一意になる列を追加します。**CTE には `レコード番号` が無いので、足せるのは集約キーだけ**です。
  **この例は月次集約なので `年月` は 1 行に定まりますが、実行すると全順序の警告が出ます。**エンジンは集約結果の列の一意性を証明できないためで、**値は正しく、この形では警告を消せません**。
