...
> **前提バージョン**: 本レシピはバッチ変数を使うため **v2.1.0 以降**。主な機能の追加版 — バッチ実行・一時テーブル `v1.4.0`／ASSERT `v1.10.0`／プラグインの DML バッチ `v1.9.0`／`tempTableMaxRows` `v1.11.0`／バッチ変数・IN リストの変数 `v2.1.0`。
...
UPDATE APP100
SET 処理ステータス = '未処理'
WHERE 処理ステータス = '処理中' AND 確保日時 < TODAY();

-- 1) 件数ゲート（確保対象の上限。0 件を正常にするなら BETWEEN 0 …、異常検知したいなら BETWEEN 1 …）
ASSERT (SELECT COUNT(*) FROM APP100 WHERE 処理ステータス = '未処理') BETWEEN 0 AND 10000;
...
SELECT 顧客コード, 顧客名 FROM APP100 WHERE バッチID = @now AND 処理ステータス = '処理中';
...
- 0 件のとき `#tgt` は 0 行で、`UPSERT … SELECT FROM #tgt` は書き込み 0 件になります。**空の #tgt に対する UPSERT がエラーにならないことは、少数レコードで一度実機確認**してください（parse では出ない挙動）。
