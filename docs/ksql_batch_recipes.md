...
4. **件数ゲート**: 確保対象の上限は確保の前に、実際に確保できた件数は確保の後に ASSERT する。**v3.72.0 未満のエンジンでは、サブクエリ件数の大小比較・BETWEEN が辞書順になる不具合（B171）があり、本書のゲート例は桁跨ぎで誤動作します。v3.72.0 以上で使用してください（実測・v3.72.0）**。**差分 0 件を正常とするか異常とするかは運用で選ぶ**（下記）。
...
### 日付軸は `GENERATE_SERIES` で作る（v3.59.0・月／年 step は v3.63.0）

kintone に日付ディメンションのアプリはありませんが、**`WITH` の CTE 本体に `GENERATE_SERIES` を書けば、
レコードを 1 件も読まずに日付系列を作れます**（[言語リファレンス §13](ksql_language_reference.md#13-with-句cte)）。
起点をこの系列に置けば、マスタと同じ形で**取引の無い日を 0 として並べられます**。

```sql
WITH 日付系列 AS (
  GENERATE_SERIES('2026-07-01', '2026-07-31', '1 day') AS 日付
),
日別 AS (
  SELECT 日付, SUM(個数) AS 出庫数
  FROM APP4228
  WHERE 入出庫区分 = '出庫' AND 日付 >= '2026-07-01' AND 日付 <= '2026-07-31'
  GROUP BY 日付
)
SELECT s.日付,
       CASE WHEN t.出庫数 = '' THEN 0 ELSE t.出庫数 END AS 出庫数
FROM 日付系列 AS s LEFT JOIN 日別 AS t ON s.日付 = t.日付
ORDER BY s.日付
```

**31 行が返り、出庫の無い 5 日が 0 で並びます**（実測。`GROUP BY` だけなら 26 行で、その 5 日は行ごと消えます）。

- **`LEFT JOIN` で一致しなかった側は 0 ではなく空文字**です。`CASE WHEN … = '' THEN 0` まで書いて初めて 0 埋めになります
- 月次は `GENERATE_SERIES('2025-08-01', '2026-08-01', '1 month') AS 月` で月初系列を作り、
  `DATE_FORMAT(月, '%Y-%m')` を年月キーにして月次集計と結合します（start は月初限定）。
  **空月があると `LAG` は「直前に存在する月」と比べてしまう**（静かに 2 か月前比になる）ので、前月比は 0 埋めしてから取ります
- 集計側を `THIS_MONTH()` などの相対日付で絞りたい場合は、CTE を JOIN の入力にした形では使えないため、
  先に `CREATE TEMP TABLE #日別 AS SELECT … WHERE 日付 = THIS_MONTH() GROUP BY …;` で実体化してから系列と JOIN します
- 「日付 × 製品」のように軸が 2 本あるときは、系列と小さいマスタを `CROSS JOIN` で格子にしてから実績を当てます（v3.63.0・言語リファレンス §7）。
  `ON` は等式 1 本だけなので、両側で `CONCAT(DATE_FORMAT(日付, '%Y-%m-%d'), '|', 製品名)` のような連結キーを作って結合します
- 生成行数は同一 `WITH` 文で合計 10,000 行まで（日次で約 27 年分）。`WHERE` や `LIMIT` を付けても免除されません

> 歴史的経緯: v3.58.0 以前は連番生成が無く、日付アプリを自作するか SQL の外で埋めるしかありませんでした。
> 月次なら歯抜けが起きにくい（実測では 13 か月すべてに行があった）という回避も、今は不要です。
