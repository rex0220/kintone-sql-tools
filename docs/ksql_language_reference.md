...
| 規則 | 内容 |
|---|---|
| 構文 | `SET @名前 = <式>`。式は**リテラル**（文字列・数値）・**関数**（`NOW()` / `TODAY()` / `LOGINUSER()` / 文字列・数値関数）・**数値算術**。`@名前` は英字か `_` で始まり英数字・`_`、64 文字以内。大文字小文字は区別しない |
...
**現時点で非対応（今後のフェーズ）**: スカラーサブクエリ代入（`SET @x = (SELECT ...)`）・外部パラメータ注入（`DECLARE`）・`NULL` の代入・`IN (@a, @b)` のようなリスト展開・関数引数への `NOW()` 直接指定（整形バッチ ID）。
