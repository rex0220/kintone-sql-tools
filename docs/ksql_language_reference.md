...
> **どの除数にガードが要るかは、値の作り方で決まります。**
>
> | 除数の作り方 | 該当が無いときの値 | `= 0` だけ | 必要なガード |
> |---|---|---|---|
> | **集計を通した値**（`SUM` / `COUNT` / `MIN` / `MAX` / `AVG`） | `0` | 捕まえる | `= 0` で足りる |
> | **素の列として渡ってきた値** | 空文字 | **すり抜けて `NaN`** | `= '' OR = 0` |
>
> **集計関数は空文字を返しません。** 対象行が 1 行も無いときも `0` になります（MySQL は `NULL` を返すため、ここは方言差です）。**空文字は「行が無かった」ことから生まれます**——`LEFT JOIN` で一致しなかった行、`LAG` / `LEAD` でパーティションの端、`NULLIF` の結果。**この 3 つを除数にするときだけ `= '' OR = 0` が要ります。**
>
> ```sql
> -- 集計を通しているので = 0 で足りる
