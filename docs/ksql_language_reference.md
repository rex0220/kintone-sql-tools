...
> **`MAX`/`MIN` の比較規則（v2.14.0）**: 実アプリの NUMBER と数値形式 CALC は数値順、テキスト・選択・日時フィールドと文字列形式 CALC は UTF-16 辞書順で比較します。日時の時系列順と一致するのは kintone が返す正規化形式（DATE=`YYYY-MM-DD`、TIME=`HH:mm`、DATETIME/作成日時/更新日時=`...Z`）が前提です。複数値・ユーザー等の対象外型、同名列が競合する JOIN、一時テーブル/CTE 経由の列は型不明として従来の数値経路を使うため、非数値値は `NaN` になります。
...
> **注意**: 上記の `MAX(受注日)` は、`APP300` のフォーム定義から DATE 型を解決して最新日を返します。一時テーブル/CTEから読み直した列には型メタが伝播しないため、`SELECT MAX(受注日) FROM #latest` のような再集約は型不明の従来経路になります（前掲の集計関数の比較規則を参照）。
