...
> **注意（`MAX`/`MIN`）**: 現状 `MAX`/`MIN` は値を数値化して比較するため、**数値フィールド専用**です。テキスト・日時などの非数値列に適用すると `NaN` になります。文字列の辞書順 min/max（および日時の最古/最新）への対応は将来の課題です（`docs/internal/ksql_string_min_max_aggregate_spec.md`）。テキスト列を業務キー単位で 1 行化したい場合は `SELECT DISTINCT キー, '定数' AS フラグ` を使ってください。
...
> **注意**: 上記の `MAX(受注日)` のように**日時・テキスト列**へ `MAX`/`MIN` を適用すると、現状は数値化により `NaN` になります（前掲の集計関数の注意を参照）。日時列の最古/最新・文字列の辞書順対応は将来の課題です。
