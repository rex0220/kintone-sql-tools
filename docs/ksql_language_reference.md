...
**2. 有効桁は 15〜17 桁**（整数は 9,007,199,254,740,992 = 2^53 まで正確）

```sql
SELECT 9007199254740993         -- → 9007199254740992（リテラルは構文解析の時点で丸まる）
SELECT 9007199254740993 + 1     -- → 9007199254740992（+1 が結果に反映されない）
```

16 桁以下の整数の加減算は誤差ゼロです。16 桁を超えると、数値リテラルは**書いた時点で**丸められ、演算でも 1 の位の変化が消えることがあります。
...
> **なお、フィールド値そのものは無傷です。** kSQL は取得した数値を文字列のまま保持するため、算術式を通さない限り 30 桁の値も桁落ちしません（表示・`ORDER BY`・`WHERE` の直接比較で使えます。ただし 17 桁前後を超える数値どうしの大小・等値比較には既知の制限があります — v3.0.0 移行ガイドの既知制限を参照）。
