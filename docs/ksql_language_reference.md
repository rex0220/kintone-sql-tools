...
- GROUP BY なしの集計サブクエリ（`AVG(...)` 等）は対象 0 件でも 1 行を返すためエラーになりません。**戻り値は関数ごとに違います**（`COUNT` / `SUM` / `AVG` は `0`、`MIN` / `MAX` 等は空文字。→ §8「0 件時の挙動」）
- **相関サブクエリは非対応**（外側テーブルのフィールドをサブクエリ内で参照不可）
- 自動的に FULL_SCAN モードで実行されます
...
> **集計する値が 1 つも無いときの戻り値**（実測・v3.54.1）。 `GROUP BY` の無い集計は、対象が 0 行でも 1 行を返します（SQL 標準と同じ）。そのときの値は関数で分かれます。
>
> | 関数 | 値が無いとき | 標準 SQL |
> |---|---|---|
> | `COUNT` | `0` | `0`（一致） |
> | `SUM` / `AVG` | `0` | `NULL` |
> | `MIN` / `MAX` / `MODE` / `MEDIAN` / `GROUP_CONCAT` | **空文字** | `NULL` |
> | `VAR_POP` / `VAR_SAMP` / `STDDEV_POP` / `STDDEV_SAMP` | **空文字** | `NULL` |
>
