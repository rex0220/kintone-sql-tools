...
`WHEN` の条件には `WHERE` と同じ述語を書けます（比較 `= != < >`・`IN`／`NOT IN`・`LIKE`・`BETWEEN`・`AND`／`OR`／`NOT` 等）。**普通の SQL と同じ boolean 述語**として行ごとに評価され、`IN` の一致規則も §6 と同一です（複数値フィールドは「いずれかを含む」判定など）。
...
- したがって、選択系フィールドに**定義に無い選択肢値**を `WHEN … IN (…)` へ渡しても、押し下げされる SIMPLE な `WHERE` と違い**エラーにならず単に不一致**になります（§6 の「定義に無い選択肢値」の挙動差を参照）。
...
- ユーザー・組織・グループ選択・作業者・作成者・更新者は、表示名ではなく `code` で比較します。
- 型情報を取得できない値、型と値の形が一致しない場合は文字列比較を維持します。
- 選択系の空／未設定セルは `IN ('')` に一致し、`NOT IN ('')` で除外されます（空スカラー値の投影も空文字で返します）。

この規則はリテラル／バッチ変数の IN リスト・`IN (SELECT ...)`・サブテーブルの JavaScript 評価すべてに適用され、**SIMPLE 押し下げと FULL_SCAN／`CASE` の JS 評価で一致**します。
...
> 標準SQLの `x IN ('A','B')` は `x = 'A' OR x = 'B'` というスカラー等値で、**`x` の値全体**を `'A'`・`'B'` と突き合わせます。したがって `x` がチェックボックスの `['A','B']` のような集合だと、**値全体は `'A'` でも `'B'` でもないので一致しません**（`['A']` 単独でも同様に不一致）。標準SQLは 1 フィールド＝スカラーを前提とし、多値列という概念がないためです。  
> 一方 kintone のチェックボックス・複数選択は 1 フィールドが複数値なので、kSQL は kintone ネイティブの `in` に合わせ、**集合を要素に開いて「列挙値の少なくとも 1 つを含むか」**（集合の overlap）で判定します。これにより `['A']` も `['A','B']` も `IN ('A','B')` に**一致します**。標準SQLで同義を書くなら中間テーブルの `EXISTS`（セミ結合）や配列 overlap（`checkbox && ARRAY[...]`）に相当します。この解釈は SIMPLE 押し下げと FULL_SCAN／`CASE` の JS 評価で一致します（単一選択・テキスト・数値は従来どおりスカラー等値で、標準SQLと同じです）。
...
> **選択系フィールドに「定義に無い選択肢値」を渡したときの挙動差（重要）**  
> `DROP_DOWN`／`RADIO_BUTTON`／`CHECK_BOX`／`MULTI_SELECT`／ステータスに、そのフィールドの選択肢として**定義されていない値**を `IN`／`NOT IN` へ渡すと、実行モードによって挙動が分かれます。
> - **SIMPLE**（`WHERE` 全体を kintone へ押し下げ）: kintone がクエリを拒否し `GAIA_IQ10`（項目に「値」は存在しません）で**実行エラー**になります。
> - **FULL_SCAN**（関数・算術式などで JavaScript 評価）／**`CASE WHEN` などの条件**（→ §4）: その値は押し下げず JS で評価するため、**エラーにならず単に一致 0 件**（`NOT IN` は全一致）になります。
>
> つまり同じ `WHERE 選択肢 IN ('未定義値')` が、押し下げ有無で「実行エラー／0 件」に分かれます。テキスト・数値フィールドは定義選択肢の概念が無いため、未一致でも常に 0 件（エラーなし）です。**バッチ変数やサブクエリなど動的な値を選択系 `IN` に渡す場合**は、選択肢に無い値が混じると SIMPLE で失敗し得る点に注意してください（事前に有効値へ絞る・`VALIDATE ONLY` で確認する等）。
...
| バッチ変数の高度な利用 | `SET @x = (SELECT ...)`、`DECLARE @x = default` と CLI/MCP 外部注入に対応。`NULL` 代入・1 変数が複数値を持つ配列展開（`IN (@list)`）・サブクエリ結果への算術は現時点で非対応（→ [§25 バッチ変数](#25-バッチ実行と一時テーブル)） |
...
| 規則 | 内容 |
|---|---|
| 構文 | `SET @名前 = <式>`。式は**リテラル**（文字列・数値）・**関数**（`NOW()` / `TODAY()` / 文字列・数値関数）・**数値算術**・**スカラーサブクエリ**（`(SELECT ...)`。Phase 1b。下記）。`@名前` は英字か `_` で始まり英数字・`_`、64 文字以内。大文字小文字は区別しない |
...
#### スカラーサブクエリ代入 `SET @x = (SELECT ...)`（Phase 1b）
...
#### 外部パラメータ注入 `DECLARE @x = default`（Phase 1c）
