...
- v2.6.0 以降、選択系の空／未設定セルは `IN ('')` に一致し、`NOT IN ('')` で除外されます（空スカラー値の投影も空文字で返します）。
...
> 標準SQLの `x IN ('A','B')` は `x = 'A' OR x = 'B'` というスカラー等値です。kintone のチェックボックス・複数選択は 1 フィールドが複数値（集合）のため、kSQL は kintone ネイティブの `in` に合わせ、**「選択集合が列挙値の少なくとも 1 つを含む」**（集合の overlap）と解釈します。標準SQLで同義を書くなら中間テーブルの `EXISTS`（セミ結合）や配列 overlap（`checkbox && ARRAY[...]`）に相当します。この解釈は SIMPLE 押し下げと FULL_SCAN／`CASE` の JS 評価で一致します（単一選択・テキスト・数値は従来どおりスカラー等値で、標準SQLと同じです）。
...
> **v1.4.0 で追加**。**CLI（`-e` / `-f` / `--console`）と MCP（`ksql_query` / `ksql_validate` / `ksql_mutate` / `ksql_explain`）で利用可能**です。プラグイン UI もバッチに対応し（**v1.9.0 で DML を含むバッチも解禁**。v1.4.0〜v1.8.0 は read-only バッチのみ）、最後に結果セットを返した文（通常は最終 SELECT）だけを表示します。詳細仕様は [ksql_batch_temp_table_spec.md](internal/ksql_batch_temp_table_spec.md) を参照してください。
>
