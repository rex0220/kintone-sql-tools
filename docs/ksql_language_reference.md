...
| フィールド型 | `=` | `!=` | `<` | `>` | `<=` | `>=` | `in` | `not in` |
|---|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| `CALC` | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない |
| `CATEGORY` | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない |
| `CHECK_BOX` | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がる | 押し下がる |
| `CREATED_TIME` | 押し下がる | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない |
| `CREATOR` | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない |
| `DATE` | 押し下がる | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない |
| `DATETIME` | 押し下がる | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない |
| `DROP_DOWN` | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がる | 押し下がる |
| `FILE` | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない |
| `GROUP_SELECT` | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない |
| `LINK` | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない |
| `MODIFIER` | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない |
| `MULTI_LINE_TEXT` | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない |
| `MULTI_SELECT` | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がる | 押し下がる |
| `NUMBER` | 押し下がる | 押し下がらない | 押し下がる | 押し下がる | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない |
| `ORGANIZATION_SELECT` | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない |
| `RADIO_BUTTON` | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がる | 押し下がる |
| `RECORD_NUMBER` | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない |
| `RICH_TEXT` | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない |
| `SINGLE_LINE_TEXT` | 押し下がる | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない |
| `STATUS` | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がる | 押し下がる |
| `STATUS_ASSIGNEE` | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない |
| `TIME` | 押し下がる | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない |
| `UPDATED_TIME` | 押し下がる | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない |
| `USER_SELECT` | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない | 押し下がらない |
...
[専用節](#klike--not-klikekintoneキーワード検索)の規則に従います。選択系の `=` は
kintone query が `in` / `not in` だけを受け付けるため押し下がりません。同じ条件を表せる場合は
`IN (...)` を使用してください。
...
|---|---|---|
| `確度 = 'A'`（選択系の `=`） | kintone query が `in` / `not in` しか受け付けない | **`確度 IN ('A')`** |
...
> **サブテーブルの `SELECT` は、常に親レコードを全件取得します**（実測・v3.54.5）。
> **`WHERE` を書いても、`COUNT(*)` だけでも変わりません。**
>
> ```
> SELECT COUNT(*) FROM APP100          → fetch: COUNT_ONLY  （1 リクエスト・転送 1 件）
> SELECT COUNT(*) FROM APP100$明細     → fetch: ALL         （全件取得）
> SELECT COUNT(*) FROM APP100$明細 WHERE _pid = 1
>                                      → fetch: ALL         （WHERE があっても同じ）
> ```
>
> **kintone の `totalCount` は親レコードの件数**で、サブテーブル行を数える手段が API に
> ありません。**行はレコードを取得してから展開する**ため、件数を知るだけでも全件が要ります。
> `WHERE` も kintone へ押し下げず、取得後にローカル評価します。
>
> **大きなアプリでは `maxRecords` に達してエラーになります。**
> 親を絞れる条件があるなら、**親アプリ側の `SELECT` で対象を絞ってから**
> サブテーブルを扱う形（一時テーブル経由など）を検討してください。
