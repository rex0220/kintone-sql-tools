...
正規化された別名を次の段（CTE・一時テーブル）や同じ文の `ORDER BY` / `HAVING` から参照するときは、元の表記でも小文字でも解決されます（v3.78.0 以降。それ以前は小文字で書く必要がありました）。物理フィールドコードは kintone の定義どおり区別します。物理アプリと CTE を結合した文で未修飾の名前が物理フィールドと完全一致するときは物理フィールドが優先されるので、CTE 側の列を指すなら `t.Amount` のように修飾してください。物理フィールドと同名の別名を付けると列名が小文字化されるので、元の表記を出力名に残したいなら別名を付けないでください。詳しい優先順位は [GROUP BY の名前の解決順](#group-by-にエイリアスを指定する) を参照してください。
...
| `COALESCE` | `COALESCE(値1, 値2, ...)` | 最初の非空値を返す。引数がすべて数値なら結果も数値として並べ替え・比較される |
| `ISNULL` | `ISNULL(値, 代替値)` | 値が空なら代替値、それ以外は値（`COALESCE` の 2 引数版）。引数がすべて数値なら結果も数値として並べ替え・比較される |
| `NULLIF` | `NULLIF(値1, 値2)` | 値1 と 値2 が等しければ空文字、それ以外は値1。引数がすべて数値なら結果も数値として並べ替え・比較される |
...
`in` / `not in` へ正規化したうえで押し下げるため ○ です（条件は `in` と同じ＝実在選択肢・空でない値）。
...
この正規化は WHERE の AST を各プランナーで共有する前に行われるため、
**単一表と JOIN の field-vs-literal prefilter（上表）の両方の経路に掛かります**。
...
> JOIN 中の直接 NUMBER 比較は、許可範囲内の数値リテラルであれば `>=` / `<=` を含めて
> そのまま exact prefilter になります。整数境界への手動書き換えは不要です。フィールドを
> 算術式や関数で包むと直接 field leaf ではなくなるため押し下がりません。
...
- `LIKE` はkSQLのワイルドカード／部分一致をJavaScriptで評価します。`KLIKE` はkintoneのキーワード検索であり、半角・全角、大小文字、単語分割などの一致規則はkintoneに準拠します。**SQLの部分一致とは異なる**点に注意してください。
  ...
  - **書込直後の `KLIKE` は結果が変わることがあります。** kintoneの検索索引は書込に対して即時ではないため、**同じバッチで書いた行を直後に `KLIKE` で拾うと、更新後の値では引けず、更新前の値で引けてしまう**ことがあります。エラーにはならず件数が変わるだけなので気づけません。
    ```sql
    UPSERT INTO APP100 (コード, 区分) VALUES (...) ON DUPLICATE (コード);
    SELECT * FROM APP100 WHERE 区分 KLIKE '要確認';  -- 直前に書いた行が入らないことがある
    ```
    **対処は、書込文をバッチの最後に置くことです。** どうしても書いた行を同じバッチで数える／絞り込む必要がある場合は `LIKE`（JavaScript評価）に替えられますが、`LIKE` はFULL_SCANになるため**大規模アプリでは取得上限（`maxRecords`）に達してエラーになります**（下記「性能」参照）。`ASSERT` の件数ゲートを `KLIKE` で書くと、ゲートが素通りしたり、古い条件で誤って成立したりし得ます。
    なお**索引に載る内容が変わったときだけ**起こります。`KLIKE` の対象フィールドを書き換えない更新や `DELETE` では観測されていません。（実測・v3.72.0）
...
実体化列・SELECT 別名に英字がある場合の大文字・小文字の扱いは [基本ルール「大文字・小文字」](#大文字小文字) を参照してください。完全一致を先に試し、その後で小文字へ正規化された列名を解決します。物理 APP のフィールドコードにはこの正規化を適用しません。
...
HAVING に直接書いた集計は、SELECT 列に無くても評価されます（v3.80.0 以降。以前は SELECT に同じ集計がある場合に限り評価され、無いと 0 行 + 警告になりました）。HAVING の集計は出力列にはなりません。v3.16.0 以降の `CASE` 式引数も同じ規則で直接記述でき、SELECT で付けた alias から参照する書き方も有効です。ただし同じ SELECT のウィンドウ列の別名（`RANK() OVER (…) AS 順位` の `順位`）は、HAVING がウィンドウより先に評価されるため参照できません（v3.85.0 以降は `HAVING_WINDOW_ALIAS` で実行前に停止。[§10.1](#101-ウィンドウ関数) を参照）。
...
v3.81.0 から、`GROUP BY` / 集計と同じ SELECT にウィンドウ関数を書けます。ウィンドウが参照できるのはグループキー・集計の別名・集計式・`GROUPING()` です。評価は `GROUP BY` → `HAVING` → ウィンドウの順なので、`HAVING` で除外されたグループは順位や累計に入りません。逆に `HAVING` からは同じ SELECT のウィンドウ列の別名（`HAVING 順位 <= 5`）を参照できず、実行前に `HAVING_WINDOW_ALIAS` で止まります（v3.85.0 以降。v3.81.0〜v3.84.0 は別名が空文字として比較され、`<= 5` は全件・`= 1` は 0 件を静かに返していました）。順位で絞るときは `WITH` で段を分け、次の段の `WHERE` で絞ります。文レベルの `ORDER BY 順位` はウィンドウの後に評価されるので参照できます。
...
- 比較の型規則は WHERE 句と同一（`=` / `<>` は文字列比較、大小比較は双方が数値なら数値比較）。**数値かどうかは値の出どころで決まります**: 数値リテラル・算術式・数値の `@変数`・`COUNT` / `SUM` / `AVG` 等の数値集計サブクエリ＝数値。文字列リテラル・文字列の `@変数`（外部注入含む）＝文字列（辞書順。ISO 日付文字列の比較はこちら）。片方でも文字列なら辞書順です（v3.72.0 で大小比較がこの規則どおりに修正されました。それ以前は数値集計サブクエリの大小比較が辞書順になる不具合があります）
...
- 使える場所: `WHERE` の比較・`SELECT` 列（**別名省略可**）・DML の値・**`INSERT ... VALUES`（v3.72.0〜）**・`SET` / `DECLARE` の右辺など、文字列リテラルが書ける位置。`UPSERT ... VALUES` と APPLY の値では使えません（`INSERT ... SELECT` / `UPSERT ... SELECT` で注入してください）。評価後は通常のリテラルとして扱われ、`WHERE` の押し下げにもそのまま乗ります
...
`@NOW()` 等の as-of 関数は **EXPLAIN / dry-run 経路でも評価**されます（基準時刻は EXPLAIN 開始時に確定。公式 API `explainScript` では `asOf` / `timezone` を注入可能・v3.70.0）。
