...
- DML バッチに read-only 文を混在させた場合、その取得上限も `dmlMaxRows + 1` になる(ksql_mutate は DML 用ツールであり、大きな SELECT は `ksql_query` を使う)。ただし **`CREATE TEMP TABLE` の実体化は例外**で、常に `tempTableMaxRows`(既定 10,000 行)が適用される(§5.6)
- DML の影響件数(insertedCount / updatedCount / deletedCount 等)は `statements[]` の各エントリに展開する。途中失敗時に「どこまで反映されたか」を文ごとに読み取れる
- `INSERT_SELECT`(`INSERT INTO APPxxx ... SELECT`)のソースは **APP・一時テーブル・両者の混在(JOIN・サブクエリ)のいずれも指定できる**(v1.4.0 M4: temp のみ → v1.5.0: APP のみ → v1.7.0: 混在、の段階解禁で制限なしに到達)。source SELECT 実行後・POST 前の confirm で `dmlMaxRows` が適用される。一時テーブルはバッチスコープのため、参照する場合は **`CREATE TEMP TABLE` を同一の `ksql_mutate` バッチ内に含める必要がある**(呼び出しをまたぐ参照は不可)。読み取り上限はソース種類ごとに異なる: APP fetch は `dmlMaxRows + 1`、一時テーブルは実体化上限 10,000 行。経緯は `docs/internal/ksql_mcp_insert_select_app_source_spec.md`(v1.5.0)・`docs/internal/ksql_mcp_insert_select_mixed_source_spec.md`(v1.7.0)
