...
- **`UPSERT_SELECT`(APP ソースのみ)は v1.6.0 で解禁**(単文・バッチとも。既存レコード照合後・書き込み前の confirm で insert + update **合計**に `dmlMaxRows` を適用)。**一時テーブルソースはエンジン未対応**のため引き続き実行前拒否(INSERT_SELECT と異なり temp 経由の迂回路はない)。詳細は `docs/internal/ksql_mcp_upsert_select_unlock_spec.md`
...
| 一時テーブルソースの `UPSERT_SELECT`(v1.6.0 で APP ソースは解禁。temp ソースはエンジン未対応) | `ArgumentError: temp table references in UPSERT_SELECT are not supported yet.` |
