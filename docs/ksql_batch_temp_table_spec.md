...
- 更新履歴:
  ...
  - 2026-07-09 R35(M6): 実機検証完了(プラグイン A〜E 系・CLI・MCP)。公開ドキュメントの「v1.4.0 予定」表記を確定に変更。検証中に発見・修正したバグ3件: INSERT_SELECT 確認ダイアログの誤表示(R31)/ HAVING × 集計 alias で全滅(R32・v1.3.0 からの既存バグ)/ 非 Error reject の [object Object](R34)
  - 2026-07-09 R33(プラグイン対応): §8.4 を新設し、プラグイン UI の read-only バッチ(最終結果のみ表示)に対応。「最終結果のみに割り切れば表示 UI の再設計が不要」という実機検証中の指摘を採用。EXPLAIN ボタンのバッチ入力はプラン表示モード(前置方式だと2文目以降が実行される穴を同時に塞ぐ)
  - 2026-07-09 R29(M2 実装後): CLI の DML バッチを実装(§8.3)。確認プロンプトはバッチ全体で1回(全 DML 文の一覧: タイプ / 対象アプリ / WHERE 有無)、`--yes` でスキップ。console からの実行は REPL 側で同一の一覧確認後、子実行に `--yes` を付与。文ごとの `--dml-max-rows` は confirm 経由で適用。これでフェーズ2 のコードステップ(M1〜M4)完了
...
- ステータス: **実装・実機検証完了(v1.4.0)**。フェーズ1(S1〜S8)・フェーズ2(M1〜M4)・プラグイン対応(§8.4)。bulkRequest(M5)は v1.4.0 では見送り(P0-3 の実機スパイクとセットで v1.5 以降に判断)
...
- `dmlTotalMaxRows` の集計対象は INSERT(VALUES 行数を静的に加算)と、confirm を呼ぶ文種 = UPDATE / DELETE / **`INSERT_SELECT`(source 行数)** / **UPSERT / `UPSERT_SELECT`(insert + update 合計)** / REORDER(対象親件数)の実行時加算。UPSERT は既存レコード照合後・書き込み前に `confirm(toInsert + toUpdate, "UPDATE")` が呼ばれるため対象(`src/execute.ts` executeUpsert。初回公開コミットから実装済み)。静的カウント(`getInsertValuesCount`)は INSERT のみのため二重計上はない
- DML バッチの読み取り上限は文種構成で決まる(v1.8.0): **SELECT-based DML(INSERT_SELECT / UPSERT_SELECT)を1文でも含むバッチは runtime の通常 `maxRecords` 解決(既定 500)**、含まないバッチは従来どおり `dmlMaxRows + 1`。read-only 文を混在させた場合の取得上限も同じ値になる(ksql_mutate は DML 用ツールであり、大きな SELECT は `ksql_query` を使う)。ただし **`CREATE TEMP TABLE` の実体化は例外**で、常に `tempTableMaxRows`(既定 10,000 行)が適用される(§5.6)
- DML の影響件数(insertedCount / updatedCount / deletedCount 等)は `statements[]` の各エントリに展開する。途中失敗時に「どこまで反映されたか」を文ごとに読み取れる
- `INSERT_SELECT`(`INSERT INTO APPxxx ... SELECT`)のソースは **APP・一時テーブル・両者の混在(JOIN・サブクエリ)のいずれも指定できる**(v1.4.0 M4: temp のみ → v1.5.0: APP のみ → v1.7.0: 混在、の段階解禁で制限なしに到達)。source SELECT 実行後・POST 前の confirm で `dmlMaxRows` が適用される。一時テーブルはバッチスコープのため、参照する場合は **`CREATE TEMP TABLE` を同一の `ksql_mutate` バッチ内に含める必要がある**(呼び出しをまたぐ参照は不可)。読み取り上限はソース種類ごとに異なる: APP fetch は runtime `maxRecords`(既定 500。v1.8.0 で `dmlMaxRows + 1` から分離 — `docs/internal/ksql_mcp_dml_source_read_limit_issue.md`)、一時テーブルは実体化上限 10,000 行。経緯は `docs/internal/ksql_mcp_insert_select_app_source_spec.md`(v1.5.0)・`docs/internal/ksql_mcp_insert_select_mixed_source_spec.md`(v1.7.0)
- **`UPSERT_SELECT` は v1.6.0 で APP ソースを解禁**(単文・バッチとも。既存レコード照合後・書き込み前の confirm で insert + update **合計**に `dmlMaxRows` を適用。詳細は `docs/internal/ksql_mcp_upsert_select_unlock_spec.md`)、**v1.7.0 で一時テーブル・混在ソースも解禁**(INSERT_SELECT と同じ注入経路)。注意: 一時テーブルに実体化して回避できるのは **source / JOIN 側の APP 読み取り上限のみ**で、書き込み先 APP への既存レコード照合読み取りはソース種類に関わらず残る
...
| DML(UPDATE / DELETE / UPSERT)内の一時テーブル参照 | `ArgumentError: temp table references in UPDATE are not supported yet.` |
