...
const result = await runQuery("SELECT 'ok' AS status, 21 AS release", { client, maxRecords: 3000, cursorMaxActive: 2 });
if (version !== "${exactVersion}" || result.rows[0].status !== "ok" || result.rows[0].release !== "21") process.exit(1);
`;
...
(async () => {
  const client = createReadonlyKintoneClient();
  const result = await runQuery("SELECT 'ok' AS status, 21 AS release", { client });
  if (version !== "${exactVersion}" || result.rows[0].status !== "ok" || result.rows[0].release !== "21") process.exit(1);
...
`;
  ...
  const result = await engine.runQuery(
    "SELECT 'ok' AS status, 21 AS release",
    { client }
  );
  assert(
    result.rows[0].status === "ok" && result.rows[0].release === "21",
