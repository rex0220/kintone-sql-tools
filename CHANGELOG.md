...
- **互換性注意（minor）**: 従来 `VALIDATE ONLY` で valid だった桁超過行が invalid へ、`ON ERROR SKIP` では API エラー前に `#err` へ移る場合がある。また NUMBER を書き込む credential には一般設定取得 API を利用できるレコード閲覧権限または追加権限が必要で、設定を取得できない場合は文全体が失敗する。
