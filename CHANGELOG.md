...
- v3.81.0（B184-B）で「ウィンドウ関数の結果を同じ SELECT の式の中で使える」ようになりましたが、**元が CTE か一時テーブル**（実体化テーブル）の非集計 SELECT で、
  **`CASE` の `WHEN` 条件の左辺**にウィンドウ関数を書いた形（`CASE WHEN SUM(売上合計) OVER () = 0 THEN 0 ELSE … END`・`CASE WHEN RANK() OVER (…) <= 3 THEN …`）だけが
  `ArgumentError: unknown field code(s): __ksql_window_0 (base)` で止まっていました。CASE 条件の左辺を歩く取得列収集だけが隠しウィンドウ列を除外しておらず、
  実体化テーブルの列存在検査（B86）が内部名を未知の列として拒否していたためです。
- **落ちていた形（修正対象）**: 元が `WITH` の CTE または `#一時テーブル` で、集計を持たない SELECT の `CASE` 条件の左辺にウィンドウ関数を置いた形。
  典型は「集計を `base` に分け、次の SELECT で `CASE WHEN … OVER () …` と書く」標準 SQL の書き方です。
- **元から通っていて変わらない形**: 算術・関数の引数・`CASE` の `THEN` / `ELSE` の中のウィンドウ（元が CTE／一時テーブルでも）、元が物理アプリの `CASE` 条件、
  集計と同じ SELECT に書いたウィンドウ（B184-A）、ウィンドウを列に出してから次の段で `CASE` に使う従来形。
- 元が物理アプリの場合、これまで内部名 `__ksql_window_n` が kintone の `fields` パラメータに混ざっていました（kintone が無視するため結果は不変）。修正後は混ざりません。
- 実機（dev profile・SFA パック）: 第 3 回の「標準 SQL ならこう書く」を kSQL の名前に置き換えた CTE 版と一時テーブル版が、3 段版と同じ 10 行を返すことを確認。

## v3.82.0（2026-09-16）

### 改善（B188 残務: プラグインの実行画面に先行文の警告を表示）**※ プラグイン UI のみ・エンジン不変**

- バッチを実行したとき、プラグインは最後に結果セットを返した文だけを表示します（従来どおり）。v3.79.0 で `CREATE TEMP TABLE … AS SELECT` などの
  実体化警告（ウィンドウの既定フレーム RANGE など）が文結果の `warnings` に載るようになりましたが、結果セットを持たない文はサマリ行から
  除外されていたため、プラグインでは画面のどこにも出ていませんでした（CLI・MCP・`/flow` には届いていた）。
- 修正後: 表示中の最終結果の警告欄に、先行文の警告が `[文番号] <警告文>` の形で前置されます。対象は結果セットを持たない文の警告、
  表示されない途中の SELECT の警告、dialect 1 のバッチ警告（接頭辞なし）です。表示結果自身の警告は従来どおり接頭辞なしで、重複しません。
  結果セットの無いバッチでも情報行の後ろに同じ形式で出ます。単文の表示・サマリ行・警告文の文言・エンジンは変わりません。
...
- 影響する形（v3.79.0 以前）: `SELECT 商談フェーズ, COUNT(*) AS n FROM APP4149 GROUP BY 商談フェーズ HAVING SUM(売上) > 1000000` のように
  **HAVING に直接書いた集計が SELECT 列に無い**文は、契約どおり評価されず **0 行**になり、`warnings` に
  「比較条件で参照した集計値を確認できません。SELECT リストに同じ集計式を含めてください。」が付いていました。`ROLLUP` / `GROUPING SETS` でも同じ。
- 修正後: 標準 SQL と同じ意味で評価されます（上の例は 3 行）。HAVING の集計は出力列にはならず、取得列・kintone API の回数・EXPLAIN の行は変わりません。
  HAVING 内の集計は単体・算術（`SUM(a) - SUM(b) > 0`）・`CASE` 引数・文字列関数の引数のいずれの位置でも評価されます。この形では上の警告は出なくなります。
- 元から正しく変わらない形: SELECT に同じ集計がある文、SELECT の別名を HAVING で参照する文（`HAVING 合計 > 100`）。
  GROUP BY も SELECT の集計も無い `HAVING` は従来どおり構文エラーです。
- 言語リファレンス §9 の契約文を「SELECT 列に無くても評価される（v3.80.0〜）」に書き換えました。
...
**件数ガードが fail-open になり得た正しさの修正です**（最適化ではありません）。ksql-flow からの
報告（実測マトリクス付き）で発覚し、起票前に両方向を実機再現して確認しました。

```sql
ASSERT (SELECT COUNT(*) FROM APP100) <= 9;
-- 旧: 12 件でも成立（'12' < '9' の辞書順）＝大量更新前ガードが素通り
-- 新: 12 件で AssertError（10 進の数値比較）
```

- **原因**: 比較の型（数値/文字列）を左オペランドの AST 型だけで決めており、`COUNT(*)` 等の
  スカラーサブクエリの数値性が捨てられて文字列（コードポイント順）比較になっていました。
  等値 `=` だけ正常に見えるのは「桁が揃うと文字列比較でも正答する」ため（B119-B122 と同じ罠）。
  ASSERT 導入時からの不具合で、`ASSERT WARN` / `EXIT SUCCESS IF` の条件部・`BETWEEN` も同一でした
- **影響していた形（今回から正しくなる）**: 大小比較（`<` `<=` `>` `>=`）・`BETWEEN` で、
  両辺が数値（数値リテラル・算術式・数値の `@変数`・`COUNT`/`SUM`/`AVG` 等の数値集計サブクエリ）
- **変わらないもの（元から正しい経路）**: `=` / `!=` / `<>`・文字列同士の大小（辞書順＝
  `ASSERT @d >= '2026-01-01'` の ISO 日付比較を含む）・文字列の `@変数`（外部注入含む）・
  算術式の計算自体・空文字等の既存規則。数値性は**値の出どころ**で決まり、実値からの推測はしません
- レシピ R1 の `BETWEEN 0 AND 10000` ゲート等、公開例の件数ガードが桁跨ぎでも意図どおり動きます

### 機能追加（B171 F-2: dialect 1 の `INSERT ... VALUES` で as-of 関数）**※純加法**

`@NOW()` / `@TODAY()` / `@MONTH_START()` / `@NEXT_MONTH_START()` を `INSERT ... VALUES` の値に
書けます（シード投入・バックフィル用）。実行・`previewStatement`・`EXPLAIN` の 3 面で同値に
展開されます。`UPSERT ... VALUES` / APPLY の値は対象外（拒否時に文型限定のヒントを表示・
`INSERT ... SELECT` / `UPSERT ... SELECT` で注入してください）。dialect 0 は従来どおりです。
