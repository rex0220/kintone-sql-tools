...
- **B29 kintone の数値精度設定と DML/Tier-0 を整合**。書き込み先に NUMBER 列がある INSERT / UPSERT / UPDATE では運用環境の `app/settings.json` から `numberPrecision`（`digits` / `decimalPlaces` / `roundingMode`）を取得し、通常書き込み・`VALIDATE ONLY`・`ON ERROR SKIP` の全経路で同じ厳密10進 primitive による整数部／小数部の桁検証を行う。取得は同一実行コンテキスト・アプリごとに最大1回で、失敗・不正レスポンス・未知モードでは既定値を補わず fail-closed する。CALC の直接指定は従来どおり書き込み不可で、ソース側 CALC だけを理由に設定は取得しない。
- 桁超過は `ERR_NUMBER_INTEGER_DIGITS` / `ERR_NUMBER_DECIMAL_PLACES` で API 呼び出し前に診断し、入力値を暗黙に丸めない。HALF_EVEN / UP / DOWN の文字列ベース量子化 primitive は追加するが通常 DML からは呼ばず、既存 `ROUND` の意味も変更しない。`+ - * / %`、集計、数値関数、CASE、CALC 由来値の計算は引き続き binary64 であり、検証器へ到達した最終10進文字列だけを判定する。
- **互換性注意（minor）**: 従来 `VALIDATE ONLY` で valid だった桁超過行が invalid へ、`ON ERROR SKIP` では API エラー前に `#err` へ移る場合がある。とくに**小数超過**は、これまで kintone が書き込みを受理して暗黙に `decimalPlaces` 桁へ丸めていた（実機確認: `12.34567` は保存時 `12.3457` になっていた）が、v3.3.0 以降は通常 DML でも `ERR_NUMBER_DECIMAL_PLACES` で拒否し、静かな丸めによるデータ改変を防ぐ。整数部超過は従来から kintone が `CB_VA01` で拒否していた挙動を、書き込み前のローカル診断へ前倒しする。また NUMBER を書き込む credential には一般設定取得 API を利用できるレコード閲覧権限または追加権限が必要で、設定を取得できない場合は文全体が失敗する。証跡: docs/internal/evidence/b29_number_precision_smoke.md
