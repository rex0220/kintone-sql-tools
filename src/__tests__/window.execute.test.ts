...
describe("B128: LAG / LEAD execution", () => {
  ...
  test("B140-C: CTE / 一時テーブル経由では実行できない助言を出さない", async () => {
    ...
    // 助言だけでは「従っても消せない」ため、限界も併記する（依頼元の指摘・2026-08-06）。
    // GROUP BY キーで並べている場合はすでに一意だが、エンジンは証明できない。
    const cteAdvice = "ウィンドウの各パーティション内で、ORDER BY の値の組が入力行を一意に識別するとクエリ構造または保証済みのデータ制約から確認できる場合に限り、この警告は無視できます。"
      // B140 の肯定例を残しつつ、B160/B53 にも同じ判断条件を適用する。
