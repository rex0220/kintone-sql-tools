...
test.each(["STDDEV_POP", "STDDEV_SAMP", "VAR_POP", "VAR_SAMP", "MEDIAN"])(
  "B56: CHECK 内の %s は既存集約と同じ明示エラーで拒否する",
  async (func) => {
    await expect(execute(
      `INSERT INTO APP1 (a) VALUES (1) CHECK WHEN ${func}(a) > 0 THEN 'bad' VALIDATE ONLY`,
      client({}, { a: "NUMBER" })
