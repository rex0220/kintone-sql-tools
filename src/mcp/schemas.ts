...
const onLimit = z.enum(["error", "truncate"])
  .describe("Behavior when maxRecords is exceeded: 'error' rejects, 'truncate' returns the first maxRecords rows (default 'error'). ORDER BY and VALIDATE ONLY require complete input and therefore override 'truncate' to 'error'.")
  .optional();
