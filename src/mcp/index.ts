...
- Window functions may share a SELECT with aggregates and appear inside expressions (v3.81.0+): OVER (...) may reference only group keys, aggregate aliases, aggregate expressions, or GROUPING(); a non-aggregate alias in OVER (ORDER BY ...) is rejected, so split stages with WITH or temp tables when needed. Running totals: ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW plus a tie-break on the key.
...
export function createServer(args: ServerArgs): McpServer {
  ...
  server.registerTool("ksql_validate", {
    title: "Validate kSQL",
    description: "Parse and validate kSQL without calling kintone APIs. Statically recognizes read-only WITH RECURSIVE, recursive CTE column lists, UNION ALL self-reference shape, and optional single-column CYCLE; schema-dependent recursive column types and runtime boundaries are finalized by ksql_query/ksql_explain. WITH CTE-body GENERATE_SERIES integer/DATE literals are checked statically; DATE step supports signed day/month/year coefficients with month-start/year-start anchors. Variable-dependent type, step, and 10000-row-limit decisions are finalized by execution. All APPLY forms (UPDATE/INSERT/UPSERT/multi-value) are accepted for syntax and static validation, but this does not enable APPLY mutation. Relative-date validation here covers syntax and arguments only; ksql_query/ksql_explain/runtime makes the final schema-aware decision. Use this before executing generated SQL. Do not use validate probing to discover functions or syntax; call ksql_docs instead. IMPORT CSV/JSON is enabled only when named inline importSources are supplied.",
    inputSchema: validateInputShape,
  }, tools.validateTool);

  server.registerTool("ksql_explain", {
    title: "Explain kSQL",
    description: "Return the schema-aware kSQL execution plan, including read-only WITH RECURSIVE/CYCLE and every APPLY form (UPDATE/INSERT/UPSERT/multi-value). Recursive plans show strategy, path scope, the effective recursiveCteMaxDepth/recursiveCteMaxRows/recursiveCteMaxExpansions boundaries, and the empty-key runtime check. Reads form metadata and, when needed, process status metadata; never calls records or mutation APIs. IMPORT CSV/JSON is enabled only when named inline importSources are supplied.",
    inputSchema: explainInputShape,
  }, tools.explainTool);

  server.registerTool("ksql_query", {
    title: "Run read-only kSQL",
    description: "Execute read-only kSQL: SELECT, WITH (including WITH RECURSIVE/CYCLE), UNION, EXPLAIN, SHOW APPS, DESCRIBE, ASSERT, leading VALIDATE app existing-record audits, and INSERT/UPSERT/UPDATE ... VALIDATE ONLY. Recursive CTEs allow one UNION ALL recursive definition with one self INNER JOIN and optional single-column path-scoped CYCLE; recursive DML sources, mutual/multiple recursion, and path output are unsupported. recursiveCteMaxDepth (default 100), recursiveCteMaxRows (default 10000), and recursiveCteMaxExpansions (default 100000) are always fail-closed; CYCLE is not an expansion boundary and onLimit=truncate cannot relax them. WITH name AS (GENERATE_SERIES(start, stop [, step]) [AS column]) creates integer/DATE series without kintone reads; DATE step supports signed day/month/year coefficients, with month start anchored to YYYY-MM-01 and year start to YYYY-01-01. It uses a fixed 10000-row total guard per WITH. Read-only validation tails are DML ... VALIDATE ONLY [INTO #err] and leading VALIDATE APPn ... [INTO #err]; INTO is batch-only. ASSERT failure always stops the batch. Local ORDER BY plans require complete input and fail instead of returning a truncated top-N; REST top-N and KORDER_NATIVE do not fetch a partial candidate set. VALIDATE and VALIDATE ONLY always treat onLimit=truncate as error and perform zero write API calls. UPDATE/INSERT/UPSERT/multi-value APPLY VALIDATE ONLY evaluates dmlMaxSubtableRows with the fixed default 500; ksql_query does not expose an override. Existing-record VALIDATE applies built-in form constraints plus optional CHECK groups and can materialize its diagnostic columns with INTO #err in a batch (nine columns, or five with SUMMARY). NUMBER targets use the app numberPrecision settings for integer-digit validation and fail closed if settings cannot be read. Excess fractional digits pass through for kintone to round automatically. Supports multi-statement batches with temp tables, including VALIDATE ONLY INTO #err for later SELECT. APPLY mutation remains fail-closed; mutating DML is rejected. For kSQL dialect details, follow the server instructions and read ksql://language-reference (or call ksql_docs when resources are unavailable).",
  ...
  server.registerTool("ksql_run_saved_query", {
    ...
    description: "Run a saved kSQL query. Read-only queries and batches, including WITH CTE-body GENERATE_SERIES integer/DATE series with signed day/month/year DATE steps, use ksql_query safety and support DECLARE variable injection; DML queries require ksql_mutate safety inputs.",
